...
(i.e. H.8 2.1.5G out).
 ...
 \item update  14.01/07 H.10, 2.1.8-t: for comparison with ADAS PRB \\
  ...
  \item update Dec. 08 2011:  O ionisation, O+ recombination: ADAS 96, H4 and H10 fits added 2.8A0 and 2.3.8A0, resp.
  \item update Dec. 09 2011:  N ionisation, N+ recombination: ADAS 96, H4 and H10 fits added 2.7A0 and 2.3.7A0, resp.
...
\subsection*{Reading Data from AMJUEL}
\addcontentsline{toc}{subsection}{Reading Data from AMJUEL}
...
Fits for $\langle\sigma \cdot v  \rangle (T) \ [cm^3/s]$

Maxwellian rate coefficients are taken for neutral particle energy = 0.0 eV
vs. temperature $T$ (electron or ion temp., resp.) of the Maxwellian $f_{maxw}$.
I.e. :
...
Here $M$ is the mass of the ion as used in the rate coefficients,
...
Fits for $\langle\sigma \cdot v_{rel}  \rangle (E,T) \ [cm^3/s]$
...
Fits for $\langle\sigma \cdot v  \rangle (n_e,T) \ [cm^3/s]$

Same expression of fit as in Section \ref{sec1.3.3}, but with energy
$E$ (eV) replaced by density $n$. For historic reasons and to
preserve backward compatibility, the density $n$ in this fit must be
given in $10^8 cm^{-3}$, i.e., with density given in $cm^{-3}$ and
then divided by $10^8$, or density given in $m^{-3}$ and then
divided by 100.
...
Langevin cross section, $\sigma \times v_{rel} = const = 2e-8$

\begin{small}\begin{verbatim}
   a0 -3.187171717171e 01    a1 -0.500000000000e-00    a2  0.000000000000e-00
...
\subsection{
Reaction 2.2.5
$\rm e + H_2(X_g\!^+ S ) \rightarrow . . .\rightarrow e + H(1s) + H(1s)$}
...
\end{verbatim}\end{small}
same as 3-2-3, above, previous reaction, but evaluated with old $H_2$ energy: 0.37 eV, rather than 0.1 eV.
Kept here only for backward compatibility.(Mostly used in ITER applications).
\newpage
...
Reaction 2.7B0   $e + N \rightarrow e + N^+ + e  $
  ...
  Ionization rate for neutral Nitrogen Atoms
  (Bell et al., CLM-R216
...
\subsection{
  Reaction 2.1.8JH  $ p + electron(s) \rightarrow H(1s) + ...$
}
...
\subsection{
Reaction 3.2.3r   $p + e+ H_2   \rightarrow H + H + H  $ (MAR via $H_2^+$ for cold $H_2$)
}

$H_2$ multi-step model, MAR rate coefficient, $H_2^+$ condensed

 Data: Sawada/Fujimoto ,\cite{kn:Sawada}

$  H(1), H_2, H^+ $ transported, $H_2^+$ in QSS with $H_2$, $E_{H_2}=0.1$ eV


$H_2$ is in QSS vibrational distribution as fct. of $T_e$
...
\subsection{
Reaction 3.2.3d   $p + H_2   \rightarrow p + H + H  $ (MAD via $H_2^+$ for cold $H_2$)
}

$H_2$ multi-step model, MAD rate coefficient, $H_2^+$ condensed
...
\subsection{
Reaction 3.2.3i   $p + H_2   \rightarrow p + p + H +e $ (MAI via $H_2^+$ for cold $H_2$)
}

$H_2$ multi-step model, MAI rate coefficient, $H_2^+$ condensed
...
$H^*$ production based on HYDHEL 7.2.2, 7.2.3 (not included 7.2.1).
...
$  \langle \sigma*v_{rel} \rangle(T_e,n_e) (cm^3/s)$ \\
Assume low energy of projectile, $E(H^-)=0.1$ eV, $T_e = T_i$.
$H^*$ production based on HYHDEL 7.2.2, 7.2.3 (not included 7.2.1).
...
\section{H.8 : Fits for $\langle\sigma \cdot v \cdot E \rangle (T) \ [cm^3/s
\cdot eV]$}

$E$ is a relevant energy related to the process, e.g. it may be the impacting electron or ion in eV,
...
\section{H.9 :Fits for $\langle\sigma \cdot v \cdot E_{kin} \rangle (E,T) \ [cm^3/s
\cdot eV]$}

\subsection{
Reaction 3.1.8   $p + H(1s)  \rightarrow  H(1s) + p $
}
$E_{kin}$ is the kinetic energy of the impacting ion in eV.
...
This page is intentionally left blank
\newpage
\section{H.10 :Fits for $\langle\sigma \cdot v \cdot E \rangle (n_e,T) \
[cm^3/s \cdot eV]$}
...
\subsection{
  Reaction 2.1.8t $p + electron(s)  \rightarrow H(1s) + ....$
}
...
\subsection{
  Reaction 2.3.7A0  $N^+ + electron(s) \rightarrow N   + ... $
}
...
\subsection{
  Reaction 2.3.8A0  $O^+ + electron(s) \rightarrow O   + ... $
}
...
\subsection{
Reaction 2.0b   Ratio $H_2^+/H_2,$ gain of $H_2^+$:  CX on cold $H_2(v=0)$ only
}

Note: the contribution 2.0a to this ratio from EI processes alone is electron density dependent
and given in the next chapter under the ``H12 fits''.
\medskip

same as 2.0c below, but $H_2^+$ production by CX (ion conversion, proton impact) only from vibr. ground state $H_2(v=0)$
rather than from vibrationally distributed $H_2(v)$.
...
\subsection{
Reaction 2.0c   Ratio $H_2^+/H_2,$ gain of $H_2^+$:  CX on cold $H_2(v)$
}
...
$E_{H2} = 0.1$ eV, $T_i = T_e$, vibrational distribution with
...
\subsection{
Reaction 7.0a      Ratio $H^-/H_2: $ gain of $H^-$:  DA on cold $H_2(v)$
}



  Ratio of densities: $n_{H^-}/n_{H_2}$. \\
  vibr. distribution of $H_2(v)$: coll. model for vibr. excitation, Greenland, Reiter, \cite{kn:Green}\\
  ...
  Destruction of $H^-$: .... with $E_{H^-}=0.1$ and $T_p = T_e$ and $n_p = n_e$.
...
\subsection{
Reaction 7.0b      Ratio $H^-/H_2: $ gain of $H^-$:  DA on cold $H_2(v=0)$ only
}
...
Assumed:  $n^+ = n_e$. This is the invers ratio of the ratio H$^+$/H given
above.
\begin{small}\begin{verbatim}
...
\subsection{
  Reaction 2.1.8det  $p + electron(s) \rightarrow H(1s) + ...   <de>+13.6 \ [eV] $
}
...
\subsection{
  Reaction 2.1.8o $p + electron(s) \rightarrow H(1s) + ... <de>+13.6 \ [eV] $
  Ly-opaque
}
...
\newpage
\subsection{
Reaction 7.2e $ H^- + p \rightarrow H(1) + H(6) , \  $
Ratio $H(6)/H^- $, cold $H^-$
}
 Multi-step hydrogenic population coefficients
 Data: T.Fujimoto, P.T. Greenland
...
\subsection{
Reaction 2.0a Ratio $H_2^+/H_2$, from electr. impact only
}
 ...
 Data: T.Fujimoto, P.T. Greenland
  ...
  only contribution from  EI on H2 and H2*
...
\newpage
\subsection{
Reaction 2.0b Ratio $H_2^+/H_2$, from EI plus CX on cold $H_2(v=0)$
}
...
\subsection{
Reaction 2.0c   Ratio $H_2^+/H_2$ from EI plus CX on cold $H_2(v)$}
...
\bibitem{kn:Janev} R.K. Janev et al.  Springer Series on Atoms+Plasmas,
                   Vol 4, 1987
