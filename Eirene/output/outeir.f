...
C
      IF (XMCP(ISTRA).LT.1.) THEN
        WRITE (IUNOUT,*) 'OUTEIR CALLED, EIRENE_ISTRA, XMCP: ',
     .                    ISTRA,XMCP(ISTRA)
...
C   REFLECTED FLUX
     ...
     .  ('SUM OVER SPECIES                               ')
      ...
      IF (.NOT.LSPTAT) THEN
        IF (LMSSPTAT) THEN
        CALL EIRENE_MASAGE
     .  ('FLUX SPUTTERED BY ATOMS:                       ')
        CALL EIRENE_MASAGE
     .  ('SPTAT   SWITCHED OFF => MISSING IN BALANCE     ')
        ENDIF
      ELSE IF (ANY(SPTATI(1:NATM,ISTRA).NE.0.D0)) THEN
        CALL EIRENE_MASAGE
     .  ('FLUX SPUTTERED BY ATOMS:                       ')
        CALL EIRENE_MASYR1 ('SPTATI = ',
     .                SPTATI,LOGATM,ISTRA,0,NATM,0,NSTRA,TEXTS(NSPH+1))
        CALL EIRENE_MASAGE
     .  ('SUM OVER SPECIES                               ')
        CALL EIRENE_MASR1 ('TOTAL=  ',SPTATI(0,ISTRA))
      ENDIF
      DO 405 IATM=0,NATMI
        DIFA(IATM,ISTRA)=
...
C  REFLECTED MOLECULES
     ...
     .  ('SUM OVER SPECIES                               ')
      ...
      IF (.NOT.LSPTML) THEN
        IF (LMSSPTML) THEN
        CALL EIRENE_MASAGE
     .  ('FLUX SPUTTERED BY MOLECULES:                   ')
        CALL EIRENE_MASAGE
     .  ('SPTML   SWITCHED OFF => MISSING IN BALANCE     ')
        ENDIF
      ELSE IF (ANY(SPTMLI(1:NMOL,ISTRA).NE.0.D0)) THEN
        CALL EIRENE_MASAGE
     .  ('FLUX SPUTTERED BY MOLECULES:                   ')
        CALL EIRENE_MASYR1 ('SPTMLI = ',
     .                SPTMLI,LOGMOL,ISTRA,0,NMOL,0,NSTRA,TEXTS(NSPA+1))
        CALL EIRENE_MASAGE
     .  ('SUM OVER SPECIES                               ')
        CALL EIRENE_MASR1 ('TOTAL=  ',SPTMLI(0,ISTRA))
      ENDIF
      DO 415 IMOL=0,NMOLI
        DIFM(IMOL,ISTRA)=
...
C  REFLECTED TEST IONS
     ...
     .  ('SUM OVER SPECIES                               ')
      ...
      IF (.NOT.LSPTIO) THEN
        IF (LMSSPTIO) THEN
        CALL EIRENE_MASAGE
     .  ('FLUX SPUTTERED BY TEST IONS:                   ')
        CALL EIRENE_MASAGE
     .  ('SPTIO   SWITCHED OFF => MISSING IN BALANCE     ')
        ENDIF
      ELSE IF (ANY(SPTIOI(1:NION,ISTRA).NE.0.D0)) THEN
        CALL EIRENE_MASAGE
     .  ('FLUX SPUTTERED BY TEST IONS:                   ')
        CALL EIRENE_MASYR1 ('SPTIOI = ',
     .                SPTIOI,LOGION,ISTRA,0,NION,0,NSTRA,TEXTS(NSPAM+1))
        CALL EIRENE_MASAGE
     .  ('SUM OVER SPECIES                               ')
        CALL EIRENE_MASR1 ('TOTAL=  ',SPTIOI(0,ISTRA))
      ENDIF
      DIFI(0,ISTRA)=0.
      TOTI(0)=0.
      DO 425 IION=0,NIONI
...
C   REFLECTED FLUX
     ...
     .  ('SUM OVER SPECIES                               ')
      ...
      IF (.NOT.LSPTPHT) THEN
        IF (LMSSPTPHT) THEN
        CALL EIRENE_MASAGE
     .  ('FLUX SPUTTERED BY PHOTONS:                     ')
        CALL EIRENE_MASAGE
     .  ('SPTPHT   SWITCHED OFF => MISSING IN BALANCE    ')
        ENDIF
      ELSE IF (ANY(SPTPHTI(1:NPHOT,ISTRA).NE.0.D0)) THEN
        CALL EIRENE_MASAGE
     .  ('FLUX SPUTTERED BY PHOTONS:                     ')
        CALL EIRENE_MASYR1 ('SPTPHTI = ',
     .                SPTPHTI,LOGPHOT,ISTRA,0,NPHOT,0,NSTRA,TEXTS( +1))
        CALL EIRENE_MASAGE
     .  ('SUM OVER SPECIES                               ')
        CALL EIRENE_MASR1 ('TOTAL=  ',SPTPHTI(0,ISTRA))
      ENDIF
...
C  PHOTON PLASMA INTERACTION
     ...
     .  ('SUM OVER SPECIES                               ')
      ...
      IF (.NOT.LSPTPL) THEN
        IF (LMSSPTPL) THEN
        CALL EIRENE_MASAGE
     .  ('FLUX SPUTTERED BY BULK IONS:                   ')
        CALL EIRENE_MASAGE
     .  ('SPTPL    SWITCHED OFF => MISSING IN BALANCE    ')
        ENDIF
      ELSE IF (ANY(SPTPLI(1:NPLS,ISTRA).NE.0.D0)) THEN
        CALL EIRENE_MASAGE
     .  ('FLUX SPUTTERED BY BULK IONS:                   ')
        CALL EIRENE_MASYR1 ('SPTPLI = ',SPTPLI,
     .                LOGPLS,ISTRA,0,NPLS,0,NSTRA,TEXTS(NSPAMI+1))
        CALL EIRENE_MASAGE
     .  ('SUM OVER SPECIES                               ')
        CALL EIRENE_MASR1 ('TOTAL=  ',SPTPLI(0,ISTRA))
      ENDIF
      CALL EIRENE_LEER(2)
