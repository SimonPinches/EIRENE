...
C
      DO 5 IF=1,IRAPS
        NRPS=60+IF
        OPEN (UNIT=NRPS+ifoff,ACCESS='SEQUENTIAL',FORM='FORMATTED')
        REWIND NRPS+ifoff
5     CONTINUE
C
C  3D
         ...
         do ir=1,nr1st-1
            do ip=1,np2nd-1
               do it=1,nt3rd-1
                  ...
                  WRITE(18+ifoff,'(1X,A1,8I10)') '0',ird1,ird2,ird3,
     .                                        ird4,ird5,ird6,ird7,ird8
               enddo
            enddo
         enddo
...
!pb     .         'PSS','1BEISPIELDATEN',(ANZ+nstab)*(iplane-1)
          WRITE(18+ifoff,'(A3,I3,A60,I6)')
     .         'PSS',1,'BEISPIELDATEN',-3334
        else
!pb          WRITE(18+ifoff,'(A3,2X,A14,45X,I8)')'PSS','1BEISPIELDATEN',
!pb     .                                   (ANZ+NSTAB)*iplane
          WRITE(18+ifoff,'(A3,I3,A60,I6)')'PSS',1,'BEISPIELDATEN',-3334
        endif
...
!pb     .        WRITE(18+ifoff,'(A8,2I6,5X,A1)') 'HEXE8        ',ipl+1,ANZ,'8'
            if (ipl < iplane-1)
     .        WRITE(18+ifoff,'(A8,I6,I9,I6)') 'HEXE8   ',ipl+1,ANZ,8
            igroups = iplane-1
          else
!pb            WRITE(18+ifoff,'(A8,2I6,5X,A1)') 'QUAM4        ',ipl+1,ANZ,'4'
...
50          CONTINUE
     ...
     .              XTRIAN(I),YTRIAN(I),ipl*rapsdel
              ...
              WRITE(17+ifoff,'(I6,1P,2E12.4)') I,XTRIAN(I),YTRIAN(I)
            endif
40        CONTINUE
          DO IF=1,IRAPS
            close(60+IF+ifoff)
...
!pb            WRITE(18+ifoff,'(A8,2I6,5X,A1)') 'TRIM3   ',ipl+1,ANZ,'3'
            ...
            DO 60 I=1,NTRII
              WRITE(18+ifoff,'(1X,A1,3I10)') '0',NECKE(1,I)+ipl*nrknot,
     .                                    NECKE(2,I)+ipl*nrknot,
     .                                    NECKE(3,I)+ipl*nrknot
60          CONTINUE
            igroups = iplane
...
!pb        WRITE(18+ifoff,'(A3,2X,A14,45X,I8)') 'PSS','1BEISPIELDATEN',ANZ+NSTAB
        WRITE(18+ifoff,'(A3,I3,A60,I6)') 'PSS',1,'BEISPIELDATEN',-3334
        WRITE(18+ifoff,'(A8,I6,I9,I6)') 'TET4    ',1,ANZ,4
        do i=1,ntet
          if (ntbar(1,i) >= 0)
...
c ist das jemals getestet worden ???
        ...
        do ipl=0,iplane-1
          if (lraps3d.and.lr3dcon) then
            if (ipl < iplane-1) then
              WRITE(18+ifoff,'(A8,I6,I9,I6)') 'QUAM4   ',IGR+ipl,NSTAB,4
              igroups = igroups+1
            endif
          else
            WRITE(18+ifoff,'(A8,I6,I9,I6)') 'FLA2    ',IGR+ipl,NSTAB,2
            igroups = igroups+1
          endif
C
          CUR => FIRST_POINT
...
C  CONTINUATION OF A LINE
     ...
     .                   CUR%NXTPNT%XPL2D*sin(ipl*rapsdel)
                elseif (lraps3d.and.nltrz) then
                  NCO=NCO+1
                  WRITE (17+ifoff,'(I6,1P,3E12.4)') NCO,CUR%XPL2D,
     .                   CUR%YPL2D,
     .                   ipl*rapsdel
                  NCO=NCO+1
                  WRITE (17+ifoff,'(I6,1P,3E12.4)') NCO,
...
c     punkte schreiben
                  if (nltra) then
                     WRITE(17+ifoff,'(I6,1P,3E12.4)') nco+ipoint,
     .                    phelp(Ipoint,1)*cos(ipl*rapsdel),
     .                    phelp(Ipoint,2),
     .                    phelp(Ipoint,1)*sin(ipl*rapsdel)
                     WRITE(19+ifoff,'(I6,1P,50E12.4)') nco+ipoint,
...
c     element schreiben
     ...
     .           '-9999','FIN','0','0','0','0'
      WRITE(19+ifoff,'(1X,A5,8X,A3,50(11X,I1))') 
     .           '-9999','FIN',(0,IF=1,IRAPS)
      deallocate(valcont)
