...
C
     ...
     .           IUNIN_SAVE, NLOGIN, NINITL_READ, NPRMUL, IFLG
...
CVK MPTS
        IF(MPTS.GT.0) THEN
         NPTS(ISTRA)=INT(NPTS(ISTRA)*MPTS)
         NMINPTS(ISTRA)=INT(NMINPTS(ISTRA)*MPTS)
        END IF
CVK END
...
!pb        IF (ILIIN(J).LE.0) TRANSP(:,1,J)=0.D0
!pb        IF (ILIIN(J).LE.0) TRANSP(:,2,J)=0.D0
