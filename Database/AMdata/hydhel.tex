...
a factor 10 too large. The fitting coefficients for  cross section and rate coefficient appear to be correct, however.
...
\begin{itemize}
\item {2.1.8  and 2.3.13 missing fits for H.2 rate coefs.}
...
Perhaps the closed form expression allow a direct (analytical) translation into polynomial fit? It seems to be the case indeed from
...
So the H.8  2.2.14 is approximately, but not exactly, the previous EIRENE default.
...
Done:  H.8 follows from H.2 directly if only b0 and b1 coefficients are non-zero, see: AMJUEL. change eirene default to that 2 parameter fit
from AMJUEL for H.8  2.2.14
\item{state notation for process 2.3.4b: final state wrong}
to be corrected
\item{process 2.2.15 is linear in log log scale. Check coefficients, make H.8, set spurious coeffs. to zero}
\end{itemize}
\newpage
...
\newpage
\subsection{
Reaction 2.3.4b $e + He(1s^12s^13S) \rightarrow e + He(1s^12s^11P)$}
...
\newpage
\subsection{
Reaction 2.3.12 $e + He(1snl^1{2S+1}L) \rightarrow e + He^+(nl)+e$}
...
\newpage
\subsection{
Reaction 2.3.13 $  e + He^+(1s) \rightarrow He(1snl^1{2S+1}L) + h\nu$}
