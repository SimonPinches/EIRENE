...
of the book are retained in the database AMJUEL, (reaction H.1 2.2.5org, H.2 2.2.5org), for reference
purposes only.
\item{Reaction 3.1.8}
...
A possible conflict: \textbf{\emph{Eth}} related to COM (reduced) mass. EMIN mostly refers to either
smallest correct value of cross-section, or to threshold energy in Lab system (neutral reactant at rest)
...
because higher non-zero coefficients
in the original book \cite{kn:Janev87} seem to be spurious for this reaction.
In this simpler (AMJUEL) form for the rate coefficient H.2 the corresponding H.8 fit
is exact: $\ln \langle\sigma v E_{elec}\rangle(T) = h_0 +h_1 \ln(T)$ with $h_0 = b_0+\ln(3/2+b_1)$ and $h_1=b_1+1$, see H.8  2.2.14 in AMJUEL.
An independently computed H.8 fit using a table obtained from the full (spurious) H.2 rate fit is still retained in the present HYDHEL file here, (was formerly at AMJUEL H.8),
...
E1min of H.3 fit for 3.2.3 is about 0.1 eV.
...
In original Janev-Langer-Post monograph only closed form analytic expressions are given for H+ and He+ rad rec. rate coefficients.
...
No: two different slopes Te**s at low Te and at high Te. Cannot be accommodated
...
from AMJUEL for H.8 rate. Done.

In Sept. 18  a fit for recombination: H.8 2.1.8 has been added, using only the first 2 coefficients b0 and b1 from
the corresponding H.2 fit. (In an attempt to mimic the closed form analytical default model in EIRENE):
However: in case of process 2.1.8 this turns out to be unsatisfactory.
Apparently the higher coefficients in H.2, although small, do matter in order to provide the
...
Check coefficients, make H.8, set spurious coeffs. to zero and proceed according to what was said above for 2.2.14
...
original fit from 1987 Springer book replaced by this one, which has better
