...
\subsubsection{\textbf{H.8: energy weighted rates vs. temperature}}\label{sec1.3.8}
...
This fit seems to be completely corrupted. Probably misprints in original FJ report.
...
\subsection{
  Reaction 2.1.8t $H^+ + e  \rightarrow H(1s) $
     ...
     do not use: only for testing  ADAS PRB rate. Same as above, JH database
...
\subsection{
  Reaction 2.1.8det  $H^+  + e \rightarrow H(1s) $, \  $<de> + 13.6 \ [eV] $
     ...
     do not use: only for testing  ADAS PRB rate. Same as above, Johnson Hinnov
     but $dln\langle sig v\rangle /dlnT$ correction removed in free bound transitions
