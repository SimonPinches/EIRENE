...
(i.e. H.8 2.1.5G out).
 ...
 \item update  23.5/00 \\
  H.12 2.2.5fl revised: labelling of n=2 triplet levels in Sawada's
  code corrected.
 \item update  06.8/00 \\
 ...
 \item update  14.01/07 H.10, 2.1.8-t: for comparison with ADAS PRB \\
  ...
  \item April 15: HYDHEL cross section for 3.1.8 (cx) added here too (same as in hydhel.tex file)\\
  ...
  Some header text changed, e.g \emph{ratio} $\rightarrow$ \emph{Ratio} in H.12 headers
  \item June 15:  state notations changed for H, H2, He, to remove further blanks from parenthesis, and
 ...
 \item July 15:  started to add H.2 rates from fits of H.3 rates for Ebeam $\rightarrow 0$
 ...
 threshold $1/T^2\times \exp(-\Delta E/T)$ ??
...
\item 2.2.14: fit made strictly linear on log-log scale, for H.1, H.2 H.8. Then use H.2 coefficients to
derive, exactly, the corresponding H.1 and H.8 coefficients. Done for 2.2.14, and H.8.  $h1=b1+1, h0=b0+\ln(3/2+b1)$
To be done for H.1 for this
process, and also for other (probably mostly recombination) processes, e.g. 2.1.8, 2.3.13,....
\end{itemize}
...
but  with all fit coefficients a2, a3,...a8 set to zero.
...
original Janev-Langer fit as in 1987 Springer book, i.e. without low energy correction.
...
same as HYDHEL cross section. Improved from original Janev-Langer fit at low energies by asymptotic fit.
...
\subsection{
Reaction 2.2.14  $  e + H_2^+(v) \rightarrow H(1s) + H(n)   (v=0\ldots 9, n \geq 2)$}
Fit as given in \cite{kn:Janev}  but with all coefficients b2,b3,...b8 set to zero,
...
to $H_2$ ground state. No population of $H_2(v)$ from electronically
excited $H_2^*$, no radiative transitions between vibrational levels.
Assume: incident $H_2$ particle with 0.1 eV (for the rate taken to be for $H_2$ at rest) and $T_i = T_e$,
...
\subsection{
Reaction 3.1.8L   $p + H \rightarrow H + p  $
}
...
\subsection{
Reaction 3.1 $W + e \rightarrow W^+ + 2e $ potential}
...
Corresponding rate coefficient is given in HYHEL with same sub-section number.
\begin{small}\begin{verbatim}
         E-Index:     0                     1                     2
  T-Index:
...
Corresponding corona rate coefficient is given in HYHEL with same sub-section number.
\begin{small}\begin{verbatim}
...
i.e. low energy electrons are preferred in this reaction, over the average electrons with $3/2~kT_e$
...
amongst all N=3 triplet states. 2E7: approx. Fulcher Aik coeff for $N=3: d \rightarrow N=2: a$ transition in triplet system.
...
upper Fulcher population coefficient. Triplet d-state, A$_{Fulch}$=2.53E7
