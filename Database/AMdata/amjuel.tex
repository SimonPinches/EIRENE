...
unfinished. Was started in late 2018  (until March 19) to sync. the eirene neutral and trace ion reaction data treatment.
...
Kept here with both names (via an alias) for old input files only.
A corresponding ``thermal" H.2 3.2.3 (without ``g") has been added in hydhel.tex, without that vibr. distribution,
i.e. just from the vibr. ground state data.
\item Jan. 22: \\
...
as well as H.10 2.6A0 and 2.3.6A0 fits are redone. Also added: H.10 2.6A0r: radiation loss only, whereas 2.6A0
...
The corresponding (H.2) Maxwellian rate coefficient is
...
(see paragraph \ref{sec1.3.3} below) collapse to the same constant.
...
may become negative (i.e., become a gain), for some values of the parameters,
...
ionization of that molecule individually, but only to the (weighted) sum of these three loss channels.
...
conditions H.10 2.1.5o the opposite is the case. (Radiation rate H.10 2.1.5r is not yet given here for this process.)
...
\subsection{
Reaction 3.2.3g   $p + H_2 \rightarrow H + H_2^+  $
}
