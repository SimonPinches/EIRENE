...
\item Oct 15:  Further H.2 rates added for heavy particle collisions. These H.2 rates should be scalable to
...
\item Oct 15:  started to add TEMIN, TEMAX, NEMIN, NEMAX parameters for extrapolation beyond fit range.
Currently all H.4 rates should have NEMIN=1.e8, and be extrapolated with this values taken also for all
...
\item Jan.16>  N2+ diss.rec added: rate: 2.7.14, linear on log-log scale, and hence corresponding H.1 and H.8 data follow analytically.
\end{itemize}
...
\item 2.2.14: H.2 fit made strictly linear on log-log scale, for H.1, H.2 H.8. Then use H.2 coefficients to
derive, exactly, the corresponding H.1 and H.8 coefficients. Done for 2.2.14, and H.8.  To be done for H.1 for this
...
\end{equation}
with ${\bf v}_0', \mu, \theta$ being the post collision test particle velocity, the reduced mass and the scattering angle, respectively.
The second equality follows from momentum conservation and then no internal energy is transferred during the collision (i.e., elastic, or resonant charge exchange).
...
\end{equation}
where $\widetilde{{\bf v}}_0$ the test particle velocity in the rest frame of the plasma and  $\widetilde{E}_0$ its the corresponding energy, Eq. (\ref{E0rest}.
...
Exception: the reduced population coefficients for hydrogenic plasmas, i.e.
2.1.5a,....2.1.5e, 2.2.5a, 2.2.5e, etc... are currently automatically read in
EIRENE post processing routines in code section ``output", routines: Ba$\_$alpha, Ba$\_$beta,...
...
\cite{kn:Petersen}
$\Delta E_{el}$ from H.8 rate, exact. KER: 1.06 - 5.824 eV at zero electron impact energy,
...
NEMIN =   1.00000D 08 1/CM3
NEMAX =   1.00000D 16 1/CM3
