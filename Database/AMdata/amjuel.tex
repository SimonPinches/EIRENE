...
%this version: latex-to-dvi --> dvi-to-pdf.
...
\hypersetup{pdftitle={AMJUEL Documentation},linkcolor=blue, citecolor=blue, urlcolor=blue, filecolor=blue}
 \usepackage{bookmark} % use only together with dvipdfmx option of hyperref package!
...
The data file AMJUEL\_DR: \\
Additional Atomic and Molecular Data for EIRENE
}
{\large \bf \center

Since 2019 this particular version, taken from the SOLPS-ITER code repository (https://www.iter.org/),
is updated and maintained by Detlev-Reiter-Consulting, D\"uren,
under various consultancy contracts with 3rd parties,
as well as under the auspices of the ITER ISFN fellowship network.
\vspace*{3truecm}

D. Reiter \\
Detlev-Reiter-Consulting \\
Lerchenweg 10\\
52353 D\"uren \\
%Post-box 1917 \\
Germany
...
Available via E-mail from detlevreiter-consulting@dn-connect.de, or as part of
EIRENE code git-repositories, if these have been updated accordingly.
}
\pagenumbering{Roman}
\newpage
...
\subsection{Record New (from March 2019):}\label{record_new}
\begin{itemize}
\item March 20 \\
Fit H.4 2.2.9g added and also Te range of fit H.4 2.2.9 extended.
2.2.9g includes Greenland's (Te dep) vibr. distribution in ground state.
2.2.9 is from H2(v=0) initial state only. The difference is too small to be relevant,
This update was just for consistency with other rates implicitly involving Greenland's vibr. distr.
such as 2.2.5g and 3.2.3g
\item Dec 20: \\
ratios H.12  2.2b,c:  continue = const above temperature P1MAX=500 eV.
The fits H.12 2.0b,c are corrupt above 1000eV,
but these ratios are about constant above a few 100 eV (for each selected density), which is also the default eirene extrapolation.
Tbd: check 2.2a.  as well as other density ratios.
\item Oct. 21: \\
H.0, H.2 Trubnikov potential parameters added, for EIRENE Trace Ion Module.
But unfinished.
Was started in late 2018  (until March 19) to sync. the eirene neutral and trace ion reaction data treatment.


H.2 3.2.3 renamed to H.2 3.2.3g, because Greenland's vibr. distribution is included.
H.3 3.2.3 renamed to H.3 3.2.3g, because Greenland's vibr. distribution is included.
Kept here with both names (via an alias) for backward compatibility with old input files only.
A corresponding ``thermal" H.2 3.2.3 (without ``g") has been added in hydhel.tex, without that vibr. distribution,
i.e., just a thermal rate coefficient from the vibr. ground state data.
\item Jan. 22: \\
fitting range limits d2min, d2max, t1max added to some H.10, H.11 reactions.
N.B.: these must be given in exponential form, e.g. 123.4E2 or 1234.5d-02, etc.
\item Oct. 22 \\
For consistency with other H.10 rates: N and O ionising component electron cooling rates added. The old rates H.10 2.7A0 and 2.8A0 have
apparently been
the ADAS PLT rates, i.e., the associated radiative cooling rates only.
These are now labeled: 2.7A0r and 2.8A0r, resp., in section H.10, while the new full electron cooling rates
are now stored under the same names 2.7A0 and 2.8A0 as previously were the old (radiation only) fits.
\item Nov. 22 \\
Reaction 3.2.3i (MAI channel of H2+ condensed CX on H2) low Te asympt. behaviour corrected.
The old fit was valid only down to 0.3 eV. The new one stays plausible down to 0.02 eV (EIRENE default Te cut off),
now the same as it was the case already for the corresponding MAR and MAD channels 3.2.3r and 3.2.3d, resp.
\item Nov. 22 \\
Replacing ADAS 93 Carbon ionisation, recombination, and radiation/electron cooling rates, with
newer ADAS data (ADAS 96, but apparently re-compiled 1999). I.e.,  H.4 2.6A0 and 2.3.6A0 fits,
as well as H.10 2.6A0 and 2.3.6A0 fits are redone.
Also added: H.10 2.6A0r: radiation loss only, whereas 2.6A0
is the (total) electron cooling rate. The older ADAS 93 previously used for the fits
had a narrower validity range in density, and
a shifted validity range in temperature.
\item Aug. 23 \\
Eth parameter is only allowed for cross sections (H.1),
not yet for rates. Check: slreac, read_poly, etc.. in eirene
\end{itemize}

\subsection{Record Old (until March 2019):}\label{record_old}
%\addcontentsline{toc}{subsection}{Record:}
\begin{itemize}
\item update  6.12/94 \\
   Boron ionization rates H.2, 2.5B0, 2.5B1 \\
                           Boron rec.   rates H.4, 2.3.5B0, 2.3.5B1 \\
 \item update  6.4/95 \\
  elast. data revised, from: elrep.dat \\
 \item update  25.4/95 \\
  H.9, 3.1.8 improved   \\
 \item update 12. 1/96 \\
   Ar ionization rates H.2, 2.18B0, 2.18B1   \\
   Ar rec.   rates H.4, 2.3.18B0, 2.3.18B1 \\
   Ar el.cool      H.8, 2.18B0, 2.18B1, H.11,2.18B0\\
...
with protons into an excited state of the $H$ atom,

or

H2(v) + p $\rightarrow$ H$_2^+$  + H , and the H$_2^+$ may provide a photon (e.g., H-alpha)
after dissociative recombination into an excited state of the $H$ atom.
...
\item
3.) proportional to H$_2$ - density PDENM(..)
\item
...
Since we may not always have computed the H$^-$ density, we can multiply instead by the H$_2$ density
...
must be retained in the line emission routines as multiplicative factor for the H$_2$ density, for consistency.
 ...
 \item update   6. 9/96 \\
   H- CX multistep recombination, low H- energy, Tp=Te H.4  7.2.3a\\
   H- CX multistep ionization, low H- energy, Tp=Te H.4  7.2.3b
 \item update   26.2/97 \\
...
(i.e. H.8 2.1.5G out).
 \item update   18.3/97 \\
  ...
  H.2 3.2.3  (only slow molecules, E0=0.37 eV as proxy for E0=0, and no plasma flow) \\
  H.2 2.2.17
 \item update   28.7/97 \\
  ...
  H.4 2.2.5g, as 2.2.5 but Greenland's vibr. distribution of H2 molecules
 \item update    8.8/97 \\
  H.2 2.10B0, 2.10B1 \\
  H.8 2.10B0, 2.10B1 \\
  H.11 2.10B0
 ...
 \item update   23.9/97 \\
  ...
  H.4 2.6A0 ADAS 93  carbon ionization    \\
  H.4 2.3.6A0 ADAS 93  carbon recombination \\
  H.10 2.6A0 ADAS 93  carbon line radiation plus 11.3 per ionization\\
                   (=electron cooling rate)\\
  H.10 2.3.6A0 ADAS 93  carbon line radiation due to recombination\\
                   (=electron cooling rate + 11.3 per ionisation)\\
  H.12 2.6A0 ADAS 93  carbon line radiation per ionization\\
  H.12 2.3.6A0 ADAS 93  carbon line radiation per recombination
 \item update   22.4/98 \\
 ...
 \item update  11.05/04 \\
  ...
  H.3  3.2.3 new (aka 3.2.3g), $E_{H2}$ consistent for all H2(v) \\
  H.2  3.2.3 (aka 3.2.3g) and 2.2.17 redone, $E_{H2}= 0.1$, rather 0.37
 ...
 \item update  14.01/07 H.10, 2.1.8-t: for comparison with ADAS PRB \\
  ...
  fit H.3: 3.2.3 (aka 3.2.3g) corrected. Was completely off.
  ...
  N.b.: H.10 rates are the rad loss rates (PLT, PRB) only, not including the ionisation energy cost,
  see: update from oct. 2022
  \item update Dec. 09 2011: \\
   N ionisation, N+ recombination: ADAS 96, H.4 and H.10 fits added 2.7A0 and 2.3.7A0, resp.
  N.b.: H.10 rates are the rad loss rates (PLT, PRB) only, not including the ionisation energy cost,
  see: update from oct. 2022
  \item update July 12:  \\
  H.2: 3.2.3old (aka 3.2.3g.old)re-introduced, for backward compatibility, i.e. $E_{H_2}=0.37$ eV, (as around year 2000)
  whereas now (since 2004) in  H.2: 3.2.3 (aka 3.2.3g) this energy is 0.1 eV.
  ...
  Reaction H.2 2.2.5 renamed to 2.2.5org (this is the old, incorrect fit from the 87 Springer book),
  ...
  \\
  checked: e+N ionisation. Cross-sections from Bell and Brooks are identical.
  ...
  related to processes 2.2.5 (DE) plus 2.2.9 (I), but not yet including dissociative excitation and ionization channels 2.2.10
  \item April 15:\\
  ...
  to be done: make H.2 rate to be used for low E0 limit extrapolation in HYDHEL. Nov. 17 done now !
  \item April 15:\\
...
\item Oct 15:\\
 started to add T1MIN, T1MAX, N2MIN, N2MAX, E2MIN, E2MAX, P2MIN, P2MAX parameters for extrapolation beyond fit range.
...
I.e., the 9 H.2 coefficients $b_i$ can directly be obtained as subset of the 81 H.4 coefficients.
\item Jan.16: \\
N2+ diss.rec added: rate: H.2 2.7.14, is linear on log-log scale, and hence corresponding H.1 and H.8 data follow analytically.
\item May 16: \\
...
\item Dec 20: \\
ratios H.12  2.2b,c:  continue = const above temperature P1MAX=500 eV. The fits H.12 2.0b,c are corrupt above 1000eV,
but these ratios are about constant above a few 100 eV (for each selected density), which is also the default eirene extrapolation.
Tbd: check 2.2a.  as well as other density ratios.
\item Oct. 21: \\
H.0, H.2 Trubnikov potential parameters added, for EIRENE Trace Ion Module.
unfinished. Was started in late 2018  (until March 19)
to sync. the eirene neutral and trace ion reaction data treatment.
H.2 3.2.3 copied to H.2 3.2.3g, because Greenland's vibr. distribution is included.
Kept here with both names for old input files only.
Tbd: make a H.2 3.2.3 in HYDHEL, without that vibr. distribution,
just from the ground state data, i.e. evaluate H.3 rate at Ebeam=0.1 eV. Done: around 2018
\end{itemize}

\subsection{to be done (Old (until March 2019):}
...
H.8 2.7.14: done
\item  Aug. 17:   3.1.8  H.2, H.3 and H.9 rate coefficients must be redone, due to Aug. 17 correction of left asymptotic part in cross-section.
Will perhaps further improve the track-length estimator energy balance.
So far we have no 3.1.8S (Schultz) rate coeff. of any kind.
\end{itemize}

\subsection{to be done (New (from March 2019):}
\begin{itemize}
\item Jan. 22: H.10 rates from ADAS tables: missing d2min,d2max
validity range. Check original tables from which these fits have been made.
\item Aug. 22: H.8 electron cooling rates in corona limit (STRAHL code): 
missing graphs: N, B, C.
poor quality pdf figure: Fe.
\end{itemize}
...
The present compilation contains data fits for atomic/molecular processes in a format which can be utilized e.g. in EIRENE code runs.
...
\paragraph*{\textbf{Special cases A: Power law, e.g. Langevin cross-section}}
...
\begin{equation}\label{sig-powerlaw}
...
The (H.2) Maxwellian rate coefficient corresponding to cross-section \ref{sig-powerlaw}, for general valves of parameter $r>-4$, is
\begin{equation}\label{rat-powerlaw}
...
\begin{equation}
\langle\sigma(v) \cdot v  \rangle (T) =  c_r  \ [cm^3/s]
\end{equation}
independent of $T$ or $T/m$.
Note that in this case also the (H.3) Beam-Maxwellian rates
(see paragraph \ref{sec1.3.3} below) collapse to the same constant $c_r$.

Cross-sections in this present database are fitted vs. ``laboratory" collision energy $E$, rather than vs.
``laboratory" collision velocity $v$, with $v=\mbox{cvela} \sqrt{E(eV)/m(amu)}$.
The reference velocity $cvela$
in the EIRENE code is $\mbox{cvela} = 1.38912 e6 \ [cm/s]$ and $m$ is the mass of the first (usually the charged) particle, assuming the second (usually the neutral) particle to be at rest.

Hence the mass $m$ must not be confused with the reduced mass $\mu$ for the center
of mass collision velocity.

A resulting fit (linear on log-log scale) for cross-section (\ref{sig-powerlaw}) is
$\ln(\sigma(E)) = a_0 + a_1 \ln(E) $ then is: $a_0 =  $ and $a_1 = $   to be written...

and for the corresponding thermal rate coefficient (\ref{rat-powerlaw})
$\ln(k(T))= b_0 + b_1 \ln(T)$ then is: $b_0 =  $ and $b_1 = $  to be written

\paragraph*{\textbf{Special cases B: Arrhenius cross-section}}
to be written
The generic cross-section form now reads: % from cpp JR silane paper, dort auch: energetics ausfuehrlich
\begin{equation}
 \sigma(E) = A \left( 1-\frac{E_{th}}{E}\right)^\alpha \frac1E \ln(e+cE)
\end{equation}
...
Maxwellian rate coefficients are taken for neutral particle energy $E_0 = 0.0$ eV and for stationary background (plasma): i.e.,  drift velocity ${\bf V}_p ={\bf 0}$
...
Beam-Maxwellian rate coefficients. These coefficients are generalisations of the H.2 (isotropic) stationary Maxwellian rate coefficients and account for either
a finite velocity (energy $E_0$) of the  test particle (or beam) or for a fluid drift ${\bf V_p}$ in the Maxwellian background, or both.
...
to $\ln(\tilde n) = \ln(10^8) = 8 \ln(10) \approx 18.421$  (at $n=10^{16} \, cm^{-3}$).

Further rescaling the fit expression to a density parameter
$\ln(\hat n) = 1/b \ln(n/10^8)$ with $b = 8 \ln(10)$ brings the validity range $[10^8 ,10^{16}]$ of the density parameter $n$ $\, [cm^{-3}]$  to the
interval $[0, 1]$ for the scaled density parameter $\ln(\hat n)$.
At these boundaries for the scaled density parameter the collisional radiative rate coefficients
...
then becomes
$$\ln \langle\sigma v\rangle = \sum_{n=0}^N\sum_{m=0}^M \hat\alpha_{n,m}(\ln \hat{n})^m(\ln T)^n $$

with $ \hat\alpha_{n,m} = b^m \alpha_{n,m}$.
At the validity boundaries $\ln(\hat n) =0$  and $\ln(\hat n)  =1$
...
$[10^8 ,10^{16}]$,  is representative for most relevant applications of EIRENE, but strictly is also temperature dependent.
...
As discussed already for general Beam-Maxwellian (= "displaced Maxwellians" rate coefficients,
paragraph H.3, in a first step we transform to the rest frame of the plasma:
\begin{equation}
...
Momentum transfer from an individual test particle to a drifting Maxwellian plasma background is then in the direction $\widetilde{{\bf e}}_0=\widetilde{{\bf v}}_0/\widetilde{v_0}$
...
$$
Where\\
${\bf V}^L_t$ and ${\bf V}_{dr}$ is the test particle velocity in laboratory frame and the plasma drift velocity, respectively;\\
...
If $\sigma=\sigma(V_r)$ then $I^{(l,n)}=F\left(\frac{E}{m_t},\frac{T}{m_p}\right)$ and
...
The rate 0.3 (elastic $p+H_2$ collisions) from this set was successfully compared with the same sources, calculated by collisional estimator. The same test for
the resonant CX rates 3.1.8 \textcolor[rgb]{0.98,0.00,0.00}{ {\em was not yet successful}}
...
Under label H.8 energy-weighted rate coefficients are stored, vs. temperature $T$ (eV) of the
...
These are typically corona limit (density independent) rates, i.e. the intermediate excited states (if any)
are assumed to decay radiatively.


For reactions with no electron (or incident heavy particle) in the exit channel,
such as for recombination processes, the net electron (or net incident heavy particle) energy loss
rate coincides with the incident energy weighted rate coefficient, for which a simple analytic expression
then can be used:

The general relation between energy-weighted rate coefficients,
when weighted by the incident energy of particles with temperature $T$
H.8 and the ``ordinary" Maxwellian rate coefficients $\langle H.2 \rangle$
is (integration by parts):
\begin{equation}
H.8 [eV cm^3/s]= T \times H.2 \times \left[3/2+\frac{d \ln(H.2)}{d \ln T}\right]
\end{equation}
with rate coefficient H.2 in $cm^3/s$ and H.8 in $eV cm^3/s$ and temperature $T$ in $eV$.
...
\begin{equation}
\ln(H.8) = \ln(T) +  \ln(H.2) + \ln(3/2 + b_1 + 2b_2 \ln(T) + 3b_3 \ln^2(T) + ...)
\end{equation}
...
An explicit numerical example is detailed under H.8 2.2.14 (dissociative recombination of $H_2^+(v)$ molecular ions)
or H.8 2.7.14 (dissociative recombination of $N_2^+$ molecular ions).
...
collision event, in $eV$ units. These rates, therefore, if multiplied by the
...
to be in a certain (collision radiative) equilibrium, then these
...
may become negative (i.e., may become a gain), for some values of the parameters,
...
By default we have chosen $\Delta E_{subtr.} = 0.0$ in this expression
for all processes in which the potential energy (reaction enthalpy) is
...
One can show with some boring algebra on the matrices which arise in collision radiative
models that
with this particular choice of the subtracted energy loss rate for collision radiative
...
plus the effective potential energy loss rate, with the latter rate being simply
given as

$\langle\sigma \cdot v \cdot \Delta E_{pot} \rangle_{effective} = \Delta E_{pot}
\cdot \langle\sigma \cdot v \rangle_{effective}$

or, in case of multiple loss channels, $i, i=1,il$ with $\Delta E_{pot,i}$ being the associated potential energy
difference:

$\langle\sigma \cdot v \cdot \Delta E_{pot} \rangle_{effective} = \sum_i^{il} \Delta E_{pot,i}
\cdot \langle\sigma \cdot v \rangle_{effective,i}$
...
direct ionization of that molecule individually, but only to the (weighted) sum of these three loss channels.

Similarly, electron cooling rates associated with a Helium atom, if meta-stable He atoms are retained explicitly
...
\\
\textbf{Note:}
In some instances both radiation rates and electron cooling rates are retained.
The former are then labeled H.10 xxx\_r, while the latter are just H.10 xxx.

In downward processes, when only radiation rates are given, the old notation H.10 xxx is retained.

Also:  radiation rates typically decrease with increasing density, while the potential energy loss rate
typically increase with density. In total electron cooling rates both effects often nearly cancel,
although we find usually a weak increase with electron density.

See e.g. total electron cooling rate H.10 2.1.5, increasing with density, but also: for Lyman opaque
conditions H.10 2.1.5o the opposite is the case.
(Radiation rate H.10 2.1.5r is not yet given here for this process.)
...
Morse potential, see \cite{kn:Bachmann2}
...
\begin{small}\begin{verbatim}
   fit-flag 03
   p0  0.100000000000D+00  p1  0.000000000000D+00  p2  0.000000000000D+00
   p3  0.000000000000D+00  p4  0.000000000000D+00  p5  0.000000000000D+00
   p6  0.000000000000D+00  p7  0.000000000000D+00  p8  0.000000000000D+00
\end{verbatim}\end{small}

\subsection{
Reaction 0.101e  $ i + e_b \rightarrow i + e_b$ Trubnikov potential}
bulk-electrons + test-ions, Fokker-Planck elastic \\
Data for Trubnikov Potentials for Fokker-Planck collision
operator, bulk-electrons on test-ions. effective diffusion coefficient
...
\begin{small}\begin{verbatim}
   fit-flag 03
   p0  0.010000000000D+00  p1  1.000000000000D+00  p2  0.000000000000D+00
   p3  0.000000000000D+00  p4  0.000000000000D+00  p5  0.000000000000D+00
   p6  0.000000000000D+00  p7  0.000000000000D+00  p8  0.000000000000D+00
\end{verbatim}\end{small}
...
\subsection{
Reaction 0.103e $ i + i_b \rightarrow i + i_b$ Trubnikov potential}
bulk-ions + test-ions, Fokker-Planck elastic \\
Data for Trubnikov Potentials for Fokker-Planck collision
operator, bulk-ions on test-ions.
...
\subsection{
Reaction 0.104 $ i + i_b \rightarrow i + i_b$ Trubnikov potential}
dummy reaction used to invoke FPKCOL
...
EIRENE uses in its minimal (default) database the corresponding fit as given in the unpublished preprint for
\cite{kn:Janev}.
...
are labeled ``FJ", and are kept for comparison with old cases.

Note: FJ Maxwellian rate coefficients are taken at neutral particle energy = 0.0 eV
(i.e.: neutral atom at rest) vs. temperature (electron or ion temp., resp.) of the Maxwellian $f_{maxw}$.
% Oct. 21: tested and verified with hydkin integrator
I.e. :

$\langle\sigma v \rangle = \int d^3v_p \ \sigma(v_{p}) \cdot v_{p} \cdot
f_{maxw}(v_{p})$

The FJ ion impact rates can be scaled to different isotopes and to finite
...
$M=1$ (AMU) for hydrogen ion targets.
$M_1$ and $M_2$ are the masses (AMU) of the two isotopes in the particular collision
...
added by DR for convenience: single parameter Maxwellian rate coeff., vs. $T_p$, for neutral target at rest,
obtained from corresponding HYDHEL fit for Beam-Maxwellian rate coeff. evaluated at $E_b =0.1$ eV
and then verified by independent integration of cross-section with proper low energy asymptotic.
...
\end{verbatim}\end{small}
\begin{figure} \label{2.2.5.org}
\epsfxsize=16truecm
\epsffile{Amjuel_PS/fig2.2.5_old-vs-new.ps}
\end{figure}
...
\subsection{
Reaction 2.2.17  $e + H_2(v) \rightarrow H_2^- \rightarrow H + H^-$
}
...
\subsection{
Reaction 2.2.17s  $e + H_2(v) (+e) \rightarrow H_2^- (+e)\rightarrow H + H +2e $  (Diss via $H^-$, cold $H_2$)
}
Effective (intermediate $H^-$ condensed) dissociation rate coefficient,
via $H_2^- --> H + H^-$ channel and $ e + H^- --> H + 2e$.
...
Competing processes: $H^-$ condensed MAR,  $H^-$ condensed MAD, below.
...
\subsection{
Reaction 3.2.3g   $p + H_2(v) \rightarrow H + H_2^+  $
}
%  The next two lines allow EIRENE to still find
%  this reaction under its old name. Not sure about HYDKIN ?

%  alias  (for old eirene input files):
%  Reaction 3.2.3  $p + H_2 \rightarrow H + H_2^+  $

  3.2.3 is renamed now to 3.2.3g, because it
  implicitly contains Greenland's vibr. distribution (and hence: $T_e=T_p$).
...
\langle\sigma v \rangle_{H_2(v)} \cdot pH_2(v) $
  ...
  Same ``Greenland" vibrational distribution  $pH_2(v;T_e)$ (as function of $T_e$) as for other rates with
  implicit ``Greenland" vibrational distribution.
  Therefore: single parameter fit vs. $T_e$,
  since vibrational distribution does not depend upon density, $E_0$ is fixed
  (at 0.1 eV) and $T_p = T_e = T$.
...
\begin{figure} \label{3.2.3g}
\epsfxsize=16truecm
\epsffile{Amjuel_PS/ionconv2.ps}
\end{figure}

\newpage

\subsection{
Reaction 3.2.3g.old   $p + H_2 \rightarrow H + H_2^+  $}
...
<\sigma v>_{H_2(v)} \cdot pH_2(v) $
  ...
  Same vibrational distribution $pH_2(v;T_e)$ (as function of $T_e$) as above.
  Therefore:
...
\end{verbatim}
\end{small}
In principle the same as 3-2-3g, above, previous reaction,
but here evaluated with old default $H_2$ energy: E = 0.37 eV, rather than
the current choice of E =0.1 eV (as proxy for E = 0.0 eV).
Old data are kept here only for backward compatibility.
(The old rate coefficient was mostly used in ITER applications
and in SOLPS4.x in general).
...
\begin{figure} \label{3.2.3g.old}
\epsfxsize=16truecm
\epsffile{Amjuel_PS/ionconv2_old.eps}
\end{figure}
\newpage
...
Ionization Rate for neutral Helium Atoms, STRAHL code.
\begin{small}\begin{verbatim}
...
Ionization Rate for single charged Helium Ions, STRAHL code
...
Reaction 2.6B0   $e + C \rightarrow e + C^+ + e  $
  ...
  Ionization rate for neutral Carbon Atoms, STRAHL code
...
Reaction 2.6B1   $e + C^+ \rightarrow e + C^{++} + e  $
  ...
  Ionization rate for singly charged Carbon Ions (STRAHL code)
...
\subsection{
Reaction 2.7Bl   $e + N \rightarrow e + N^+ + e  $
}

  Ionization rate coefficient for neutral Nitrogen Atoms, STRAHL code
...
\subsection{
Reaction 2.7Br   $e + N \rightarrow e + N^+ + e  $
}
cross-section data from Brook, \cite{kn:Brook}   for $ e + N -> N^+ + 2e $,
same cross-section data source was used for the Bell rate coefficient in the STRAHL code data.
...
\begin{small}\begin{verbatim}
   fit-flag 03
   b0 -1.000000000000D+30  b1  0.000000000000D+00  b2  0.000000000000D+00
   b3  0.000000000000D+00  b4  0.000000000000D+00  b5  0.000000000000D+00
   b6  0.000000000000D+00  b7  0.000000000000D+00  b8  0.000000000000D+00
\end{verbatim}\end{small}
...
%  Reaction 3.2.3  $p + H_2 \rightarrow H + H_2^+  $
  ...
  3.2.3 is renamed now to 3.2.3g, because it
  implicitly contains Greenland's vibr. distribution.
  Effective ion conversion (Charge exchange on $H_2$)
  as function of $T_p$ (from Janev,\cite{kn:Janev}).

  Greenland's vibrational distribution $pH_2(v;T_e)$ is density independent,
  assume: $T_e = T_p = T$,
...
Reaction 2.1.5JH   $H + e \rightarrow H^+ + 2e $}
   ...
   Report: Mahn, Welge, IPP, \cite{kn:Mahn}
...
effective CR ionization rate, only from $v=0$, vibrational ground state,
Sawada-Fujimoto CR model, 2022: new fit, with wider Te range.


\begin{small}\begin{verbatim}
        E-Index:     0                     1                     2
  T-Index:
        0   -3.592236857853D+01    7.588483385936D-02    2.535474843325D-02
        1    1.626726383397D+01   -8.404492944487D-02   -1.421174642841D-01
        2   -6.362920375170D+00   -1.315258055169D-01    1.878685202713D-01
        3    1.479278171454D+00    2.130215176332D-01   -1.491006868222D-01
        4   -2.086974490197D-01   -9.320485842872D-02    5.282873945879D-02
        5    1.589853265399D-02    1.715621508423D-02   -7.806212788386D-03
        6   -3.763682603705D-04   -1.297026248461D-03    2.620089287801D-04
        7   -2.407346768556D-05    1.523419297360D-05    3.813770677095D-05
        8    1.261638023541D-06    1.646876007347D-06   -2.614554813347D-06

        E-Index:     3                     4                     5
  T-Index:
        0   -7.879944316854D-03    1.926947743183D-03   -1.182535914681D-04
        1    6.541352964596D-02   -1.523394642974D-02    1.813412339869D-03
        2   -5.439034353614D-02    7.467862942483D-03   -5.776243113392D-04
        3    3.477587449149D-02   -2.605792741376D-03   -4.149275624611D-05
        4   -1.231513472407D-02    8.630660535337D-04    2.661391169210D-05
        5    1.910636707371D-03   -1.409241505974D-04   -2.310134017102D-06
        6   -7.468644482138D-05    3.613580195483D-06    2.835872227933D-07
        7   -8.616410645273D-06    1.129943262132D-06   -5.833639190048D-08
        8    6.351942122828D-07   -7.512685568360D-08    3.501169476579D-09

        E-Index:     6                     7                     8
  T-Index:
        0   -4.401918055202D-06    5.920205216987D-07   -1.367370686834D-08
        1   -1.132611574429D-04    3.564818647332D-06   -4.482392846850D-08
        2    2.575990024621D-05   -6.109678848651D-07    5.798738622401D-09
        3    1.500117690451D-05   -7.188689775602D-07    1.126791065389D-08
        4   -5.958230464514D-06    2.598672639011D-07   -3.742067299155D-09
        5    6.477466850430D-07   -2.324675686606D-08    2.266162425808D-10
        6   -6.038534834392D-09   -1.496412976670D-09    5.203406469838D-11
        7   -2.476499598978D-09    3.302539513585D-10   -8.012922613581D-12
        8    9.089258219104D-11   -1.338587612270D-11    3.155160367434D-13
T1MIN =   0.10000D 00 EV
T1MAX =   1.00000D 04 EV
N2MIN =   1.00000D 08 1/CM3
N2MAX =   1.00000D 16 1/CM3

  Max. rel. Error:   0.510E+01 %
  Mean rel. Error:   0.140E+01 %
...
\epsfxsize=16truecm
\epsffile{Amjuel_PS/hydh2in.eps}
\end{figure}
\newpage

\subsection{
Reaction 2.2.9g  $  e + H_2(v)       \rightarrow 2e + H_2^+  $
}

effective H2 ionization rate, with Greenland's vibrational distribution
in ground state, and Janev/Reiter scaling \cite{kn:JnRei2003} of ionisation cross sections from vibrational states.

\begin{small}\begin{verbatim}
        E-Index:     0                     1                     2
  T-Index:
        0   -3.482395131930D+01   -9.081453174790D-02    1.123386725743D-01
        1    1.615241430950D+01   -3.478039974697D-02   -9.090359886209D-02
        2   -6.990857037353D+00    3.113610733366D-01   -7.345126427513D-02
        3    1.902593074900D+00   -2.953107005555D-01    7.690046745718D-02
        4   -3.406696127590D-01    1.310126135142D-01   -2.982850102237D-02
        5    3.947146726166D-02   -3.149544561351D-02    6.242479664869D-03
        6   -2.851707592241D-03    4.169603905670D-03   -7.148191827662D-04
        7    1.179246477119D-04   -2.846857336975D-04    4.068968306051D-05
        8   -2.166270504626D-06    7.814780479722D-06   -8.606819638940D-07

        E-Index:     3                     4                     5
  T-Index:
        0   -4.001034213136D-02    7.284506504256D-03   -5.946088693634D-04
        1    5.188751204891D-02   -1.354848343652D-02    1.650718967014D-03
        2    2.588410789884D-03    2.938995819769D-03   -6.078722815238D-04
        3   -9.177751798153D-03    4.058265985369D-04    7.424555321833D-05
        4    2.425686291567D-03   -2.057287369018D-04    9.490287400767D-06
        5   -1.488381460029D-04    5.981072606708D-06   -3.551768998710D-06
        6   -3.742531313780D-05    7.657247101042D-06    6.290116557953D-08
        7    6.869294984747D-06   -1.285621323676D-06    5.549155691196D-08
        8   -3.260679014349D-07    6.171939737475D-08   -3.961768240505D-09

        E-Index:     6                     7                     8
  T-Index:
        0    1.940783625677D-05   -4.236706611614D-08   -6.651791542302D-09
        1   -1.014229889636D-04    3.073163339207D-06   -3.661761766017D-08
        2    5.057452455898D-05   -1.950726249785D-06    2.883329278528D-08
        3   -1.258128746046D-05    6.852294183262D-07   -1.273678573730D-08
        4    1.154972739661D-06   -1.235090485408D-07    2.997083874888D-09
        5    1.358046278978D-07    7.646079458368D-09   -3.425560628958D-10
        6   -2.556355171781D-08    1.731973672568D-10    1.924792718762D-11
        7   -2.766754063511D-10    2.654501140771D-12   -9.096387425722D-13
        8    1.227483507349D-10   -2.676524607095D-12    4.586864171705D-14
N2MIN =   1.00000D 08 1/CM3
N2MAX =   1.00000D 16 1/CM3
T1MIN =   0.10000D 00 EV
T1MAX =   1.00000D 04 EV

  Max. rel. Error:   0.543E+01 %
  Mean rel. Error:   0.132E+01 %

\end{verbatim}\end{small}
\begin{figure} \label{2.2.9g}
\epsfxsize=16truecm
\epsffile{Amjuel_PS/hydh2iig.eps}
\end{figure}
\newpage
...
Exothermic by -20.614 + 19.818 eV = -0.796 eV
...
\subsection{
Reaction 2.6A0old $e + C \rightarrow C^+  + 2e $
}
  Ionization Rates for neutral carbon atoms,
  ADAS 93
...
Reaction 2.6A0 $e + C \rightarrow C^+  + 2e $
  ...
  Ionization Rates for neutral carbon atoms,
  ADAS 96 (SCD file created 1999), wider ne range, valid down to Te

$  \langle sigma*vrel \rangle(Te,ne) $(cm**3/s)
        7    6.550309402473D-04   -4.393065970093D-05   -1.036405910354D-05
        8   -1.847457580217D-05    1.312856950673D-06    3.322982165259D-07

        E-Index:     3                     4                     5
  T-Index:
        0   -3.235896717762D-02    1.058873776305D-02   -1.710302443470D-03
        1   -1.432167451365D-02    3.991884915376D-03   -5.752159798480D-04
        2    6.579990576789D-03   -1.711173254046D-03    2.471461141394D-04
        3   -1.076244103261D-03    2.333787946767D-04   -2.765733583071D-05
        4    7.009089449357D-05   -4.787665210412D-05    4.816602111031D-06
        5    4.864774756898D-05    8.008233456428D-06   -1.456885739549D-06
        6   -1.813541577007D-05    5.543949043609D-07    9.444280069593D-08
        7    2.196906769877D-06   -2.169818778260D-07    1.039102846927D-08
        8   -8.856499450725D-08    1.195743943528D-08   -9.279005105188D-10

        E-Index:     6                     7                     8
  T-Index:
        0    1.452483800866D-04   -6.183333912008D-06    1.041103899070D-07
        1    4.642474333442D-05   -2.012796753335D-06    3.611687769688D-08
        2   -1.883650960415D-05    7.208627253974D-07   -1.098881493964D-08
        3    1.432158848272D-06   -1.356564602721D-08   -6.484565786223D-10
        4   -1.188047699234D-07   -8.137941011234D-09    3.668739870069D-10
        5    1.043503386039D-07   -3.533424648528D-09    4.435771058377D-11
        6   -1.483929180968D-08    8.541721518049D-10   -1.763336737449D-11
        7    2.924832871584D-10   -5.157701888881D-11    1.429627587009D-12
        8    2.710612984187D-11    5.535730249483D-13   -3.333237604857D-14
T1MIN =   0.20000D 00 1/CM3
T1MAX =   1.50000D 04 1/CM3
N2MIN =   5.00000D 07 1/CM3
N2MAX =   2.00000D 15 1/CM3
  Max. rel. Error:   0.195E+02 %
  Mean rel. Error:   0.429E+01 %
\end{verbatim}\end{small}


\begin{figure} \label{2.3.6A0}
\epsfxsize=16truecm
\epsffile{Amjuel_PS/carbr-rate96.eps}
\end{figure}
\newpage


\subsection{
Reaction 2.3.6A0old  $e + C^{+} \rightarrow C + hv$
}
 Recombination Rates for single
 charged Carbon Ions, ADAS 93,
 smaller valid density range than in Behringer (STRAHL) rate, 1e9 --4e15,
...
Reaction 2.3.7A0  $e + N^{+} \rightarrow N + hv$ }
 ...
 smaller valid density range than in Behringer (STRAHL) rate: 1e8 --4e15
...
Reaction 2.3.8A0  $e + O^{+} \rightarrow O + hv$ }
 ...
 smaller valid density range than in Behringer (STRAHL) rate
...
$H_2$ multi-step model, ``intermediate H$_2^+$ condensed" MAR rate coefficient $cm^3/s$.
Data: Sawada/Fujimoto \cite{kn:Sawada}.
$H_2(v=0)$ transported, $H_2^+$ in QSS with $H_2$, $E_{H_2}=0.1$ eV

$H_2(v\geq 1)$ is also in QSS with $H_2(v=0)$. Vibrational distribution $pH_2(v;T_e)$ as fct. of $T_e~(=T_p)$ only (assuming $n_e=n_p$, so density cancels here).
...
and then with $n_{H_2(v=0)}$ to turn it into a volumetric reaction rate ($cm^{-3}s^{-1}$). This is consistent with underlying $pH_2(v;T_e)$ only
for $n_e=n_p$.
...
$H_2$ multi-step model, ``intermediate H$_2^+$ condensed" MAD rate coefficient $cm^3/s$,

same conditions as for effective MAR rate coefficient: $n_e=n_p$ to remove $n_e$ and $n_p$ dependence in $pH_2(v;T_e)$,
$T_e=T_p$ to remove $T_p$ dependence in $pH_2(v;T_e)$,  $E_{H_2}=0.1$ eV.
...
\end{verbatim}\end{small}
\begin{figure} \label{3.2.3mad}
\epsfxsize=16truecm
\epsffile{Amjuel_PS

\epsfxsize=16truecm
\epsffile{Amjuel_PS/mar-mad-mai-H2+.eps}
\end{figure}
\newpage
...
Assume low energy of test particle projectile, $E(H^-)=0.1$ eV, $T_e = T_p$.
Intermediate $H^*$ production based on HYDHEL 7.2.2, 7.2.3.
...
\subsection{
Reaction 7.2.3b   $ p + H^{-} \rightarrow  H + H^+ + e$ (for cold $H^-$)
}

  CX (MN) multistep ionization rate for $H^-$ ions,
 \cite{kn:Sawada}
...
Intermediate $H^*$ production based on HYHDEL 7.2.2, 7.2.3
(not included: process 7.2.1 of proton impact electron detachment).
...
\subsection{
Reaction 2.2.17r   $e + H_2(v) (+p)   \rightarrow H + H + H  $ (MAR via $H^-$, cold $H_2$)
}
...
$H_2(v\geq 1)$ is also in QSS with $H_2(v=0)$. Vibrational distribution $pH_2(v;T_e)$ as fct. of $T_e$ only (assuming $n_e=n_p$, so density
dependence cancels in $pH_2(v;T_e)$).
...
\newpage
\subsection{
Reaction 2.2.17d   $e + H_2(v) (+p)   \rightarrow p + H + H + e  $ (MAD via $H^-$, cold $H_2$)
}

$H_2$ multi-step model, ``intermediate H$^-$ condensed" MAD rate coefficient $cm^3/s$,
...
Reaction 2.6B0 $e + C \rightarrow e + C^+ + e  $ 1/98 update
 ...
 constant multiplier to corresponding ionisation rate fit.
...
\subsection{
  Reaction 2.1.8  $H^+ + e  \rightarrow H(1s) $
   ...
   potential energy term 13.6*(eff-rec.rate) still needs to be
...
Helium multi-step model, here recombination:  radiative + three-body + dielectronic
...
\subsection{
  Reaction 2.6A0r  $e + C  \rightarrow C^+   + 2e $
}
radiation rate due to ionization of C atoms [eV * cm**3/s]
ADAS 96, i.e., PLT rate only

\begin{small}\begin{verbatim}
       E-Index:     0                     1                     2
  T-Index:
        0   -2.134320594564D+01   -5.946603516310D-02    8.447557526366D-02
        1    5.747871027304D+00    1.742910759701D-01    6.358855662184D-02
        2   -1.446292343924D+00   -1.934880440813D-01   -6.137783135780D-02
        3   -2.975197342585D-02    1.012407305789D-01   -9.718853445116D-03
        4    1.064347292200D-01   -2.743226249142D-02    9.525798537604D-03
        5   -2.837533619480D-02    3.973936070899D-03   -1.153950153613D-03
        6    3.653851093532D-03   -2.952184312930D-04   -8.475825853417D-05
        7   -2.385788202088D-04    9.230939672709D-06    2.196401178503D-05
        8    6.295597901357D-06   -3.739019584385D-08   -9.970598553763D-07

        E-Index:     3                     4                     5
  T-Index:
        0   -5.406978466857D-02    1.281083229789D-02   -1.524632271001D-03
        1   -6.767588642303D-02    2.421647016699D-02   -3.892494482542D-03
        2    7.147500084854D-02   -2.467911698429D-02    3.787372907637D-03
        3   -8.551103015011D-03    4.588623493575D-03   -7.759605546668D-04
        4   -3.105314942145D-03    3.910363804964D-04   -2.637644628597D-05
        5    5.644866007299D-04   -1.302260794666D-04    1.650834989207D-05
        6    2.723119788519D-05   -2.201121528129D-06   -2.002446340934D-08
        7   -9.668998805059D-06    1.832226214738D-06   -2.011877719908D-07
        8    4.625120996597D-07   -9.396591971771D-08    1.083943504650D-08

        E-Index:     6                     7                     8
  T-Index:
        0    9.919783469232D-05   -3.390120579583D-06    4.764279569959D-08
  Reaction 2.7A0r  $e + N  \rightarrow N^+   + 2e $
}
electron cooling rate due to ionization of N atoms [eV * cm**3/s]
ADAS 96, here: only radiation loss, ADAS ``PLT" file.
One must add: 14.53 eV per ionisation to turn ADAS radiation
rate into electron cooling rate.
...
\begin{figure} \label{2.7ilr}
\epsfxsize=16truecm \epsffile{Amjuel_PS/niilr-erate.eps}
\end{figure}
\newpage
...
ADAS 96, PLT + 13.62 SCD


\begin{small}\begin{verbatim}
        E-Index:     0                     1                     2
  T-Index:
        0   -2.466138341969D+01   -8.116786196024D-01    2.156098826096D-01
        1    6.294065613373D+00    1.917653675870D+00   -5.126040455489D-01
        2    4.169947620973D-01   -1.762272010936D+00    4.946754888194D-01
        3   -1.440821095346D+00    8.348952646637D-01   -2.315185825999D-01
        4    5.793856506637D-01   -2.254745053995D-01    5.770621380639D-02
        5   -1.152585175726D-01    3.591634468522D-02   -7.906197030141D-03
        6    1.270352271875D-02   -3.327034036230D-03    5.749935870122D-04
        7   -7.402495436662D-04    1.651879651465D-04   -1.907021606554D-05
        8    1.780053898010D-05   -3.387119327934D-06    1.618345525908D-07

        E-Index:     3                     4                     5
  T-Index:
        0   -1.551623604131D-02   -1.035328327703D-02    2.942777879853D-03
        1    5.415595131601D-02    1.226795295130D-02   -3.849935243694D-03
        2   -7.985211650371D-02    5.748074680812D-04    1.272681449806D-03
        3    4.286952781626D-02   -2.665900453388D-03   -1.214864213289D-04
        4   -1.070010255570D-02    7.198609421959D-04    6.516970568169D-06
        5    1.352605021497D-03   -6.620192814455D-05   -3.657688381337D-06
        6   -8.412494926860D-05    1.061990976552D-06    5.570636828925D-07
        7    2.093122097794D-06    8.762294262323D-08   -1.399830328643D-08
        8   -4.854934999850D-09   -1.329206593116D-10   -8.809765813180D-10

        E-Index:     6                     7                     8
  T-Index:
        0   -3.080298061324D-04    1.437189742453D-05   -2.507423177221D-07
        1    3.843177456353D-04   -1.678461366568D-05    2.731173341260D-07
        2   -1.356433138071D-04    5.357153507040D-06   -7.215666632437D-08
        3    1.670075321919D-05   -2.917167266543D-07   -6.340520240578D-09
        4   -5.920359721220D-07   -1.442194804790D-07    6.194752166291D-09
        5    1.505297788299D-08    3.612860525491D-08   -1.336874885628D-09
        6    1.202458568069D-08   -4.914839753825D-09    1.632417540012D-10
        7   -3.549414908473D-09    4.233445855041D-10   -1.181080686334D-11
        8    2.133708107663D-10   -1.585146599884D-11    3.777218666320D-13
T1MIN =   0.20000D 00 EV
T1MAX =   1.50000D 04 EV
N2MIN =   5.00000D 07 1/CM3
N2MAX =   2.00000D 15 1/CM3
  Max. rel. Error:   0.780E+01 %
  Mean rel. Error:   0.108E+01 %
\end{verbatim}\end{small}

\begin{figure} \label{2.8il}
\epsfxsize=16truecm \epsffile{Amjuel_PS/oxyil-erate.ps}
\end{figure}
\newpage

\subsection{
  Reaction 2.8A0r  $e + O  \rightarrow O^+   + 2e $
}
electron cooling rate due to ionization of O atoms [eV * cm**3/s]

\epsfxsize=16truecm
\epsffile{Amjuel_PS/carbrl-erate96.eps}
\end{figure}
...
Reaction 2.2B0   $ He + e  \rightarrow He^*,    \ <de>(T_e) \ [eV], $
  ...
  charged Helium Particles, divided by corresponding collision rates,
...
The density dependence on vibrational distribution $pH_2(v;T_e)$ of $H_2(v)$ vanishes for $n_e=n_p$.
...
$E_{H2} = 0.1$ eV, $T_p = T_e$, vibrational distribution $pH_2(v;T_e)$ in electr. ground state of $H_2$ with
...
Reaction 7.0a    $ e + H_2(v) \rightarrow H^- + H$, Ratio \ $H^-/H_2$ from DA
 ...
 Ratio of densities: $n_{H^-}/n_{H_2}$. \\
  vibr. distribution $pH_2(v;T_e)$ of $H_2(v)$: coll. model for vibr. excitation, Greenland, Reiter, \cite{kn:Green}\\
...
\subsection{
Reaction 7.0b      $ e + H_2(v=0) \rightarrow H^- + H$, Ratio \ $H^-/H_2$ from DA
}
...
P2MIN =   1.00000D 08 1/CM3
P2MAX =   1.00000D 16 1/CM3
  Max. rel. Error:    .9684 %
  Mean rel. Error:    .4161 %
...
Reaction 2.2.15a $ H_3^+ + e \rightarrow ...+ H(3) $, \   Ratio $H(3)/H_3^+  $
 ...
 then CR redistribution in H leading to $n=3$ population
...
\subsection{
Reaction 2.0a  $e + H_2(v=0) \rightarrow H_2^+ + ... $ , Ratio \ $H_2^+/H_2$
}

 Multi-step hydrogenic density ratios
 Data: K. Sawada, T.Fujimoto, \cite{kn:Sawada} and Janev et al, 1987, and P.T. Greenland

alias: 2.0ei  (still missing: 2.0eig: with Greenland's vibr. distr.)
  CR equilibrium ratio H2+ to H2 density
  (coll.rad model, Sawada/Fujimoto/Greenland)
 here:  H2+ gain from electron impact on $H_2(v=0)$ only
...
alias 2.0tot,  only v=0 state
 ...
 Data: K. Sawada, T.Fujimoto, \cite{kn:Sawada} and R. Janev et al, 1987, and P.T. Greenland
  ...
  contribution from both: EI on $H_2$ and $H_2^*$  plus from CX on cold $H_2(v=0)$,
($E_{H_2}(v=0)=0.1$ eV) and assuming $n_e=n_p, T_e=T_p$

Should always be larger than the corresponding H11, 2.0cx pure p process, ratio for contrib. from CX alone.
Slightly violated due to fitting inaccuracies near 1-2 eV
\begin{small}\begin{verbatim}
        E-Index:     0                     1                     2
  T-Index:
...
alias 2.0totg
 ...
 Data: K. Sawada, T.Fujimoto, \cite{kn:Sawada} and R. Janev et al, 1987, and P.T. Greenland
  ...
  CR equilibrium ratio H2+ to H2 density
  (coll.rad model, Sawada/Fujimoto/Greenland) for vibrational population $pH_2(v;T_e)$
  contribution from CX on cold H2(v) ($E_{H_2}(v)=0.1$ eV) and EI on H2 and H2*


Should always be larger than the corresponding H11, 2.0cxg pure p process, ratio for contrib. from CX alone.
Slightly violated due to fitting inaccuracies near 1-2 eV
...
\bibitem{kn:Bachmann}  Bachmann, P., Reiter, D., Contrib. Plas. Phys.
                       35, (1995), 1, pp45
\bibitem{kn:Bachmann2} P. Bachmann, H.J. Belitz, Database for Elastic Collisions of Protons
with Noble Gas Atoms, Max-Planck-Institut fuer Plasmaphysik Report IPP 8/5, June 1995
...
\bibitem{kn:JnRei2003} Janev, R., Reiter, D., Samm, U.,
                   ``Collision Processes in Low-Temperature
Hydrogen Plasmas", Report, JUEL-4105 (1996), FZ-J\"ulich, Dezember 2003
ISSN 0944-2952
