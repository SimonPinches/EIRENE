...
(i.e. H.8 2.1.5G out).
 ...
 \item update  14.01/07 H.10, 2.1.8-t: for comparison with ADAS PRB \\
  ...
  \item Jan 15:  electron energy loss rates for H$_2$ added: H.10  2.2.H2c (total electron cooling)
  ...
  \item April 15: original Janev cross section for 3.1.8 (cx) added here too (same as in hydhel.tex file)\\
  This can be used for high E0 limit extrapolation.
  to be done: make H.2 rate to be used for low E0 limit extrapolation.
  Some header text changed, e.g \it{ratio} --> Ratio in H.12 headers
...
\newpage
\section*{Numerical Fits to $\sigma$ and $\langle\sigma v\rangle$}
...
A more useful fit for the heavy-particle reactions is a double polynomial fit in both E and T:\\
      ...
      $$\ln \langle\sigma v\rangle = \sum_{n=0}^N\sum_{m=0}^M \alpha_{mn}(\ln E)^n(\ln T)^m.$$
...
is a double polynomial fit in both $n$ and $T$:\\
      ...
      $$\ln \langle\sigma v\rangle = \sum_{n=0}^N\sum_{m=0}^M \alpha_{mn}(\ln \tilde{n})^n(\ln T)^m.$$
Most fits in the present database have been set up for density range $1e8 - 1e16$ cm$^{-3}$,
but with the density parameter $\tilde n$ being a scaled density: $\tilde n = n/10^8$, $n$ in cm$^{-3}$.


\subsection*{Example of Use of Fits}
...
\begin{eqnarray*}
\ln \langle\sigma v\rangle & = & (3.27139e+01) + 1.35365e+01 (2.30259)\\
& & -5.73932e+00 (2.30259)^2 + 1.56315e+00 (2.30259)^3\\
& & -2.87705e-01 (2.380259)^4 + 3.48255e-02 (2.30259)^5\\
...
\end{equation}
the \textbf{``total scattering cross section"} where $\theta$ is the scattering angle in the center of mass frame and $d\sigma/d\theta$ is the differential scattering cross section,
...
\begin{equation}
\sigma^t(v_{rel}) = \int_0^{\pi} d\theta \frac{\partial\sigma(\theta,v_{rel})}{\partial\theta} (1-\cos(\theta)) \sin(\theta)
\end{equation}
the \textbf{``diffusion cross section"}, in which scattering events are weighted with their momentum transfer efficiency  $(1-\cos(\theta)$
\begin{equation}
\sigma^t(v_{rel}) = \int_0^{\pi} d\theta \frac{\partial\sigma(\theta,v_{rel})}{\partial\theta} (1-\cos^2(\theta)) \sin(\theta)
\end{equation}
the \textbf{``viscosity cross section"} arising in continuum descriptions of gas and plasma transport.
...
rest frame of the background $``p"$, i.e. with the beam energy parameter re-defined as $\widetilde{E}_0=m_0/2|{\bf V_0}-{\bf V_p}|^2$.
...
As discussed already for general Beam-Maxwellian rate coefficients, paragraph \ref{sec1.3.2}, in a first step we transform to the rest frame of the plasma:
\begin{equation}
...
original Janev-Langer  cross section, improved at low energies by asymptotic fit.
...
\begin{small}\begin{verbatim}
  ...
  al0 -3.225274882000e+01   al1 -1.713112000000e-01   al2  0.000000000000e+00
...
\end{verbatim}\end{small}
\subsubsection{
Reaction 3.1.8JM $  p + H(1s) \rightarrow H(1s) + p  $
}
Janev cross section for momentum exchange ( = cx total cross sections times 2), obtained by
increase absolute coefficient $a_0$ by factor $\ln(2)$
...
Langevin cross section: $\sigma(E)=A \times E^{-1/2}$ such that rate coefficient,
$\langle\sigma \times v_{rel} \rangle= const = 2.0~ 10^{-8}$ cm$^3$/s\\
...
\begin{small}\begin{verbatim}
   b0 -3.455402000000E+01  b1  1.633449000000E+01   b2 -7.4809520000000E+00
   b3  2.334744000000E+00  b4 -5.161607000000E-01   b5  7.9023200000000E-02
   b6 -7.919992000000E-03  b7  4.635395000000E-04   b8 -1.1917140000000E-05
\end{verbatim}\end{small}
...
\begin{small}\begin{verbatim}
   b0 -4.583945000000E+01  b1  2.487449000000E+01   b2 -1.1691680000000E+01
   b3  4.066783000000E+00  b4 -1.070205000000E+00   b5  1.9857720000000E-01
   b6 -2.383370000000E-02  b7  1.636738000000E-03   b8 -4.8491950000000E-05
\end{verbatim}\end{small}
...
missing: dissociative recombination DR, $N_2^+ \rightarrow 2 N$
instead here, additionally  $N_2^+ \rightarrow N_2^{2+}$
...
These energy weighted rate coefficients are obtained from
...
\subsection{
Reaction 2.0b     $ p + H_2(v=0) \rightarrow  H_2^+ + \cdots$, Ratio  \ $H_2^+/H_2$
}
\textbf{gain }of $H_2^+$:
...
\subsection{
Reaction 2.1.5   $H + e \leftrightarrow H^+$,  Ratio $H^+/H(1)$
}
...
\subsection{
  Reaction 2.1.8det  $H^+  + e \rightarrow H(1s) + ..., \  <de> + 13.6 \ [eV] $
}
