...
\usepackage[hypertex,unicode=true,
 bookmarks=true,bookmarksnumbered=false,bookmarksopen=false,
 breaklinks=false,pdfborder={0 0 0},backref=false,colorlinks=true]
 {hyperref}
\hypersetup{pdftitle={AMJUEL Documentation},linkcolor=black, citecolor=black, urlcolor=blue, filecolor=blue}
...
(i.e. H.8 2.1.5G out).
 ...
 \item update  23.5/00 \\
  H.12 2.2.5fl revised: labelling of n=2 triplet levels in Fujimoto's
  code corrected.
 \item update  06.8/00 \\
 ...
 \item update  13.03/04 \\
  3.1.8d: Langevin approximation for cx , for testing of internal consistency\\
  H.1:  3.1.8d done  \\
  H.3:  3.1.8d done   \\
  H.9:  3.1.8d done   \\
  H.3:  3.1.8o:  original fit from Janev's springer 1987 book\\
 ...
 \item update  14.02/06 Strahl (ADAS89) data completed for Iron \\
  H.8 2.26B0, 2.26B1, H.11 2.26B0
 \item update  14.07/06 H.4 7.2.3a 7.2.3b redone\\
 ...
 \item update  14.01/07 H.10, 2.1.8-t: for comparison with ADAS PRB \\
  ...
  hence: this corrections seems to be missing in ADAS\\
  ...
  $ee, ei, ie$, and $ii$ collisions. Fit-Flag: 10
  ...
  \item update jan. 2010: some text added regarding ADAS CR rates
  ...A0, in H.4, H.10 and H.12
  \item update Mar. 2011:  fit H.3: 3.2.3 corrected. Was completely off.
  ...
  \item update Dec. 08 2011:  O ionisation, O+ recombination: ADAS 96, H.4 and H.10 fits added 2.8A0 and 2.3.8A0, resp.
  \item update Dec. 09 2011:  N ionisation, N+ recombination: ADAS 96, H.4 and H.10 fits added 2.7A0 and 2.3.7A0, resp.
  \item update July 12:  H.2: 3.2.3o re-introduced, for backward compatibility, i.e. $E_{H_2}=0.37$ eV, (as around year 2000)
  ...
  \item update Aug. 12: added H.12: 2.2.15a,..2.2.15e red. pop. coeff H(2)...,H(6) with respect to H3+,
  ...
  H.2 2.7.5, 2.7.9, 2.7.10, 2.7.11, 2.7.12, 2.7.15
  do be done: plots, and 2.7.14 (Dissociative recombination)\\
  ...
  \item Jan 14:  some reaction headers  edited, in H.12 (for automatic online processing)
\end{itemize}
\newpage
...
\subsection*{SLREAC.f: Fortran module for reading Data from AMJUEL}
...
C  H.0
      IF (ISW.EQ.0) THEN
        ...
        IFLG=0
C  DEFAULT POTENTIAL: GENERALIZED MORSE
        IFTFLG(IR,IFLG)=2
C  H.1
      ELSEIF (ISW.EQ.1) THEN
        ...
        IFLG=1
C  DEFAULT CROSS SECTION: 8TH ORDER POLYNOMIAL OF LN(SIGMA)
        IFTFLG(IR,IFLG)=0
C  H.2
      ELSEIF (ISW.EQ.2) THEN
        ...
        IFLG=2
C  DEFAULT RATE COEFFICIENT: 8TH ORDER POLYNOMIAL OF LN(<SIGMA V>) FOR E0=0.
        IFTFLG(IR,IFLG)=0
C  H.3
...
c
c   loop to find begin of data, see subr. slreac.f in "file-handling"
c
        GOTO 100
...
9           CONTINUE
          END IF
C
C  READ ASYMPTOTES, IF AVAILABLE
C  I0P1=1 FOR CROSS SECTION
C  I0P1=2 FOR (WEIGHTED) RATE
          I0P1=I0+1
          IF (ISW.EQ.0) GOTO 12 ! NO ASYMPTOTES FOR POTENTIALS
...
C
C  ANY OTHER ASYMPTOTES INFO ON FILE?  SEARCH FOR Tmin, or Emin
...
rate coefficients (H.2, H.3 and H.4)  and momentum (H.5, H.6 and H.7) and energy weighted (H.8, H.9, H.10) rate coefficients, as well as some
supplementary data fits (H.11 and H.12) mainly for post processing and other purposes.
...
in the EIRENE databases, which initially had been build on data of ref. \cite{kn:Janev}, the laboratory energy  of one of the colliding particles (usually the charged particle)
is used, with the second collision partner (usually the neutral particle) being at rest. I.e., $\sigma= \sigma(E_{lab,1})$.
To convert to center of mass energies, one uses $E_{lab,1} =m_1/2 \ v_1^2$ and $E_{CM} = \mu/2 \ v_{rel}^2$ with
...
Fits for $\langle\sigma \cdot v  \rangle (T_p) \ [cm^3/s]$
...
a finite velocity (energy $E_0$) of the  test particle (or beam) or a fluid drift $V_p$ in the Maxwellian background, or both.

Beam-Maxwellian rate coefficients with drifting Maxwellians $f_{maxw}(T_p,V_p)$ of the background particles $``p"$ can be evaluated in the
rest frame of the background $``p"$, i.e. with the beam energy parameter re-defined as $\widetilde{E}_0=m_0/2|V_0-V_p|^2$.
...
of (charged) particles $``p"$ with stationary Maxwellian distribution and temperature $T_p$.
...
but a density dependence arises due to multiple (ladder-like) processes.
...
divided by 100.
\subsubsection*{H.5: momentum weighted rates vs. temperature}
\addcontentsline{toc}{subsubsection}{H.5: momentum weighted rates vs. temperature}
currently not in use. Probably for electron-neutral friction vs. $T_e$
\subsubsection*{H.6: momentum weighted rates vs. temperature and energy}
...
&=&
{\bf e}_0 \ \mu\sqrt{\frac{2T_p}{m_p}}\cdot\left[I^{(1,1)}(E_0,T_p)-
          \frac{1}{2}\sqrt{\frac{m_0\cdot T_p}{m_p\cdot E_0}}I^{(1,0)}(E_0,T_p)\right]
\end{eqnarray}
where
...
${\bf e}_0 = {\bf v}_0/v_0$ the speed unit vector in the direction of the test particle flight. Due to symmetry of the background velocity distribution
...
$l=0$ stands for using the total collision cross section $\sigma^0(v_{rel}) = \int d\theta \sin(\theta) \ d\sigma(v_{rel},\theta)/d\theta$, here $d\sigma/d\theta$ denotes the differential scattering cross section
...
As discussed already for general Beam-Maxwellian rate coefficients, in a first step we transform to the rest frame of the plasma:
\begin{equation}
...
\end{equation}
where $\widetilde{{\bf v}}_0$ the test particle velocity in the rest frame of the plasma and  $\widetilde{E}_0$ its the corresponding energy.
...
The vector $\widetilde{{\bf V}}_0$ the average (macroscopic) test particle flow velocity in the rest frame of the plasma;
The rate of momentum transfer from the test particle community $``0"$ to the (drifting Maxwellian) background community $``p"$
is a vector in this direction  $ \widetilde{{\bf V}}_0/\widetilde{V}_0$.\\
From here on: to be re-written, below: old text....
...
\subsubsection*{H.7: momentum weighted rates vs. temperature and density}
\addcontentsline{toc}{subsubsection}{H.7: momentum weighted rates vs. temperature and density}
\subsubsection*{H.8: energy weighted rates vs. temperature}
\addcontentsline{toc}{subsubsection}{H.8: energy weighted rates vs. temperature}
\subsubsection*{H.9: energy weighted rates vs. temperature and energy}
\addcontentsline{toc}{subsubsection}{H.9: energy weighted rates vs. temperature and energy}
\subsubsection*{H.10: energy weighted rates vs. temperature and density}
...
loss rate coefficients associated with the particular process. If
...
or in electron impact de-excitation of meta-stables,
...
in section H.4.
\newpage
\subsubsection*{H.11: other data, e.g. reduced population coefficients}
\addcontentsline{toc}{subsubsection}{H.11: other data, e.g. reduced population coefficients}
\subsubsection*{H.12: other data, e.g. reduced population coefficients}
...
\subsection{
Reaction 0.100  bulk-electrons on test-electron, Fokker-Planck elastic }
...
\subsection{
Reaction 0.101   bulk-electrons + test-ions, Fokker-Planck elastic }
Data for Trubnikov Potentials for Fokker-Planck collision
...
\subsection{
Reaction 0.102   bulk-ions on test-electron, Fokker-Planck elastic }
Data for Trubnikov Potentials for Fokker-Planck collision
...
\subsection{
Reaction 0.103   bulk-ions + test-ions, Fokker-Planck elastic }
Data for Trubnikov Potentials for Fokker-Planck collision
...
Fit as given in \cite{kn:Janev} (given here only for reference purposes).
EIRENE uses fit as given in the unpublished preprint for
\cite{kn:Janev} in its default database.
...
\newpage
\subsubsection{
Reaction 3.1.8d $  p + H(1s) \rightarrow H(1s) + p  $
}
For testing purposes:
Langevin cross section: $\sigma(E)=A \times E^{-1/2}$ such that rate coefficient, $\langle\sigma \times v_{rel} \rangle= const = 2.0~ 10^{-8}$ cm$^3$/s\\
...
\begin{small}\begin{verbatim}
   a0 -3.187162820000e 01    a1 -0.500000000000e-00    a2  0.000000000000e-00
...
$ \langle\sigma v \rangle_{eff} = \langle\sigma v \rangle_{H_2(\nu=0)} + \sum_{\nu=1}^{14}
\langle\sigma v \rangle_{H_2(\nu)} \cdot pH_2(\nu) $
...
Assume: incident $H_2$ particle with 0.1 eV and $T_i = T_e$,
...
\begin{small}\begin{verbatim}
b0  -3.455402E+01  b1     1.633449E+01     b2    -7.480952E+00
b3   2.334744E+00  b4    -5.161607E-01     b5     7.902320E-02
b6  -7.919992E-03  b7     4.635395E-04     b8    -1.191714E-05
\end{verbatim}\end{small}
...
\begin{small}\begin{verbatim}
b0   -4.583945E+01  b1   2.487449E+01       b2    -1.169168E+01
b3    4.066783E+00  b4  -1.070205E+00       b5     1.985772E-01
b6   -2.383370E-02  b7   1.636738E-03       b8    -4.849195E-05
\end{verbatim}\end{small}

\newpage
this page is intentionally left blank
\newpage
Here:  $N_2^+ \rightarrow \dots$\\
fehlt: DR, $N_2^+ \rightarrow 2 N$
dafuer extra:  $N_2^+ \rightarrow N_2^{2+}$
...
\begin{small}\begin{verbatim}
b0   -5.137074E+01   b1   3.135383E+01    b2   -1.560923E+01
b3    5.444979E+00   b4  -1.354319E+00    b5    2.264527E-01
b6   -2.359782E-02   b7   1.362664E-03    b8   -3.285464E-05
\end{verbatim}\end{small}



\subsection{
Reaction 2.7.12  $e + N_2^+ \rightarrow e + N + N^+ +e$ }
...
\begin{small}\begin{verbatim}
b0  -2.695366E+01  b1   8.978136E+00  b2  -4.296075E+00
b3   1.392124E+00  b4  -3.247302E-01  b5   5.222337E-02
b6  -5.327033E-03  b7   3.023081E-04  b8  -7.105405E-06
\end{verbatim}\end{small}
...
\begin{small}\begin{verbatim}
b0   -4.675067E+01   b1    2.768681E+01   b2    -1.260874E+01
b3    3.402022E+00   b4   -5.302686E-01   b5     3.823736E-02
b6    6.310561E-04   b7   -2.798009E-04   b8     1.272992E-05
\end{verbatim}\end{small}
...
\subsection{
Reaction 2.2FJ  $e + He(1|1S) \rightarrow e + He^+ + e$ } Freeman
...
same cross section data source was used for the Bell rate coefficient. (Checked, Oct.2013: original Bell report and Brook cross sections
are identical)
...
\newpage
\subsection{
Reaction 2.3.9  $e + He \rightarrow 2e + He^+ ,\ $ }
...
Reaction 3.1.8  $ p + H(1s) \rightarrow H(1s) + p   $
 ...
 atoms. Cross section: Janev  3.1.8, improved fit. (Identically in: hydhel.tex)
...
\subsection{
Reaction 3.1.8o $ p + H(1s) \rightarrow H(1s) + p   $
}
...
Reaction 2.1.5 $H + e \rightarrow H^+ + 2e $ }
   ...
   Effective hydrogenic ionization rate    Data: K. Sawada/T. Fujimoto (redone: 2007)
   \cite{kn:Sawada}
...
\begin{small}\begin{verbatim}
  ...
  Max. rel. Error:   1.3872 %
  Mean rel. Error:   0.4632 %

\end{verbatim}\end{small}
\begin{figure} \label{2.1.5FU}
\epsfxsize=16truecm
\epsffile{Amjuel_PS/hydi1FU.ps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Amjuel_PS/hydr1FU.ps}
\end{figure}
\newpage


\subsection{
  Reaction 2.1.8a  $H^+ + e \rightarrow H(1s) + h\nu $
}
...
T_i$, and an energy of $H_2$-Beam  = 0.1 eV.
\begin{small}\begin{verbatim}
        E-Index:     0                     1                     2
  T-Index:
...
\cite{kn:Fujimoto}. Here: effective recombination into metastable
level $(2|3S)$.
...
\newpage
\subsection{
Reaction 2.3.7A0  $e + N^{+} \rightarrow N + h\nu$ }
...
\subsection{
Reaction 2.3.8A0  $e + O^{+} \rightarrow O + h\nu$ }
...
\epsfxsize=16truecm
\epsffile{Amjuel_PS/mar.ps}
\end{figure}
\newpage

\subsection{
Reaction 3.2.3d   $p + H_2   \rightarrow p + H + H  $ (MAD via $H_2^+$, cold $H_2$)
}
...
\epsfxsize=16truecm
\epsffile{Amjuel_PS/marmad.ps}
\end{figure}
\newpage

\subsection{
Reaction 3.2.3i   $p + H_2   \rightarrow p + p + H +e $ (MAI via $H_2^+$, cold $H_2$)
}
...
\epsfxsize=16truecm
\epsffile{Amjuel_PS/marmadmai.ps}
\end{figure}
\newpage
...
$\langle\sigma v_e E_{elec}\rangle(T_e) \approx 0.88 ~kT_e ~\langle\sigma v_e\rangle$,
...
\subsection{
Reaction 2.2B0 $e + He \rightarrow e + He^+ + e $ 11/94 update
}
...
\subsection{
Reaction 2.2B1 $e + He^+ \rightarrow e + He^{++} + e$ 11/94 update
}
...
The energy weighted rate is a function of ion temperature
...
$E_p$ is the kinetic Energy of the impacting electron or ion in eV.

\subsection{
Reaction 2.1.5JH  $ e + H \rightarrow H^+ + 2e $
}

Electron energy loss weighted rate coefficient. Data: Johnson
...
\begin{small}\begin{verbatim}
        E-Index:     0                     1                     2
  T-Index:
        0   -2.496890299217D+01   -1.273688545845D-04   -1.125384977325D-03
        1    1.002013909765D+01   -4.998007047717D-03    4.236137799887D-03
        2   -4.896294518117D+00    8.177115078429D-03   -4.925759681469D-03
        3    1.768851940975D+00   -4.481347509834D-03    4.648385259559D-03
        4   -4.652526585609D-01   -7.656253146152D-04   -9.361714478767D-04
        5    8.226285887582D-02    1.037727503492D-03   -1.508390575673D-04
        6   -9.129771934584D-03   -2.608462447183D-04    5.886004737995D-05
        7    5.723705563955D-04    2.633193552448D-05   -4.397132393249D-06
        8   -1.544256993027D-05   -9.406032807407D-07    1.550637085291D-08

        E-Index:     3                     4                     5
  T-Index:
        0    1.018590777442D-03   -3.269982831774D-04    5.042447421862D-05
        1   -1.466888644835D-03    2.301618349583D-04   -1.427944657832D-05
        2    4.885494542737D-04    1.941225379371D-04   -5.178391119709D-05
        3   -1.079687251318D-03    5.144157976175D-05    7.357125343406D-06
        4    2.812213510409D-04   -1.458924023754D-05   -3.083463874984D-07
        5    2.238632034696D-05   -1.005354974437D-05    1.133504770172D-06
        6   -9.113176958030D-06    2.185948491661D-06   -2.277120309707D-07
        7   -8.341204170993D-08    1.284814405982D-08   -1.480104692513D-09
        8    7.784081468177D-08   -1.612998464928D-08    1.709377000128D-09

        E-Index:     6                     7                     8
  T-Index:
        0   -4.033262865457D-06    1.607972869558D-07   -2.511916027721D-09
        1   -1.137949109233D-07    4.620694701399D-08   -1.279496830183D-09
        2    4.880534584727D-06   -2.033375661957D-07    3.121886656129D-09
        3   -7.626434576289D-07    1.571985401882D-08    1.705457752105D-10
        4   -1.624239401491D-07    2.231167127337D-08   -6.629725659802D-10
        5    1.507747938154D-08   -6.512923845349D-09    2.061794045379D-10
        6   -2.600337773802D-11    9.754181150032D-10   -3.210602375126D-11
        7    1.130104489823D-09   -1.128359028928D-10    3.018509011518D-12
        8   -1.335503125905D-10    6.654738851468D-12   -1.362100356409D-13

  Max. rel. Error:   0.5400 %
  Mean rel. Error:   0.1021 %

\end{verbatim}\end{small}
\begin{figure} \label{2.1.5iFU}
\epsfxsize=16truecm
\epsffile{Amjuel_PS/hydli1_trFU.ps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Amjuel_PS/hydlr1_tr.ps}
\end{figure}
\newpage

\subsection{
  Reaction 2.1.8t $H^+ + e  \rightarrow H(1s) + ....$
}
     do not use: only for testing  ADAS PRB rate. Same as above,
     but $dln\langle sig v\rangle /dlnT$ removed in free bound transitions

\begin{small}\begin{verbatim}

        E-Index:     0                     1                     2
  T-Index:
        0   -2.584114063574D+01    3.954583810848D-03    1.226207753369D-03
        1   -6.722051475883D-01   -2.374237574381D-02    1.220732882547D-02
        2    4.298773953306D-02    1.518903102685D-02   -1.040687790971D-02
        3    1.175376172121D-02    1.836334349240D-03   -2.184151606814D-03
        4    2.444695663440D-04   -1.441024928821D-03    1.471132341668D-03
        5   -7.999296912631D-04   -4.378152568604D-05   -2.668044290688D-05
        6    1.108758364533D-04    7.121000938022D-05   -5.163434271747D-05
        7   -4.950502517163D-06   -8.912712700419D-06    6.926837416082D-06
        8    3.557192989612D-08    3.359548284329D-07   -2.657303348021D-07

        E-Index:     3                     4                     5
  T-Index:
        0    1.161886460214D-03   -3.757012643654D-04    6.587092485914D-05
        1   -4.427696784538D-03    5.748375032819D-04   -3.220643016363D-05
        2    4.132078943994D-03   -7.076580646192D-04    6.327610663109D-05
        3    3.146181789043D-04    1.651017598401D-05   -9.612296616067D-06
        4   -4.351131677737D-04    5.788339105175D-05   -2.828897327260D-06
        5    2.417709468754D-05   -5.927135718855D-06    5.113794389586D-07
        6    1.195371206751D-05   -1.052408071577D-06    2.254123815507D-08
        7   -1.758251512799D-06    1.862206781908D-07   -6.990610754370D-09
        8    6.869977978150D-08   -7.443450588757D-09    2.763775786889D-10

        E-Index:     6                     7                     8
  T-Index:
        0   -5.795276987659D-06    2.579368998990D-07   -4.547564421362D-09
        1   -2.041847132375D-07    8.196018761672D-08   -2.096446721567D-09
        2   -2.702581145483D-06    4.031977763649D-08    1.430532547657D-10
        3    1.006448379973D-06   -4.380712513696D-08    7.199826302410D-10
        4   -7.020663824401D-08    1.049689217264D-08   -2.472269909817D-10
        5   -7.721165607539D-09   -7.397060161257D-10    2.189989404804D-11
        6    1.603333205140D-09   -1.122265017519D-10    2.612077503992D-12
        7   -1.801587396805D-10    2.305224523251D-11   -5.710248072777D-13
        8    9.775403278327D-12   -1.119509805266D-12    2.693883939329D-14

  Max. rel. Error:   6.9014 %
  Mean rel. Error:   0.7184 %

\end{verbatim}\end{small}
\begin{figure} \label{2.1.8r4}
\epsfxsize=16truecm
\epsffile{Amjuel_PS/hydlr4_tr.ps}
\end{figure}
\newpage
...
\subsection{
Reaction 2.3.13a $e+ He^+(1s) \rightarrow   He(1|1S)+ ...$
}
Helium multi-step model, here recombination
...
\subsection{
Reaction 2.2B0   $ He + e  \rightarrow He^*,  ratio  \ <de>(T_e) \ [eV], $
}
...
\subsection{
Reaction 2.4B0  $   Be + e \rightarrow Be^*,  ratio \ <de>(T_e)  \ [eV],$
}
...
\end{verbatim}\end{small}
\subsection{
Reaction 2.5B0   $  B + e \rightarrow B^*, ratio \ <de>(T_e) \ [eV] $
}
...
\subsection{
Reaction 2.6B0   $  C + e  \rightarrow C^*, ratio \ <de>(T_e) \ [eV] $
}

 Electron cooling rates for neutral and single
 charged Carbon Particles, divided by corresponding collision rates, i.e. the fit here is for the mean electron energy cost per collision [eV].
...
\subsection{
Reaction 2.7B0   $  N + e  \rightarrow N^*, ratio \ <de>(T_e) \ [eV] $
}
...
\end{verbatim}\end{small}
\subsection{
Reaction 2.10B0   $  Ne + e \rightarrow Ne^*, ratio \ <de>(T_e) \ [eV] $
}
...
\subsection{
Reaction 2.18B0  $   Ar + e \rightarrow Ar^*, ratio  \ <de>(T_e) \ [eV] $
}
...
\subsection{
Reaction 2.0b     $ p + H_2(0) \rightarrow  H_2^+ + \cdots, ratio  \ H_2^+/H_2$
}
\textbf{gain }of $H_2^+$:
...
\subsection{
Reaction 2.0c   $p + H_2(v) \rightarrow H_2^+ + \cdots , ratio  \ H_2^+/H_2$
}
...
density, for a given ground state atom density. In CR equilibrium this ratio is a function of $T_e$ and $n_e$.
...
\subsection{
Reaction 2.1.5de  $ H + e \rightarrow H^+ + 2e, \ ratio~   <de> \ [eV]$}
...
\subsection{
Reaction 2.1.5o  $ H + e \rightarrow H^+ + 2e, \ ratio~  <de> \ [eV]$ Ly-opaque}
...
\subsection{
  Reaction 2.1.8de $H^+  + e \rightarrow H(1s) + ..., ratio~ \  <de> + 13.6 \ [eV] $
}
...
\epsfxsize=16truecm
\epsffile{Amjuel_PS/hydlr2_tr.ps}
\end{figure}
\newpage

\subsection{
  Reaction 2.1.8det  $H^+  + e \rightarrow H(1s) + ..., ratio ~ \  <de> + 13.6 \ [eV] $
}

     do not use: only for testing  ADAS PRB rate. Same as above,
     but $dln\langle sig v\rangle /dlnT$ correction removed in free bound transitions

\begin{small}\begin{verbatim}
        E-Index:     0                     1                     2
  T-Index:
        0    2.717272483503D+00    4.838148584777D-03   -5.452382162679D-03
        1    9.398477868692D-02   -3.227619737070D-03    4.226409144028D-03
        2    4.655561657301D-02   -1.794086218129D-03    1.574904484615D-03
        3    1.752147403614D-02    6.571521522207D-04   -6.466109863122D-04
        4    5.711608146415D-04    3.906159455991D-05   -8.042732179727D-07
        5   -8.855564492615D-04   -4.860632362483D-06   -1.651598787923D-05
        6    1.014374299565D-04   -4.027760840144D-06    8.484592724246D-06
        7   -2.846502397515D-06    5.309122118644D-07   -9.356021739680D-07
        8   -5.679960598221D-08   -1.558419266886D-08    2.906797123044D-08

        E-Index:     3                     4                     5
  T-Index:
        0    1.885124626882D-03   -3.490656696534D-04    3.491186099671D-05
        1   -1.900077557522D-03    4.379278113300D-04   -5.603024107776D-05
        2   -4.448332157963D-04    5.714274030709D-05   -2.749703564976D-06
        3    2.681568205449D-04   -5.998554426398D-05    7.698667304288D-06
        4   -2.181979202874D-05    8.551373474056D-06   -1.412905455689D-06
        5    1.135371808870D-05   -2.664709675423D-06    2.912002096469D-07
        6   -3.800776412567D-06    7.135817419881D-07   -6.186941704782D-08
        7    3.881982777721D-07   -6.848429314039D-08    5.416002739854D-09
        8   -1.174898545591D-08    1.964026350500D-09   -1.349132007732D-10

        E-Index:     6                     7                     8
  T-Index:
        0   -1.927839216428D-06    5.349623808990D-08   -5.709971819658D-10
        1    4.013822594444D-06   -1.505270436160D-07    2.297907111216D-09
        2   -5.363215391657D-08    9.480384182490D-09   -2.385933435062D-10
        3   -5.631660932587D-07    2.182421032038D-08   -3.453636466989D-10
        4    1.168216950481D-07   -4.806652273476D-09    7.830157266677D-11
        5   -1.531714455229D-08    3.317225758981D-10   -1.284623080698D-12
        6    2.145180037262D-09    6.298630910549D-12   -1.357497097473D-12
        7   -1.412805388428D-10   -3.796755190068D-12    1.881402203423D-13
        8    1.168883361950D-12    2.677503515711D-13   -8.533609868832D-15

  Max. rel. Error:   0.5477 %
  Mean rel. Error:   0.2357 %


\end{verbatim}\end{small}

\begin{figure} \label{2.1.8lr3}
\epsfxsize=16truecm
\epsffile{Amjuel_PS/hydlr3_tr.ps}
\end{figure}
\newpage
...
\subsection{
  Reaction 2.1.8o $H^+ + e \rightarrow H(1s) + ..., ratio ~ \ <de> + 13.6 \ [eV] $
  Ly-opaque
}
...
\subsection{
  Reaction 2.3.9a  $e + He \rightarrow He^+(1s) + 2e ,  \ \  ratio ~ <de> \ [eV] $
}
...
\subsection{
  Reaction 2.6A0 $C  + e \rightarrow C^+   + 2e \ \  ratio ~ <de> [eV] $
}
...
\subsection{
  Reaction 2.3.6A0 $C^+ + e \rightarrow C     + ..., \ \   ratio ~  <de>+11.3 \ [eV] $
}
...
\subsection{
Reaction 2.2.5fu $H_2 + e \rightarrow .....+H_2(N=3,Triplet)$}

Ratio $H_2(N=3, Triplet)/H_2*2/9 $
...
Reaction 2.2.15e $ H_3^+ + e \rightarrow ...+ H(6) , \   ratio ~ H(6)/H_3^+  $
 ...
 Ratio of population coefficients: p(n6)/nH3+
...
\subsection{
Reaction 2.0a  $e + H_2 \rightarrow H_2^+ + \cdots , ratio \ H_2^+/H_2$
}
 ...
 here:  gain from electr. impact only
...
\newpage
\subsection{
Reaction 2.0b $e + H_2(v=0) \rightarrow H_2^+ + \cdots , ratio  \ H_2^+/H_2$
}
...
\subsection{
Reaction 2.0c   $e +  H_2(v) \rightarrow H_2^+ + \cdots , ratio \ H_2^+/H_2$
}
 Multi-step hydrogenic density ratios
 Data: T.Fujimoto, P.T. Greenland
