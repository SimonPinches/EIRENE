...
correction in reaction H.1 32, (same as already done in
      H.2  32:  NH3+ rather NH3 in product channel.

Nov.20:  KER and Eth parameters introduced from ITER branch
         Needs checking.

The rate coefficients $k$ (H.2 part) are given in their
original Arrhenius form as:
\begin{equation}
\ln(k) = -Eth/T + a_0 +  a_1 \ln(T)
\end{equation}
with $k$ in cm$^3$/s and $T$ in eV (electron temperature for electronic reactions ; 
common gas temperature ($T= T_1 = T_2$) for the others).

This formalism is based on the EIRENE implementation
of Arrhenius rate coefficients, as a slight generalisation
of the former AMJUEL format formalism.
