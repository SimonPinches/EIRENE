...
% hypertex, % use for activating links in DVI file
 ...
 {hyperref}
\hypersetup{pdftitle={METHANE Documentation},linkcolor=blue, citecolor=blue, urlcolor=blue, filecolor=blue}
...
Data for breakup of Hydrocarbons, taken from \cite{kn:Ehrhardt}, i.e.: so called ``Ehrhard-Langer database'', 1987.
This database has been superseded by the later ``Janev-Reiter'' cross-section database \cite{kn:JR-2002}, but is kept here
for backward compatibility.

Format as in HYDHEL \cite{kn:Janev} or AMJUEL,
both are available on same web side, as is this present file:
\begin{verbatim}
www.eirene.de/html/a_m_data/html
\end{verbatim}
\subsection{Calculation of Reaction Rate Coefficients}
(taken from \cite{kn:Janev}):

The Maxwellian-averaged reaction rate coefficients for a particle of mass m and fixed energy
$E = mV^2/2$
incident on a Maxwellian distribution of particles of mass M
and temperature $T = Mu^2/2$ (hence: $u = \sqrt{2T/M}$, the thermal velocity) for the
heavy-particle reactions is\\

      $$\langle\sigma v\rangle(E,T) = \frac {1}{\pi^{1/2}uV} {\int_{v_{th}}^\infty}  v_r^2dv_r\sigma(E_r) \{\exp[-(v_r-V)^2/u^2]
-\exp[-(v_r+V) ^2/u^2]\},$$\\

and for the electron reactions (taking $M= m_r \approx m_e, T \approx T_e, E=V=0)$,  with $m_e, T_e$ being the electron mass and electron temperature,
respectively\\

      $$\langle\sigma v\rangle(T) = \frac {4}{\pi^{1/2}u^3} {\int_{v_{th}}^\infty} v_r^3dv_r\sigma(E_r) \exp(-v_r^2/u^2), $$\\

where $v_r = |\vec{V}-\vec{v}|$ is the relative (collision) velocity related to $E_r$ by
$E_r = m_r v_r^2/2, m_r = mM/(m+M)$ being
the reduced mass of colliding particles, and $v_{th}$ is the value of $v_r$ at
threshold, $E_r = E_{th}$.
Note that for electron collisions our approximations result from $v_r\approx v_e$.

We used a
number of mathematical techniques to assure the reaction rate integrals were computed accurately for the
nearly five orders of magnitude variation in $E$ and $T$.

For the calculation of reaction rate coefficients, it is necessary
to know the cross-section from threshold to very large values of
the energy. However, the digitized data necessarily represent the
cross-section for a finite energy range and for a finite number of
points (less than 100). Our solution to this problem is: (1) to
calculate values of a within the digitized range by  linear
interpolation of $\ln \sigma$ in $\ln E$; and (2) to extrapolate
$\sigma$ outside the digitized range by a method depending on the
type of reaction as discussed in \cite{kn:Janev}.


\subsection{Numerical Fits to $\sigma$ and $\langle\sigma v\rangle$}
(taken from \cite{kn:Janev}):
...
For the electron reactions, $\langle\sigma v\rangle$ is essentially independent of E within the range of energies considered
here. A more useful fit for the heavy-particle reactions is a double polynomial fit in both $E$ and $T$:\\
...
$E$ and $T$. We tabulate the coefficients for a 9 x 9 double fit of this form.

An error is given in \cite{kn:Janev} for each fit as an indication of the quality of the fit. The error is defined as

      $\frac{1}{N} \sum_{i=1}^N (\ln x_i - \ln x_{fit,i})^2,$

where N is the number of points fit, and $x$ is $\sigma$  or $\langle\sigma v\rangle$. An error of $10^{-4}$ is a good fit;
that is, the fit is very close to the actual values, most likely well within the error in the data.


\subsection{various notes regarding this particular data-set:}
\begin{itemize}
\item
In PPPL report \cite{kn:Ehrhardt} also the Born-type fitting is
promised, but, apparently, never given. So do not use the  aBorn,
nBorn coefficients here.
\item
For some processes the original polynomial fit, eq. (\ref{1})
(labeled \textbf{fit-flag=0}) for cross-sections is very poor. See
\cite{kn:Dose}.
In one instant (H.1: reaction 2.1, cross-section fit) we have scanned and
digitalized the cross-section data from
\cite{kn:Ehrhardt} and redone the fit (same fitting expression).
These are then referred to as H.1 ...label...\mbox{\_new}, the original fit
coefficients for these processes are H.1 ...label...
This example indicates that the fitting procedure,
rather than the fit-expression itself, was probably the origin of the poor quality of fits.
\item
For some processes newer cross-section fits, from the 2002 ``Janev-Reiter database"
\cite{kn:JR-2002} have been included.
These reactions are labeled by additional letters ``jr",  e.g. H.1  2.1jr,
or by the single additional letter ``t" (for ``total cross-section").

The fitting expression for those is different from the 8th order polynomial fit
in the original PPPL database from 1987.
The flag: \textbf{fit-flag=3} is used for these.


fit-flag=3 is the cross-section fit expression given in the 2002 Janev-Reiter database \cite{kn:JR-2002} for so called I-DI (ionisation-dissociative ionisation) processes:
\begin{equation}\label{4}
\sigma(E)=\frac{10^{-13}}{E E_{th}}\left[A_1 \ln(E/E_{th}) + \sum_{i=2}^N A_i(1-E_{th}/E)^i \right]
\end{equation}
which, at least, has proper asymptotic behaviour (assuming that always also dipole allowed channels contribute to the cross-section).
Parameter $A_0 = E_{th}$ is the ionisation (appearance) potential, i.e. the threshold energy of the process. Typically: N=6

We have then stopped adding newer cross-section to the present database, because
the full cross-section database for hydrocarbon fragmentation, up until propane,
is now given on the web page: www.hydkin.de.  This present database now only serves for backward compatibility of code runs.
...
\item update  2.3.01 \\
 ...
 \item update 11.5.01 \\
   ...
   H.2 plots checked (labels, text)  until 2.19.2
 \item update 16.5.01 \\
   cross-sections H.1 from \cite{kn:Ehrhardt} \\
   all plots done \\
   to be done: check extrapolation of sigma H.1  3.2 \\
   to be done: check masses, reduced masses, etc. (as in HYDHEL?)
 \item update summer 2005\\
Parameter fit-flag introduced. Default: fit-flag=0

fit-flag=0 is the original
polynomial fit expression as in \cite{kn:Ehrhardt}, or \cite{kn:Janev}, Eqs. (\ref{1},\ref{2})
...
\section{H.1 :  Fits for cross-sections}

\subsection{ Reaction 2.1  $e + CH_4 \rightarrow CH_4^+ + 2e $
}
...
\subsubsection{ Reaction 2.1\mbox{\_new} $e + CH_4 \rightarrow CH_4^+ + 2e $ }

Fit redone on digitalized data from report
\begin{small}\begin{verbatim}
   fit-flag 0
...
From Janev-Reiter 2002 database, fit-flag=3
...
\subsubsection{
Reaction 2.2jr  $e + CH_{3} \rightarrow CH_{3}^{+} + 2e $
}
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.1_2.1_2.2.eps}
\end{figure}
\newpage
...
\subsubsection{
Reaction 2.3jr      $e + CH_{2} \rightarrow CH_{2}^{+} + 2e $
}
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.1_2.3_2.4.eps}
\end{figure}
\newpage
...
\subsubsection{
Reaction 2.5.1jr     $e + CH_{4} \rightarrow CH_{3}^{+} + H + 2e $
}
...
\subsubsection{
Reaction 2.5.2jr     $e + CH_{4} \rightarrow CH_{2}^{+} + H_{2} + 2e $
}
...
\subsubsection{
Reaction 2.5.3jr     $e + CH_{4} \rightarrow CH^{+} + H_{2} + H + 2e $
}
...
\subsubsection{
Reaction 2.5.4jr     $e + CH_{4} \rightarrow C^{+} + 2H_{2} + 2e $
}
...
\subsubsection{
Reaction 2.5.5jr     $e + CH_{4} \rightarrow CH_{2} + H_{2}^{+} + 2e $
}
...
\subsubsection{
Reaction 2.5.6jr     $e + CH_{4} \rightarrow CH_{3} + H^{+} + 2e $
}
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.1_2.5_2.6_2.7.eps}
\end{figure}
\newpage

\subsection{
Reaction 2.6
$e + CH_{4} \rightarrow CH_{2}^+ + 2 H + 2e: small, out $
}
regarded as small, less than 7\% of total ionization of $CH_4$

\subsection{
Reaction 2.7
$e + CH_{4} \rightarrow CH^+ + 3 H + 2e: small, out $
}
regarded as small, less than 2\% of total ionization of $CH_4$
...
\subsubsection{
Reaction 2.8.1jr    $e + CH_{3} \rightarrow CH_{2}^{+} + H + 2e $
}
...
\subsubsection{
Reaction 2.8.2jr    $e + CH_{3} \rightarrow CH^{+} + H_{2} + 2e $
}
...
\subsubsection{
Reaction 2.8.3jr    $e + CH_{3} \rightarrow CH_{2} + H^{+} + 2e $
}
...
\subsubsection{
Reaction 2.8.4jr    $e + CH_3 \rightarrow C^{+} + H_2 + H + 2e $
}
...
\subsubsection{
Reaction 2.8.5jr    $e + CH_{3} \rightarrow CH + H_{2}^{+} + 2e $
}
...
\subsubsection{
Reaction 2.9.1jr   $e + CH_{2} \rightarrow CH^{+} + H + 2e $
}
...
\subsubsection{
Reaction 2.9.2jr   $e + CH_{2} \rightarrow C^{+} + H_{2} + 2e $
}
...
\subsubsection{
Reaction 2.9.3jr   $e + CH_{2} \rightarrow CH + H^{+} + 2e $
}
...
\subsubsection{
Reaction 2.9.4jr   $e + CH_{2} \rightarrow C + H_{2}^{+} + 2e $
}
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.1_2.8_2.9.eps}
\end{figure}
\newpage
...
\subsubsection{
Reaction 2.10.1jr   $e + CH \rightarrow C^{+} H + 2e $
}
...
\subsubsection{
Reaction 2.10.2jr  $e + CH \rightarrow C + H^{+} + 2e $
}
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.1_2.10.1_2.10.2.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.1_2.11_2.12.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.1_2.13_2.14.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.1_2.15.1_2.15.2.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.1_2.16.1_2.16.2.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.1_2.17.1_2.17.2.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.1_2.18.1_2.18.2.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.1_2.19.1_2.19.2.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.1_2.20_2.21_2.22.eps}
\end{figure}
\newpage
...
\subsubsection{
Reaction 2.23jr    $e + C \rightarrow$ ionization
}
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.1_2.23.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.1_3.1.1.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.1_3.1.2.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.1_3.1.3.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.1_3.1.4.eps}
\end{figure}
\newpage
...
\begin{small}\begin{verbatim}
  ...
  al0 -7.480469649934E+01  al1  2.589186082227E+01  al2 -4.679452112884E 00
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.1_3.2.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.2_2.1_2.2.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.2_2.3_2.4.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.2_2.5_2.6_2.7.eps}
\end{figure}
\newpage
\subsection{
Reaction 2.6       $e + CH_4 \rightarrow CH_2^+ + 2H + 2e: small, out $
}
\subsection{
Reaction 2.7       $e + CH_4 \rightarrow CH^+ + 3H + 2e: small, out $
}
\subsection{
Reaction 2.8      $e + CH_3 \rightarrow CH_2^+ + H + 2e $
}
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.2_2.8_2.9.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.2_2.10.1_2.10.2.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.2_2.11_2.12.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.2_2.13_2.14.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.2_2.15.1_2.15.2.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.2_2.16.1_2.16.2.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.2_2.17.1_2.17.2.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.2_2.18.1_2.18.2.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.2_2.19.1_2.19.2.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.2_2.20_2.21_2.22.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.2_2.23.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.3_3.1.1.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.3_3.1.2.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.3_3.1.3.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.3_3.1.4.eps}
\end{figure}
\newpage
...
\epsfxsize=16truecm
\epsffile{Methane_PS/met.3_3.2.eps}
\end{figure}
...
\bibitem{kn:Janev} R.K. Janev et al.  Springer Series on Atoms+Plasmas,
                   Vol 4, (1987)
\bibitem{kn:Dose}  V. Dose, P. Pecher and R. Preuss: J. Phys. Chem. Ref. Data, \textbf{29}, No. 5, (2000) 1157

\bibitem{kn:JR-2002} R. Janev, D. Reiter, Phys. of Plasmas, \textbf{9}, (2002) 4071
\end{thebibliography}
