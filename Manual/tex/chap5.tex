\chapter{Compiling and developing EIRENE}
\label{sec5}
\section{Compiling EIRENE using cmake}
The EIRENE source code is hosted on the Juelich Gitlab server at \url{https://jugit.fz-juelich.de}. An account is required for access which can be requested at \\ \url{http://eirene.de/cgi-bin/eirene/registration.cgi}. Once you have an account the code can be cloned using git.

\begin{lstlisting}[language=bash,columns=fullflexible]
  > git clone git@jugit.fz-juelich.de:eirene/eirene.git
\end{lstlisting}

The recommended way to compile EIRENE is by using the CMake utility available on multiple platforms \url{https://cmake.org/}. Cmake will ensure that the necessary dependencies are met. EIRENE now uses JSON format input and output files and requires a json library for this purpose. This library must be installed prior to compiling EIRENE.
...
This will by default put the library in /usr/local/jsonfortran-gnu-8.3.0/. You can add \\ -DCMAKE\_INSTALL\_PREFIX=\textless path-to-jsonlib\textgreater if you would like a custom location. If you do not have ford installed the documentation will fail to build, adding -DSKIP\_DOC\_GEN=TRUE to the cmake step will resolve this. Once this is done you can compile EIRENE with
...
The location of the JSON library and the compiler dependent FORTRAN module
files can be specified by environment variables JSONF\_LIB\_DIR and
JSONF\_MOD\_DIR, respectively. This can be utilized e.g. by environment
modules.

If the JSON library and module locations are not specified by environment
variables, cmake tries to find the JSON library automatically.
If cmake can't find the JSON library,
or you have put the JSON library in a non standard location, you can force
the locations with -DLibJSON and -DJSON\_MODULES:
\begin{lstlisting}[language=bash,columns=fullflexible]
  > FC=gfortran cmake -DLibJSON=<full-path-to-jsonlib> \
  -DJSON_MODULES=<directory-of-jsonlib> ../src
\end{lstlisting}

In order to enable OPENMP also add -DOPENMP=ON to the cmake line. In order to use MPI replace gfortran with an MPI compile command such as mpif90. If using the OpenMPI library, it is best to add -DOPEN\_MPI=1 to the build options. You can also replace gfortran with the intel compiler ifort. You can also add -DCMAKE\_BUILD\_TYPE=DEBUG for a debug build.

This will generate a default EIRENE library libEIRENE.a in eirene/libRelease with the Dummy interface and default user-routines, where -j enables multi threaded compilation. EIRENE can also be compiled with different user interfaces. To compile for the structured SOLPS-ITER code

\begin{lstlisting}[language=bash,columns=fullflexible]
  > FC=gfortran cmake -DEIRENE_INTERFACE=SOLPS-ITER \
  -DEIRENE_USER-ROUTINES=iter ../src
\end{lstlisting}

For the Wide Grids SOLPS-ITER version
\begin{lstlisting}[language=bash,columns=fullflexible]
  > FC=gfortran cmake -DEIRENE_INTERFACE=SOLPS_WG \
  -DEIRENE_USER-ROUTINES=iter ../src
\end{lstlisting}

For polygons
\begin{lstlisting}[language=bash,columns=fullflexible]
  > FC=gfortran -DEIRENE_INTERFACE=B2 -DEIRENE_USER-ROUTINES=default \
  -DEIRENE_POSTFIX=_poly ../src
\end{lstlisting}

The variable values correspond to the folder name without prefix (couple\_, user\_) in src/interfaces and src/user-routines.



\section{EIRENE Continuous Integration}
...
\subsection{Continuous Integration Stages}
...
\subsubsection{build:executable}
...
\subsubsection{tag, deploy and on-schedule}
