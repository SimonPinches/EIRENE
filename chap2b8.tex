...
{\sl *** 8. Data for some specific zones}
\begin{verbatim}
NZADD
DO 81 IZADD=1,NZADD
...
\begin{list}{ }{}
\item[{\bf ``CH3-cards" }] (format: 3A,69A)
...
\item[{\bf ``T-cards" }] T, IDION, TEADD, TIADD(IDION)
...
DIIN(IDPLS,ICELL)=DIADD(IDPLS) \item[{\bf ``V-cards" }] V, IDPLS,
...
respectively.
\item[{\bf ``M-cards" }] M, IDPLS, MXADD(IDPLS),
