...
c  eliminate cut cells from balances (lcut(..)), on common and broadcast
...
C                 NCELL=NCLTAL(ITRI)
C                 SCORING OF VOLUME AVERAGED TALLIES IS ON FINE (TRIA) GRID ONLY.
C                 NCLTAL(ITRI)=ITRI
      DO ITRI=1,NTRII
        IY=IYTRI(ITRI)
        IX=IXTRI(ITRI)
...
csw
      ...
      CALL EIRENE_LEER(2)
      CALL EIRENE_HEADNG(' CASE REDEFINED IN COUPLE_SOLPS: ',33)
