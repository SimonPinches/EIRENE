...
c  for short cycle correction terms, in vol. rec. strata.
     ...
     .            EPLODA(:,:), EPEODA(:)
      ...
      REAL(DP), SAVE :: T, V, VL, BVAC, BRAD, BPOL, BTOR
...
C
!pb   INTEGER, SAVE :: MSHFRM, NTRFRM, IO29, IO, IMF
      INTEGER, SAVE :: IO29, IO, IMF
      INTEGER :: N, I, J, K, L, IX, IY, IXI, IXE, IXNI, IPL, JPLS, JUN,
     I           IREAD, IS, IS1, ISC, ISC1, ISC2, ISCS, ISTS, ISTR,
     I           IT, IN, IP, IP1, IR, IR1, IPRT, NTGPRI, IMODE,
     I           ICOADD, ICOSCR, ICOG, ICOU, IDUMMY, IERROR,
     I           IAIN, IAOT, ITRI, NBAR, NUMSI, NBARSI, 
     I           IUNIN_SAVE, IUSROUT, JS
...
C
      ...
      CHARACTER(20) :: FORM
      CHARACTER(72) :: ZEILE
      LOGICAL :: EX, L1, L2, LDUMMY, FOUND, IFBOUND
C
      IERROR=0
      IUSROUT = 0
      IUNIN_SAVE = IUNIN
...
c
      ...
      DO 2200 JPLS=1,NPLSI
        IF (IFLB(JPLS).GT.0) THEN  ! DEAL WITH B2.5 ION SPECIES ONLY, EXCLUDE VIRTUAL EIRENE BACKGROUND
          IPLSV=MPLSV(JPLS)
          DO 2201 IFL=1,NFLA
...
C
     ...
     .           ITRI
      ...
      REAL(DP) :: VPX, VPY, VPZ, VR, VT, VTEST, VTEST2, RP1,
     ...
     .            EADD, ESHEATH, ESUM
      REAL(DP) :: RRBS, UD, UP, RRBC, UDBC, UPBC, VDBC
      REAL(DP), ALLOCATABLE, SAVE ::
     .            TORL(:,:), ESHT(:,:), ORI(:,:)
...
C
        ...
        NMINPTS(ITARG)=INT(NPTCM(ITARG,1)*MPTS_COMSOU) !VK MINIMUM NUMBER OF HISTORIES FOR THE STRATUM
...
C
     ...
     .          SUMM, SUMN, SUMEI, SUMEE, SUMN_OLD, TEST, FLX, FLXI,
     .          RECADD, RECTOT, PIADD, PRESS00, SNICL, FLX_EIR,
...
cdr..  pppl_cop is set, and that part is removed here from paat
cdr    mppl_cop is set, nothing done with it.
cdr....eppl_cop is not set any more, instead that part is directly removed from eapl
cdr    epel_cop is not set any more, instead that part is directly removed from eael
cdr may 2018:  mppl moved up, sync with pppl_cop treatment (lhit condition)
c.......
...
csw             EPPL_COP(INC)=EPPL_COP(INC)+EIADD
                IF (LEAPL) EAPL(IPLS,INC)=EAPL(IPLS,INC)+EIADD
csw
...
CVK SCALING TO MAKE POSSIBLE COMPARISON WITH B2
      ...
      WRITE (iunout,*) 'RECYCLING SOURCE FROM IF3COP ',ISTRAI
        WRITE (iunout,'(3E14.6)') sum(SNIS(1:nfla)), seis, sees
C
C   NEXT:
