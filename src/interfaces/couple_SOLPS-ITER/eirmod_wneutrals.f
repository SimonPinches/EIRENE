...
! converted from g*(cm/s)/cm^3 to kg*(m/s)/m^3
     ...
     &             VZDENA(jatm,in)*vl*10.0/NMASSA(jatm)/AMUAKG
...
! converted from g*(cm/s)/cm^3 to kg*(m/s)/m^3
     ...
     &             VZDENM(jmol,in)*vl*10.0/NMASSM(jmol)/AMUAKG
...
!   so do not use these arrays in until such a check would be performed
     ...
     >       edissml(ix,iy,jmol,istra)=
     &          edissml(ix,iy,jmol,istra)+ pmml(jmol,in)*vl*diss_pot_H2
          endif
        end do
      end do
      do jion=1,nioni
...
C TOTAL AREAS OF THE SURFACES
      ...
      END DO
      SAREA_RES=SAREA_RES*1e-4 !CONVERTING FROM CM**2 TO M**2
...
C****************************************************************************************
  ...
  602 FORMAT(1X,A16,1P,E16.7,A14,E16.7,A6,E16.7)
