...
C
     ...
     .          Y0E, Z0E, SCOSE, SG, XR, X0SA, Y0SA, Z0SA
      INTEGER :: EIRENE_LEARCA, EIRENE_LEARC1, EIRENE_LEARC2, IAN, IEN,
     .           IWEI, NACLLS, MM,
...
C  SET PHI AND X01
      IF (NLTRA) THEN
C  IN CASE OF NLTRA=TRUE, COORDINATES ARE GIVEN IN LOCAL CO-ORDINATE SYSTEM
...
C  USE IPERID INSTEAD
        IF (ILTOR(MASURF).GT.0.AND.ILTOR(MASURF).NE.IPERID) THEN
          CALL
     .  EIRENE_FZRTOR(X0,Z0,ILTOR(MASURF),XR,PHI,NTNEW,NLTEST,IPERID)
...
C
      GOTO 100
C
      ENTRY EIRENE_ADDNOR (X0E,Y0E,Z0E,SCOSE,MSURFE,IPERIDE,*,*)
      X0=X0E
      Y0=Y0E
      Z0=Z0E
      SCOS=SCOSE
