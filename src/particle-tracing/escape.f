...
C  SPECIAL CASE: INCIDENT TEST IONS. SHEATH ACCELARATION.
...
!pb 22.01.2014  taken over only for test 
csw CHECK! 26oct2011
cvk        IF (SG.GT.0.D0.OR.ILIIN(MSURF).EQ.-3) GOTO 90
!VK WARNING: THIS LINE SHOULD BE KEEPT SO TO ENSURE CORRECT COUPLING
!WITH B2. BUT FOR SPECTRUM CALCULATION THIS PART HAS TO BE MODIFIED !!!
        IF ((SG.GT.0.D0).AND.(ILIIN(MSURF).NE.-3))  GOTO 90 !VK
C
C ITYP_OLD=ITNEW=ITYP
C
        IF (ITYP.EQ.1) THEN
