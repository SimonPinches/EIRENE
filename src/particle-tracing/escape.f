...
C
     ...
     .           ITYP_OLD,IGASP_OLD,IGASC_OLD, IOLD, IAD,
...
!$OMP ATOMIC
            ...
            IF (NLSPCSCL_ATM) THEN
              IAD = EIRENE_INDIRECT_ADDRESS(IATM,IATM,NATM)
!$OMP ATOMIC
              PRFAAT(IAD,MSURF)=PRFAAT(IAD,MSURF)+WPR
            END IF
          ENDIF
...
!$OMP ATOMIC
            ...
            IF (NLSPCSCL_MOL) THEN
              IAD = EIRENE_INDIRECT_ADDRESS(IMOL,IMOL,NMOL)
!$OMP ATOMIC
              PRFMML(IAD,MSURF)=PRFMML(IAD,MSURF)+WPR
            END IF
          ENDIF
...
!$OMP ATOMIC
            ...
            IF (NLSPCSCL_ION) THEN
              IAD = EIRENE_INDIRECT_ADDRESS(IION,IION,NION)
!$OMP ATOMIC
              PRFIIO(IAD,MSURF)=PRFIIO(IAD,MSURF)+WPR
            END IF
          ENDIF
...
!$OMP ATOMIC
              PRFPHPHT(IAD,MSURF)=PRFPHPHT(IAD,MSURF)+WPR
            END IF
          ENDIF
...
!$OMP ATOMIC
              ...
              IF (NLSPCSCL_ATM) THEN
                IAD = EIRENE_INDIRECT_ADDRESS(IATM,IATM,NATM)
!$OMP ATOMIC
                PRFAAT(IAD,MSURFG)=PRFAAT(IAD,MSURFG)+WPR
            END IF
            ENDIF
            IF (LERFAAT .OR. LPRFAAT) LMETSPW(NSPH+IATM) =.TRUE.
          ELSEIF (ITYP.EQ.2) THEN
...
!$OMP ATOMIC
              ...
              IF (NLSPCSCL_MOL) THEN
                IAD = EIRENE_INDIRECT_ADDRESS(IMOL,IMOL,NMOL)
!$OMP ATOMIC
                PRFMML(IAD,MSURFG)=PRFMML(IAD,MSURFG)+WPR
              END IF
            ENDIF
            IF (LERFMML .OR. LPRFMML) LMETSPW(NSPA+IMOL) =.TRUE.
          ELSEIF (ITYP.EQ.3) THEN
...
!$OMP ATOMIC
              ...
              IF (NLSPCSCL_ION) THEN
                IAD = EIRENE_INDIRECT_ADDRESS(IION,IION,NION)
!$OMP ATOMIC
                PRFIIO(IAD,MSURFG)=PRFIIO(IAD,MSURFG)+WPR
              END IF
            ENDIF
            IF (LERFIIO .OR. LPRFIIO) LMETSPW(NSPAM+IION) =.TRUE.
          ELSEIF (ITYP.EQ.0) THEN
...
!$OMP ATOMIC
              ...
              IF (NLSPCSCL_PHOT) THEN
                IAD = EIRENE_INDIRECT_ADDRESS(IPHOT,IPHOT,NPHOT)
!$OMP ATOMIC
                PRFPHPHT(IAD,MSURFG)=PRFPHPHT(IAD,MSURFG)+WPR
              END IF
            ENDIF
            IF (LERFPHPHT .OR. LPRFPHPHT) LMETSPW(IPHOT) =.TRUE.
          ENDIF
...
C  ITYP_OLD.NE.ITNEW=ITYP POSSIBLE
C
      CALL EIRENE_UPDATE_SURFACE (ITYP_OLD,IOLD,WEIGHT,2)
