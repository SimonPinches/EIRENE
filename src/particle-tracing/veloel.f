...
cdr           the virtuell bgk background species may be an ion ? (not contributing to electron density, however).
cdr           example He  He+  collisions, when both are test species. or, e.g. elastic component in H + p 
C 
...
C
        ...
        do j=1,1000
          ...
          IF (LHABER) THEN
            CALL EIRENE_SCATANG (ELAB,-1._DP,ELTHDUM,CTCHDUM,SIGHABER)
...
C
        IF (LHABER) THEN
          ...
          RAN=RANF_EIRENE()
          CALL EIRENE_SCATANG (ER,RAN,ELTHDUM,CTCHDUM,SIGHABER)
          CEL= SIGHABER*AU_TO_CM2
        END IF
CH
...
C
      ...
      ENDIF
C
C  STEP 1 FINISHED, COLLIDING BULK ION'S (IPLS) VELOCITY IS SET: VXI,VYI,VZI
C
200   CONTINUE
C
C  FIND TYPE OF COLLISION: IFLAG  (NEXT: THREE TIMES THE SAME CODE, FOR ITYP =1,2,3 ???)
C    IFLAG=0     :  NOTHING TO BE DONE, POST COLLISION VELOCITY IS ALREADY SAMPLED FROM MAXWELLIAN
C    IFLAG=IFTFLG:  FORM OF ITERACTION POTENTIAL (ELASTIC COLLISION IREL)
C
C  NEUTRAL ATOMS:
C
      IF (ITYP.EQ.1) THEN
C  ATOM- TEST-PARTICLE COLLISION, IN BGK APPROXIMATION (E.G.: NEUTRAL-NEUTRAL)
        IF (NPBGKP(IPLS,1).NE.0) THEN
          IFLAG=0
C  INTERACTION POTENTIAL GIVEN
...
C  MOLECULE- TEST-PARTICLE COLLISION, IN BGK APPROXIMATION (E.G.: NEUTRAL-NEUTRAL)
        IF (NPBGKP(IPLS,1).NE.0) THEN
          IFLAG=0
C  INTERACTION POTENTIAL GIVEN
        ELSEIF (MODCOL(5,0,IREL).NE.0) THEN
...
C  TEST-ION- TEST-PARTICLE COLLISION, IN BGK APPROXIMATION (E.G.: NEUTRAL-NEUTRAL)
        IF (NPBGKP(IPLS,1).NE.0) THEN
          IFLAG=0
C  INTERACTION POTENTIAL GIVEN
        ELSEIF (MODCOL(5,0,IREL).NE.0) THEN
...
C
        IF (LHABER) THEN
          RAN=RANF_EIRENE()
          CALL EIRENE_SCATANG (ER,RAN,ELTHETA,CTTETHA,DUMSIGMA)
          PH=ELTHETA
        ELSE
C
