...
CDR  Aug. 2015  : PROGRAMMING AND NOTATION SYNCRONIZED WITH VELOCX.F
!DR
!DR
CDR  5.8.15 : ARGUMENTS ADDED TO VECUSR
cdr  4.9.15 : npbgkp(ipls,1).ne.0, rather then nchrgp(ipls)=0 to identify bgk collisions.
...
C
C  THAT'S ALL WE NEED FOR ISOTROPIC SCATTERING IN COM SYSTEM
        IF (IFLAG.GT.0) THEN
C
