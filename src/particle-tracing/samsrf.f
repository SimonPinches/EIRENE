...
C
     ...
     .           DISH(NPLS),VPSH(NPLS)
      REAL(DP), EXTERNAL :: RANF_EIRENE, EIRENE_SHEATH
...
C  GRID OF TETRAHEDONS. RRSTEP IS A CUMULATED SURFACE AREA, INTEGRATING
C                       OVER A SET OF TETRAHEDON SIDES (=TRIANGLES)
              K=0
              RRSTEP(ISTEP,1) = 0._DP
              INDSRF=INSOR(ISRFS,ISTRA)
              IF (INDSRF < 0) INDSRF=NLIM+ABS(INDSRF)
...
c  ALTERNATIVELY: STEPFUNCTIONS MAY HAVE BEEN DEFINED FROM EXTERNAL MODULS.

C  AVAILABLE SPECIES RANGE: NSPSTI,NSPSTE = 1, NPLSI IS SET
C  INDICATOR: ISTUF(ISTEP)=1 IS SET.

C  FUNCTION STEP(ISTEP,...) CAN BE USED FOR PRESENT STRATUM ISTRA, 
...
C  NEXT: CONTINUE INITIALIZATION FOR STRATUM ISTRA.
C
C
7       CONTINUE
C
C  DEFINE LEFT AND RIGHT BOUNDARY OF SAMPLING INTERVALS FOR STRATUM ISTRA.
C
...
C                  SAMPLE SECOND AND THIRD CO-ORDINATE,
...
C            SAMPLE FIRST AND THIRD CO-ORDINATE,
...
C
     ...
     .                       ALEFT(1,ISRFS,ISTRA),BRGHT(1,ISRFS,ISTRA))
              WRITE (IUNOUT,*) 'SUB-RANGE FOR UNIFORM RANDOM NUMBERS '
              CALL EIRENE_MASR2('XI,XE           ',
     .                       XI(1,ISRFS,ISTRA),XE(1,ISRFS,ISTRA))
              DO IPL=1,NANZ
     ...
     .           EIRENE_STEP0(ISPZ,ISTEP,BRGHT(2,ISRFS,ISTRA))
            IF (TRCSOU) THEN
              WRITE (IUNOUT,*) 'SAMPLING INTERVAL ON STEP FUNCTION '
              CALL EIRENE_MASR2('ALEFT, BRGHT    ',
     .                       ALEFT(2,ISRFS,ISTRA),BRGHT(2,ISRFS,ISTRA))
              WRITE (IUNOUT,*) 'SUB-RANGE FOR UNIFORM RANDOM NUMBERS '
              CALL EIRENE_MASR2('XI,XE           ',
     .                       XI(2,ISRFS,ISTRA),XE(2,ISRFS,ISTRA))
              DO IPL=1,NANZ
                WRITE (IUNOUT,*) 'FLUX ONTO SOURCE SURFACE [A] ',IPL,
     ...
     .           EIRENE_STEP0(ISPZ,ISTEP,BRGHT(3,ISRFS,ISTRA))
            IF (TRCSOU) THEN
              WRITE (IUNOUT,*) 'SAMPLING INTERVAL ON STEP FUNCTION '
              CALL EIRENE_MASR2('ALEFT, BRGHT    ',
     .                       ALEFT(3,ISRFS,ISTRA),BRGHT(3,ISRFS,ISTRA))
              WRITE (IUNOUT,*) 'SUB-RANGE FOR UNIFORM RANDOM NUMBERS '
              CALL EIRENE_MASR2('XI,XE           ',
     .                       XI(3,ISRFS,ISTRA),XE(3,ISRFS,ISTRA))
              DO IPL=1,NANZ
                WRITE (IUNOUT,*) 'FLUX ONTO SOURCE SURFACE [A] ',IPL,
