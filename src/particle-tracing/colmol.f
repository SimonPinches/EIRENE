...
cdr  this NAMIEI is the underlying discrete pdf, which led to the normalized cumulative p2nd(IREI) ?
          ...
          NAMIEI(NSPH+1:NSPA)    = IDINT(PATEI(IREI,1:NATMI))
          NAMIEI(NSPA+1:NSPAM)   = IDINT(PMLEI(IREI,1:NMOLI))
          NAMIEI(NSPAM+1:NSPAMI) = IDINT(PIOEI(IREI,1:NIONI))
...
Cdr  PTOT=0,1,2,etc..., = integer,  number of next generation particles
        ...
        IF (NLCASCAD .AND. (NLEVEL+PTOT <= MAXLEVEL)) THEN  ! PI PROCESS CASCADING MOL
          ...
          END IF
          NAMIPI = 0
          NAMIPI(NSPH+1:NSPA) = IDINT(PATPI(IRPI,1:NATMI))
          NAMIPI(NSPA+1:NSPAM) = IDINT(PMLPI(IRPI,1:NMOLI))
          NAMIPI(NSPAM+1:NSPAMI) = IDINT(PIOPI(IRPI,1:NIONI))
