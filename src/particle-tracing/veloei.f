...
C
C  FETCH A NEW SPECIES INDEX (SAMPLING FROM CUMMULATIVE DISTR. P2ND)
...
C
     ...
     .            VELQ, CVRSS, RSQDV, EFRAC, EDISS, ZEP3, VELDS
      ...
      IF ((ZEP_IN > 0._DP) .AND. (ZEP_IN <= 1._DP)) THEN
        ZEP3 = ZEP_IN
      ELSE ! zep_in = 0.0, 
cdr  random sampling of post collision species from discrete P2ND cummulative distribution 
        ZEP3=RANF_EIRENE( )
      END IF
...
C  ALSO SET EFRAC: FRACTION OF KINETIC ENERGY RELEASE (KER) ASSIGNET TO PARTICULAR
