...
C  N.B.: WE HAVE ALREADY MADE SURE ABOVE, THAT IN CASE NLMOVIE: NPTST = -1
          NPTS(NSTRAI)=NPRNLI
          NMINPTS(NSTRAI)=NPRNLI
        ELSEIF (NPTST.GT.0) THEN
cdr this was ... = IPRNL
cdr I am not sure if the old iprnl setting was needed,
cdr e.g. for the NLMOVIE option (unused since a decade),
cdr      or for the CX cascade splitting onto census
cdr
          NPTS(NSTRAI)=NPTST
          NMINPTS(NSTRAI)=NPTST
