...
C
  500 IF (.NOT.TRCNAL) RETURN
     ...
     .   MIN(NR1ST,N1ST+N2ND+N3RD+NLIM))
      ENDIF
      IF (NP2ND.GT.1) THEN
        WRITE (iunout,*) 'POLOIDAL SURFACES: '
        CALL EIRENE_MASAL1 ('NLSPLT',NLSPLT(N1ST+1:N1ST+NP2ND),NP2ND)
      ENDIF
      IF (NT3RD.GT.1) THEN
        WRITE (iunout,*) 'TOROIDAL SURFACES: '
        CALL EIRENE_MASAL1 ('NLSPLT',
     .                      NLSPLT(N1ST+N2ND+1:N1ST+N2ND+NT3RD),NT3RD)
      ENDIF
      IF (NLIMI.GE.1) THEN
        WRITE (iunout,*) 'ADDITIONAL SURFACES: '
        CALL EIRENE_MASAL1
     .  ('NLSPLT',NLSPLT(N1ST+N2ND+N3RD+1:N1ST+N2ND+N3RD+NLIMI),NLIMI)
      ENDIF
      CALL EIRENE_LEER(1)
      RETURN
      END
