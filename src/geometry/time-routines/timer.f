...
C
      ...
      REAL(DP) :: A(3,3), B(3), AB(3,3), PTS(4)
      REAL(DP) :: SIG1, SIG2, DET, XMU, XETA, EIRENE_SARRUS, AX, AY, V, 
     .          TIMTET,
...
!pb      INTEGER, SAVE :: NSTS_CELL
     ...
     .           LCT1, LCT2, LCT3, LCT4, EIRENE_BITGET
      ...
      EQUIVALENCE (LCTS(1),LCT1), (LCTS(2),LCT2),
     .            (LCTS(3),LCT3), (LCTS(4),LCT4)
      EQUIVALENCE (PTS(1),PT1),   (PTS(2),PT2),
     .            (PTS(3),PT3),   (PTS(4),PT4)
...
C       WRITE (iunout,*) 'TIMER, INIT.: NRCELL,NLSRFX,MRSURF,TT,TL'
C       WRITE (iunout,*)                NRCELL,NLSRFX,MRSURF,TT,TL
C       WRITE (iunout,*) '              NPCELL,NLSRFY,MPSURF'
C       WRITE (iunout,*)                NPCELL,NLSRFY,MPSURF
C     ENDIF
C
      IRCELL=NRCELL
...
10      CONTINUE
      ...
      ENDIF
C     IF (NLTRC) THEN
C       WRITE (iunout,*) 'TIMER, OUT: PT,MRSURF,NINCX'
C       WRITE (iunout,*)              PT,MRSURF,NINCX
C     ENDIF
      RETURN
C
C
...
C
C     IF (NLTRC) THEN
C       WRITE (iunout,*) 'NJUMP,NINCX,NRCELL '
C       WRITE (iunout,*)  NJUMP,NINCX,NRCELL
C     ENDIF
      IF (NJUMP.EQ.0) THEN
...
C     IF (NLTRC)
C    .  WRITE (iunout,*) 'IPOLGN,NRCELL FROM TIMER ',IPOLGN,NRCELL
      IF (NRI.EQ.0) THEN
        RETURN
      ELSE
...
C
      IF (NRCELL.NE.0) THEN
C       IF (NLTRC) WRITE (iunout,*) ' TIMER: IN NEIGHBOR PART'
        LNGB1=.TRUE.
        LNGB2=.TRUE.
        LNGB3=.TRUE.
        LNGB4=.TRUE.
...
6001    CONTINUE
C       IF (NLTRC) WRITE (iunout,*) ' IR,IP,LNGB1,LNGB2,LNGB3,LNGB4',
C    .                           IR,IP,LNGB1,LNGB2,LNGB3,LNGB4
C       IF (NLTRC) WRITE (iunout,*) ' velx, vely, velz ',
C    .                           velx, vely, velz
        T1=-1.D30
        T2=-1.D30
        T3=-1.D30
...
C       IF (NLTRC) WRITE (iunout,*) ' T1,T2,T3,T4 ',T1,T2,T3,T4
...
C       IF (NLTRC) WRITE (iunout,*) ' LCT1,LCT2,LCT3,LCT4 ',
...
C  PLAY SAVE: TRY ONCE AGAIN, IF PARTICLE ON POL. SURFACE
     ...
     .        ' NO INTERSECTION IN TIMER. TRY ONCE AGAIN '
        ...
        NJUMP=1
C       IF (NLTRC) THEN
C         WRITE (iunout,*) ' PT,MRSURF,NINCX,IPOLGN ',
C    .                  PT,MRSURF,NINCX,IPOLGN
C         WRITE (iunout,*) 'NCOUP ',NCOUP
C         DO ICOUP=1,NCOUP
C           WRITE (iunout,*) 'ICOUP,ALPD(ICOUP) ',ICOUP,ALPD(ICOUP)
C         ENDDO
C       ENDIF
        RETURN
      ENDIF
...
C         IF (NLTRC) THEN
C           WRITE (iunout,*) ' SURFACE NO. = ',I
C           WRITE (iunout,*) ' TIMPOL ',(TIMPOL(I,J),J=1,NTIM(I))
C           WRITE (iunout,*) ' IIMPOL ',(IIMPOL(I,J),J=1,NTIM(I))
C         ENDIF
C
        ENDIF
C
...
C
      ...
      IPOLGO=IPOLGN
C
C     IF (NLTRC) WRITE (iunout,*) 'PT,MRSURF,IPOLGN,NINCX ',
...
C
C     IF (NLTRC) THEN
C       WRITE (iunout,*) ' TIMER,NJUMP,NRCELL ',NJUMP,NRCELL
C       WRITE (iunout,*) '       NLSRFX,IPOLG ',NLSRFX,IPOLG
C     ENDIF
C
      IF (NJUMP.EQ.0) THEN
8001    CONTINUE
...
C     ELSEIF (NJUMP.EQ.1) THEN
      ...
      IF (NRCELL > 0) THEN
C       IF (NLTRC) THEN
C         WRITE (iunout,*) ' IZELL,IPOLGO ',IZELL,IPOLGO
C         WRITE (iunout,*) ' X0,Y0 ',X0,Y0
C         WRITE (iunout,*) ' VELX,VELY ',VELX,VELY
C         CALL EIRENE_LEER(1)
C       ENDIF
        XX = X0
        YY = Y0
...
C  NUMMER DER GESCHNITTENEN SEITE DES ALTEN DREIECKS
                NTIM(IZELL) = J
C               IF (NLTRC) THEN
C                 WRITE (iunout,*) 'IZELL,XX,YY,J ',IZELL,XX,YY,J
C               ENDIF
...
C               IF (NLTRC) THEN
C                 WRITE(iunout,*) 'GEHE IN ZELLE ',IZELL,' TM= ',TM
C                 WRITE(iunout,*) 'DORT AUF SEITE IPOLGO ',IPOLGO
C               ENDIF
                GOTO 8050
              ENDIF
            ENDIF
...
C
C       IF (NLTRC)
C    .    WRITE (iunout,*) ' NO INTERSECTION FOUND IN TRIANGLE '
        PT=1.D30
        NINCX=0
...
C  ON NON DEFAULT SURFACE (ADD. OR STD.) ISTS=INMTI(IPOLGN,NRCELL)
      ...
      ELSE
        GOTO 9999
      ENDIF
C     IF (NLTRC) WRITE (iunout,*) ' NRCELL,MRSURF,PT,NINCX,IPOLGN ',
...
C PARTICLE OUTSIDE STANDARD MESH IN CELL NACELL, NRCELL=0
      ...
      DO I=1, NSTS_CELL
        ...
        IF (V.GE.0..AND.V.LE.1.) THEN
          ...
          ENDIF
C         IF (NLTRC)
C    .      WRITE (IUNOUT,*) ' INTERSECTION WITH TRIANGLE ',IZELL,
C    .                       ' SIDE ',J,' FOUND, PT,T = ',PT,T
...
C       IF (NLTRC) THEN
C         WRITE (iunout,*) ' IZELL,IPOLGO ',IZELL,IPOLGO
C         WRITE (iunout,*) ' X0,Y0,Z0 ',X0,Y0,Z0
C         WRITE (iunout,*) ' VELX,VELY,VELZ ',VELX,VELY,VELZ
C       ENDIF
...
!     .          (XMU+XETA <= 1.D0) .AND. (T .GT. 0.)) THEN
                  ...
                  ZZ = ZZ + T * VELZ
C                 if (nltrc) then
C                    write(iunout,*) ' i1, i2 ',i1,i2
C                    write(iunout,'(1x,a,3es14.7)') ' velxyz ',
C    .                     velx,vely,velz
C                    write(iunout,'(1x,a,3es14.7)') ' a11, a12, a13 ',
C    .                     a(1,:)
C                    write(iunout,'(1x,a,3es14.7)') ' a21, a22, a23 ',
C    .                     a(2,:)
C                    write(iunout,'(1x,a,3es14.7)') ' a31, a32, a33 ',
C    .                     a(3,:)
C                    write(iunout,'(1x,a,3es14.7)') ' b1, b2, b3 ',b(:)
C                    write (iunout,'(1x,a,3es14.7)') ' det, xmu ',
C    .                     det,xmu
C                 end IF
                  TM = TM + T
                  TIMTET = TM
C  NUMMER DER GESCHNITTENEN SEITE DES ALTEN DREIECKS
                  NTIMT = J
C                 IF (NLTRC) THEN
C                   WRITE (iunout,*) 'IZELL,XX,YY,ZZ,J ',
C    .                                IZELL,XX,YY,ZZ,J
C                 ENDIF
C  ZELLENNUMMER DES NEUEN DREIECKS
                  IZELLO=IZELL
...
C
C       IF (NLTRC)
C    .    WRITE (iunout,*) ' NO INTERSECTION FOUND IN TETRAHEDRON '
        PT=1.D30
        NINCX=0
C  DURCHSUCHE NACHBARDREIECK
        IF (NLSRFX) THEN
...
C
     ...
     .  (IGJUM3,0,NOPTIM,ITET,IL,0,NBITS)
        ...
        DO IT=1,NTETSUR
          IF ((MRSURF == 0) .AND.
     .        .NOT.EIRENE_BITGET(ISEEOB,0,NLIMI,MSURF,IT,NBITS)) CYCLE
          I = IDROB(IT,1)
          J = IDROB(IT,2)
          IF (I == IZELLO)  CYCLE
          SIG1=SIGN(1,IRICH(1,J))
...
C  SEITENNUMMER DES NEUEN DREIECKS
          IPOLGO = NTMS
C         IF (NLTRC) THEN
C           WRITE(iunout,*) 'OUTSIDE; GEHE IN ZELLE ',NTMZ,' TM= ',TM
C           WRITE(iunout,*) 'DORT AUF SEITE IPOLGO ',IPOLGO
C         ENDIF
        END IF
