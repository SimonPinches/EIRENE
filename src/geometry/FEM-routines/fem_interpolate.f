...
c
      ...
      function eirene_fem_interpolate (fecken, icell, x, y, z, lsame) 
     .         result(res)
...
c           if lsame    : local coordinates r,s,t in cell ICELL are taken from previous call
...
cdr  tbd:
c  if either nltra or nltrt, the z-coordinate is not straight. 
c  Even in 2D  (x,y) tor. symmetric cases this curvature must be
c  taken into account when interpolating vectorial quantities.
c  The interpolated x,z vector components become a function of the z position too.
c  The curvature in z is relative to RMTOR. Hence the X vector component 
c  must also contain RMTOR,
c  or be transformed into a system with RMTOR
...
c  Local coordinates r,s,t,u  are already set in previous call
      ...
      return
      end function eirene_fem_interpolate
