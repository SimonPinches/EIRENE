...
cdr tbd:
c  in 2D cases (even with z or toroidal symmetry, but if NLTRA or NLTRT) the z component is not
c  yet treated. For vectorial tallies, however, interpolation, differentiation, etc..
c  needs to account for a possible curvature in z-coordinate.
cdr oct 18:  rename za... by a..., to avoid confusion with z coordinate, which still needs to
cdr          worked on in 2D (XY) cases, when either NLTRA or NLTRT = true

c  (was formerly:  xyz_to_rst)

      subroutine eirene_fem_local_coord (icell, x1, y1, z1, x2, y2, z2,
     .                              x3, y3, z3,
...
c
     ...
     .                               xce(:), yce(:), a(:), j1(:), j2(:),
     .                               x0(:), y0(:)
      real(dp), allocatable, save :: a23(:), a31(:), a12(:),
     .                               y23(:), y31(:), y12(:),
     .                               x32(:), x13(:), x21(:)
...
! triangle (see chapter 15, IFEM)
        ...
        if (.not.visited(icell)) then
          ...
          a23(icell) = (x2*y3 - x3*y2) * twoai
          a31(icell) = (x3*y1 - x1*y3) * twoai
          a12(icell) = (x1*y2 - x2*y1) * twoai

          visited(icell) = .true.
          visited(0) = .false.  ! reset cell 0 for cell outside mesh
        end if

        r = a23(icell) + y23(icell)*xp + x32(icell)*yp
        s = a31(icell) + y31(icell)*xp + x13(icell)*yp
        t = a12(icell) + y12(icell)*xp + x21(icell)*yp
c  3rd dimension, z-coordinate
        u = 0._dp
...
! tetrahedron (see chapter 15, AFEM)
     ...
     .    + am1(4,4,icell)*zp
      ...
      return
      end subroutine eirene_fem_local_coord
