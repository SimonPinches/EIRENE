...
C
     ...
     .            ADDA(NATM,NSTRA),ADDM(NMOL,NSTRA),
     .            ADDI(NION,NSTRA),ADDPH(NPHOT,NSTRA)
...
C   PUT WEIGHT OF CURRENT CENSUS SCORE IPAN ONTO ADDS
        ...
        ENDIF
140   CONTINUE
C  CONTRIBUTION FROM LAST CENSUS SCORE 
      FLUX(NSTRAI)=FLUX(NSTRAI)+ADDS
      FLX(ISTRO)=FLX(ISTRO)+ADDS
      SGMTOT(ISTRO)=SGMTOT(ISTRO)+ADDS*ADDS
C
...
C     
      IF (TRCCEN) THEN
        DO 950 ISTR=1,NSTRAI
          WRITE (IUNOUT,*) 
     .    'DETAILED CENSUS FLUXES, PER TYPE, FOR STRATUM ISTRA= ',ISTRA 
C   PRINT ADDA(IATM) FOR ISTRA
          WRITE (IUNOUT,*) 'ATOM FLUX AT CENSUS (AMP):'
C   PRINT ADDM(IMOL) FOR ISTRA
