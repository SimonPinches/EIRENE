...
C  DATA FOR REDUCED ENERGY SCALING
      REAL(DP), ALLOCATABLE :: EREDUC(:,:), FREDUC(:,:)
...
C  ENERGY , ABSZISSA FOR REFLECTION PROBABILITY, H INCIDENT ON FE
      DATA
     .   ZENGYS/0.,4.64,10.0,21.5,46.4,100.0,215.4,464.1,1000.0,2154.3,
     .         4641.3,10000.0,21543.0/
C  REFLECTION PROBABILITY RPROB(ENERGY)= ZR(ZENGY)
      DATA
     .   ZRS/1.,0.9,0.8,0.7,0.62,0.543,0.46,0.37,0.29,0.21,0.14,
     .      0.095,0.04/
...
C  I.E. ABSZISSA FOR ENERGY-DISTRIBUTION-FUNCTIONS, H INCIDENT ON FE
      DATA
     .   ZRANGES/0.,6.81,14.7,31.63,68.1,146.8,316.3,681.9,1468.0,
...
C
      ...
      ELSEIF (MODREF.GE.9) THEN
        CALL EIRENE_RF1USR (XMW,XCW,XMP,XCP,IGASF,IGAST,F1,F2,EXPI,
     .               RPROB,E0TERM,*400,*500,*600,*700)
        RETURN
      ELSE
        WRITE (6,*)  'INVALID REFLECTION MODEL PARAMETER '
        WRITE (6,*)  'EXIT CALLED FROM SUBROUTINE EIRENE_REFLEC.F'
        MSS=MSURF
