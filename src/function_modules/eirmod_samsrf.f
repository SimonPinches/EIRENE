...
c                         4 digits: TZYX
        ISOR=IDINT(SORLIM(ISRFS,ISTRAI))
...
C  ISTEP_SPEZ IS 3RD DIGIT "C" OF REAL FLAG SORIND
        ISTEP=MOD(IDINT(REAL(SORIND(ISRFS,ISTRAI),KIND(1.D0))),100)
        ISTEP_SPEZ=IDINT(SORIND(ISRFS,ISTRAI)/100)
C
        IF (ISTEP.EQ.0) THEN
...
C   PARAMETER: SORIND
   40   CONTINUE
          ...
          ISTEP_SPEZ = IDINT(SORIND(NLSF,ISTRA)/100)
          ISPZ=NSPEZ(ISTRA)
          IF (ISTEP_SPEZ.GT.0) ISPZ=ISTEP_SPEZ
