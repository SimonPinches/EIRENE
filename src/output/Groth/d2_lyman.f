...
cmg may 2018   : adapt d2_fulcher for d2_lyman and D2 Lyman band emission
      SUBROUTINE EIRENE_D2_LYMAN (IST,IAD1,IAD2,IAD3,IAD4,IAD5,IAD6,
     .                               IADS,IADL)
c
...
C  PASCHEN GAMMA
      FAC63=7.783E5
C
C  LYMAN BAND (Wunderlich, Yacora, April 2018)
      FACLYM=8.40E8
C
...
C
        IR=NREACI+1
        IF (IR.GT.NREAC) THEN
          WRITE (IUNOUT,*) 'FROM SUBROUTINE EIRENE_D2_LYMAN: '
...
C  H2(n=2,D)/nH2
     ...
     .              '  ',0)
        DO J=1,9
          DO I=1,9
            DLYM(J-1,I-1)=REACDAT(NREACI+1)%OTH%POLY%DBLPOL(J,I)
          ENDDO
        ENDDO
...
C  NOTHING TO BE DONE
     ...
     .             NSIGI_SPC,TRCFLE)
      ELSE
        WRITE (IUNOUT,*) 'ERROR IN D2_LYMAN: DATA FOR STRATUM ISTRA= ',
     .                    IST
        WRITE (IUNOUT,*) 'ARE NOT AVAILABLE. D2_LYMAN ABANDONNED'
        RETURN
      ENDIF
C
...
C
      ...
      IF (MAX(IAD1,IAD2,IAD3,IAD4,IAD5,IAD6,IADS,IADL) > NADV) GOTO 999
...
230     CONTINUE
C
C  CHANNEL LYMAN BAND
C  LYMAN BAND SOURCE RATE:  PHOTONS/SEC/CM**3
C  LINEAR IN PDENM (FULCHER), SAME AS FOR MOLECULAR EMISSION FORM DIATOMIC NEUTRAL
C  HYDR. MOL (SEE CHANNEL 3)
C
        DO 240 IMOL=1,NMOLI
          IF (NCHARM(IMOL).NE.2) GOTO 240
          DDL=DNLY3*PDENM(IMOL,NCELC)
C  RADIATIVE TRANSITION PROB. LEVEL 3-->2 (1/SEC)
C  SIGADL: PHOTONS/SEC/CM**3
          SIGADL=SIGADL+DDL*FACLYM
240   CONTINUE
C
500     CONTINUE
C
...
C
        ...
        ADDV(IADL,NCELC)=ADDV(IADL,NCELC)+SIGADL*VOL(NCELL)
...
cdr tbd: rausziehen hinter 1000, ist eh nur constanter faktor facte, only total power loss
        ...
        POWALFL=POWALFL+SIGADL*FACTE*VOL(NCELL)
C
...
cdr at this place we know: voltal(icoarse)=sum(vol(ifine))
      ...
      ADDV(IADL,1:NSBOX_TAL)=ADDV(IADL,1:NSBOX_TAL)/VOLTAL(1:NSBOX_TAL)
      ADDV(IADS,1:NSBOX_TAL)=ADDV(IADS,1:NSBOX_TAL)/VOLTAL(1:NSBOX_TAL)

      CALL EIRENE_LEER(2)
      CALL EIRENE_FTCRI(IST,CISTRA)
      IF (IST.GT.0) CALL EIRENE_MASBOX
     .   ('SUBR. D2_LYMAN CALLED, FOR STRATUM NO. '//CISTRA)
      IF (IST.EQ.0)
     .CALL EIRENE_MASBOX('SUBR. D2_LYMAN CALLED, FOR SUM OVER STRATA')
      CALL EIRENE_LEER(1)
     ...
     .                  ,POWALF6/FACTE*ELCHA,POWALF6
      WRITE (iunout,*) ' COUPLING TO LYMAN                           :'
     .                  ,POWALFL/FACTE*ELCHA,POWALFL
     ...
     .             NR1TAL,NP2TAL,NT3TAL,NBMLT)
      ...
      DUMMY(1:NSBOX_TAL) = ADDV(IADL,1:NSBOX_TAL)
      CALL EIRENE_INTTAL
     .  (DUMMY,VOLTAL,1,1,NSBOX_TAL,ADDVI(IADL,IST),
     .             NR1TAL,NP2TAL,NT3TAL,NBMLT)
      ADDV(IADL,1:NSBOX_TAL) = DUMMY(1:NSBOX_TAL)
...
C
      ...
      TXTTAL(IADL,NTALA) ='LYMAN, LYMAN BAND SOURCE RATE             '
      TXTSPC(IADL,NTALA) ='MOLECULES               '
      TXTUNT(IADL,NTALA) ='PHOTONS/S/CM**3         '
...
csw 19apr07 -> mg 13mar18
      entry EIRENE_d2_lyman_reinit
      ifirst=0
      return
csw
999   CONTINUE
      WRITE (IUNOUT,*) 'ERROR IN SUBR. EIRENE_D2_LYMAN '
