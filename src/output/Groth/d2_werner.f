...
cmg jan 2018   : reinstate Fulcher band (IADF) emission (Mathias Groth)
cmg may 2018   : rename fulcher.f to d2_fulcher.f to redo routine for another bands
cmg may 2018   : adapt d2_fulcher for d2_werner and D2 Werner band emission
      SUBROUTINE EIRENE_D2_WERNER (IST,IAD1,IAD2,IAD3,IAD4,IAD5,IAD6,
     .                               IADS,IADW)
c
C  SUBROUTINE FOR H EMISSIVITY. (BALMER SERIES)
C  ALSO FOR MOLECULAR BAND EMISSIVITY. (MG: copied from 2006 halpha.f)
C  CALLED FROM EIRENE, SECTION DIAGNO, SUBR. SIGHA
C  THE TOTAL MOLECULAR BAND EMISSIVITY PROFILE (PHOTONS/S/CM**3) IS COMPUTED
...
C  IADW: WERNER BAND EMISSION
C  IADS: SUM OVER ALL HALPHA CONTRIBUTIONS
C
...
C May 18, 2018 (MG): ADDITION OF TALLIES FOR MOLECULAR BAND EMISSION
...
C  PASCHEN GAMMA
      FAC63=7.783E5
C
C  WERNER BAND (Wunderlich, Yacora, April 2018)
      FACWER=1.04E9
C
...
C
      IF (IFIRST.EQ.0) THEN
        ...
        IERROR=0
C
C  READ REDUCED POPULATION COEFFICIENTS FOR HYDR. ATOMS FROM FILE AMJUEL
C  AND PUT THEM FROM REACDAT(NREACI+1,..,..) ONTO DA,DB,DM,DI2,DI3, AND DN ARRAY
C
        IR=NREACI+1
        IF (IR.GT.NREAC) THEN
          WRITE (IUNOUT,*) 'FROM SUBROUTINE EIRENE_D2_WERNER: '
...
C  H2(n=3,D)/nH2
        ...
        REACDAT(NREACI+1)%LOTH = .FALSE.
        CALL EIRENE_SLREAC(NREACI+1,FILNAM,H123,REAC,CRC,
     .              RC1MIN, RC2MAX, FP1, JFEX1MN, JFEX2MX,
     .              RC2MIN, RC2MAX, FP2, JFEX2MN, JFEX2MX,
     .              '  ',0)
        DO J=1,9
          DO I=1,9
            DWER(J-1,I-1)=REACDAT(NREACI+1)%OTH%POLY%DBLPOL(J,I)
          ENDDO
        ENDDO
...
C  NOTHING TO BE DONE
     ...
     .             NSIGI_SPC,TRCFLE)
      ELSE
        WRITE (IUNOUT,*) 'ERROR IN D2_WERNER: DATA FOR STRATUM ISTRA= ',
     .                    IST
        WRITE (IUNOUT,*) 'ARE NOT AVAILABLE. D2_WERNER ABANDONNED'
        RETURN
      ENDIF
C
...
C
      ...
      IF (MAX(IAD1,IAD2,IAD3,IAD4,IAD5,IAD6,IADS,IADW) > NADV) GOTO 999
...
230     CONTINUE
C
C  CHANNEL WERNER BAND
C  WERNER BAND SOURCE RATE:  PHOTONS/SEC/CM**3
C  LINEAR IN PDENM (WERNER), SAME AS FOR MOLECULAR EMISSION FORM DIATOMIC NEUTRAL
C  HYDR. MOL (SEE CHANNEL 3)
C
        DO 240 IMOL=1,NMOLI
          IF (NCHARM(IMOL).NE.2) GOTO 240
          DDW=DNWE3*PDENM(IMOL,NCELC)
C  RADIATIVE TRANSITION PROB. LEVEL 3-->2 (1/SEC)
C  SIGADW: PHOTONS/SEC/CM**3
          SIGADW=SIGADW+DDW*FACWER
240   CONTINUE
C
500     CONTINUE
C
...
C
        ...
        ADDV(IADW,NCELC)=ADDV(IADW,NCELC)+SIGADW*VOL(NCELL)
...
cdr tbd: rausziehen hinter 1000, ist eh nur constanter faktor facte, only total power loss
        ...
        POWALFW=POWALFW+SIGADW*FACTE*VOL(NCELL)
C
...
cdr at this place we know: voltal(icoarse)=sum(vol(ifine))
      ...
      ADDV(IADW,1:NSBOX_TAL)=ADDV(IADW,1:NSBOX_TAL)/VOLTAL(1:NSBOX_TAL)
      ADDV(IADS,1:NSBOX_TAL)=ADDV(IADS,1:NSBOX_TAL)/VOLTAL(1:NSBOX_TAL)

      CALL EIRENE_LEER(2)
      CALL EIRENE_FTCRI(IST,CISTRA)
      IF (IST.GT.0) CALL EIRENE_MASBOX
     .   ('SUBR. D2_WERNER CALLED, FOR STRATUM NO. '//CISTRA)
      IF (IST.EQ.0)
     .CALL EIRENE_MASBOX('SUBR. D2_WERNER CALLED, FOR SUM OVER STRATA')
      CALL EIRENE_LEER(1)
     ...
     .                  ,POWALF6/FACTE*ELCHA,POWALF6
      WRITE (iunout,*) ' COUPLING TO WERNER                         :'
     .                  ,POWALFW/FACTE*ELCHA,POWALFW
     ...
     .             NR1TAL,NP2TAL,NT3TAL,NBMLT)
      ...
      DUMMY(1:NSBOX_TAL) = ADDV(IADW,1:NSBOX_TAL)
      CALL EIRENE_INTTAL
     .  (DUMMY,VOLTAL,1,1,NSBOX_TAL,ADDVI(IADW,IST),
     .             NR1TAL,NP2TAL,NT3TAL,NBMLT)
      ADDV(IADW,1:NSBOX_TAL) = DUMMY(1:NSBOX_TAL)
...
C
      ...
      TXTTAL(IADW,NTALA) ='WERNER, WERNER BAND SOURCE RATE           '
      TXTSPC(IADW,NTALA) ='MOLECULES               '
      TXTUNT(IADW,NTALA) ='PHOTONS/S/CM**3         '
...
csw 19apr07 -> mg 13mar18
      entry EIRENE_d2_werner_reinit
      ifirst=0
      return
csw
999   CONTINUE
      WRITE (IUNOUT,*) 'ERROR IN SUBR. EIRENE_D2_WERNER '
