...
cmg may 2018   : rename fulcher.f to d2_fulcher.f to redo routine for another bands
      ...
      SUBROUTINE EIRENE_D2_FULCHER (IST,IAD1,IAD2,IAD3,IAD4,IAD5,IAD6,
     .                               IADS,IADF)
c
...
C  PASCHEN GAMMA
      FAC63=7.783E5
C
C  FULCHER BAND (Wunderlich, Yacora, April 2018)
      FACFUL=2.43E7
C
...
C
        IR=NREACI+1
        IF (IR.GT.NREAC) THEN
          WRITE (IUNOUT,*) 'FROM SUBROUTINE EIRENE_D2_FULCHER: '
...
C  H2(n=3,D)/nH2
     ...
     .              '  ',0)
        DO J=1,9
          DO I=1,9
            DFUL(J-1,I-1)=REACDAT(NREACI+1)%OTH%POLY%DBLPOL(J,I)
          ENDDO
        ENDDO
...
C  NOTHING TO BE DONE
     ...
     .             NSIGI_SPC,TRCFLE)
      ELSE
        WRITE (IUNOUT,*) 'ERROR IN D2_FULCHER: DATA FOR STRATUM ISTRA=',
     .                    IST
        WRITE (IUNOUT,*) 'ARE NOT AVAILABLE. D2_FULCHER ABANDONNED'
        RETURN
      ENDIF
C
...
C
      ...
      IF (MAX(IAD1,IAD2,IAD3,IAD4,IAD5,IAD6,IADS,IADF) > NADV) GOTO 999
...
230     CONTINUE
C
C  CHANNEL FULCHER BAND
C  FULCHER BAND SOURCE RATE:  PHOTONS/SEC/CM**3
C  LINEAR IN PDENM (FULCHER)
C
        DO 240 IMOL=1,NMOLI
          IF (NCHARM(IMOL).NE.2) GOTO 240
          DDF=DNFU3*PDENM(IMOL,NCELC)
C  RADIATIVE TRANSITION PROB. LEVEL 3-->2 (1/SEC)
C  SIGADF: PHOTONS/SEC/CM**3
          SIGADF=SIGADF+DDF*FACFUL
240   CONTINUE
C
500     CONTINUE
C
...
C
        ...
        ADDV(IADF,NCELC)=ADDV(IADF,NCELC)+SIGADF*VOL(NCELL)
...
cdr tbd: rausziehen hinter 1000, ist eh nur constanter faktor facte, only total power loss
        ...
        POWALFF=POWALFF+SIGADF*FACTE*VOL(NCELL)
C
...
cdr at this place we know: voltal(icoarse)=sum(vol(ifine))
     ...
     .   ('SUBR. D2_FULCHER CALLED, FOR STRATUM NO. '//CISTRA)
      IF (IST.EQ.0)
     .CALL EIRENE_MASBOX('SUBR. D2_FULCHER CALLED, FOR SUM OVER STRATA')
      CALL EIRENE_LEER(1)
     ...
     .                  ,POWALF6/FACTE*ELCHA,POWALF6
      WRITE (iunout,*) ' COUPLING TO FULCHER                         :'
     .                  ,POWALFF/FACTE*ELCHA,POWALFF
     ...
     .             NR1TAL,NP2TAL,NT3TAL,NBMLT)
      ...
      DUMMY(1:NSBOX_TAL) = ADDV(IADF,1:NSBOX_TAL)
      CALL EIRENE_INTTAL
     .  (DUMMY,VOLTAL,1,1,NSBOX_TAL,ADDVI(IADF,IST),
     .             NR1TAL,NP2TAL,NT3TAL,NBMLT)
      ADDV(IADF,1:NSBOX_TAL) = DUMMY(1:NSBOX_TAL)
...
C
      ...
      TXTTAL(IADF,NTALA) ='FULCHER, FULCHER BAND SOURCE RATE         '
      TXTSPC(IADF,NTALA) ='MOLECULES               '
      TXTUNT(IADF,NTALA) ='PHOTONS/S/CM**3         '
...
csw 19apr07 -> mg 13mar18
      entry EIRENE_d2fulcher_reinit
      ifirst=0
      return
csw
999   CONTINUE
      WRITE (IUNOUT,*) 'ERROR IN SUBR. EIRENE_D2_FULCHER '
