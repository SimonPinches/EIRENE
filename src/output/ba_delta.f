...
!dr           changed from 3-->2  to 6-->2
      SUBROUTINE EIRENE_BA_DELTA (IST,IAD1,IAD2,IAD3,IAD4,IAD5,IAD6,
     .                                IADS)
c
C  SUBROUTINE FOR H-DELTA EMISSIVITY. (BALMER SERIES)
C  CALLED FROM EIRENE, SECTION DIAGNO, SUBR. SIGHA
C  THE BA_Delta EMISSIVITY PROFILE (PHOTONS/S/CM**3) IS COMPUTED
...
C
        IR=NREACI+1
        IF (IR.GT.NREAC) THEN
          WRITE (IUNOUT,*) 'FROM SUBROUTINE EIRENE_BA_DELTA: '
...
C  NOTHING TO BE DONE
     ...
     .             NSIGI_SPC,TRCFLE)
      ELSE
        WRITE (IUNOUT,*) 'ERROR IN BA_DELTA: DATA FOR STRATUM ISTRA= ',
     .                    IST
        WRITE (IUNOUT,*) 'ARE NOT AVAILABLE. BA_Delta ABANDONNED'
        RETURN
      ENDIF
C
C  LOOP OVER 2D MESH
...
C
C  CHANNEL 1
C  H DELTA SOURCE RATE:  PHOTONS/SEC/CM**3
C  LINEAR IN PDENA (IONIZATION)
C
        DO 200 IATM=1,NATMI
          IF (NCHARA(IATM).NE.1) GOTO 200
          DDA=DAT*PDENA(IATM,NCELC)
C  RADIATIVE TRANSITION PROB. LEVEL 6-->2 (1/SEC)
C  SIGADD: PHOTONS/SEC/CM**3
          SIGADD1=SIGADD1+DDA*FAC62
200     CONTINUE
C
C  CHANNEL 2
C  H DELTA SOURCE RATE:  PHOTONS/SEC/CM**3
...
1000  CONTINUE
     ...
     .   ('SUBR. BA_DELTA CALLED, FOR STRATUM NO. '//CISTRA)
      IF (IST.EQ.0)
     .CALL EIRENE_MASBOX('SUBR. BA_DELTA CALLED, FOR SUM OVER STRATA')
      CALL EIRENE_LEER(1)
      WRITE (iunout,*) ' AFTER INTEGRATION OVER COMPUTATIONAL DOMAIN'
      WRITE (iunout,*) ' TOTAL FLUX (AMP) AND POWER (WATT) BY BA-DELTA:'
...
csw 19apr07
      entry EIRENE_ba_delta_reinit
      ifirst=0
      return
csw
999   CONTINUE
      WRITE (IUNOUT,*) 'ERROR IN SUBR. BA_DELTA '
