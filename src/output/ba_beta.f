...
!dr           changed from 3-->2  to 4-->2
      SUBROUTINE EIRENE_BA_BETA (IST,IAD1,IAD2,IAD3,IAD4,IAD5,IAD6,
     .                                IADS)
c
C  SUBROUTINE FOR H-BETA EMISSIVITY. (BALMER SERIES)
C  CALLED FROM EIRENE, SECTION DIAGNO, SUBR. SIGHA 
C  THE BA_BETA EMISSIVITY PROFILE (PHOTONS/S/CM**3) IS COMPUTED
...
C
        IR=NREACI+1
        IF (IR.GT.NREAC) THEN
          WRITE (IUNOUT,*) 'FROM SUBROUTINE EIRENE_BA_BETA: '
...
C  NOTHING TO BE DONE
     ...
     .             NSIGI_SPC,TRCFLE)
      ELSE
        WRITE (IUNOUT,*) 'ERROR IN BA_BETA: DATA FOR STRATUM ISTRA= ',
     .                    IST
        WRITE (IUNOUT,*) 'ARE NOT AVAILABLE. BA_BETA ABANDONNED'
        RETURN
      ENDIF
C
C  LOOP OVER 2D MESH
...
1000  CONTINUE
     ...
     .   ('SUBR. BA_BETA CALLED, FOR STRATUM NO. '//CISTRA)
      IF (IST.EQ.0)
     .CALL EIRENE_MASBOX('SUBR. BA_BETA CALLED, FOR SUM OVER STRATA')
      CALL EIRENE_LEER(1)
      WRITE (iunout,*) ' AFTER INTEGRATION OVER COMPUTATIONAL DOMAIN'
      WRITE (iunout,*) ' TOTAL FLUX (AMP) AND POWER (WATT) BY BA-BETA:'
...
csw 19apr07
      entry EIRENE_ba_beta_reinit
      ifirst=0
      return
csw
999   CONTINUE
      WRITE (IUNOUT,*) 'ERROR IN SUBR. BA_BETA '
