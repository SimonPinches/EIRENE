...
C  CORRESPOND ("are consistent") TO THE EIRENE DENSITY TALLIES IF
C  either:
C  THESE SPECIES ARE "NOT FOLLOWED" (NFOL=-1)
C  PLUS IF THE PRODUCTION AND LOSS RATES ARE THE SAME AS THOSE USED TO PRODUCE THE
C  RATIO FITS.
C  or:
C  IF THESE SPECIES ARE NOT INCLUDED, BUT THE CORRESPONDING MULTISTEP RATES INCLUDING THEM
C  AS INTERMEDIATE, FOR THE TRANSPORTED SPECIES TO WHICH THESE "MINORITES" ARE COUPLED.
C
C
C  FIRST: H-/H2  (COUPLED TO H2(V=0)
C
        FILNAM='AMJUEL  '
        H123='H.11'
        REAC='7.0a     '
...
C  H.11 7.0b INCLUDES ELECTRON IMPACT DISS ATTACHMENT ON H2(V=0) ONLY ne=np, Te=Tp, E_H-=0.1
...
C  NEXT : H2+/H2
        ...
        REAC='2.0c     '
        CRC='OT '
C  H.11 2.0c INCLUDES  ION CONVERION (CX) ON H2(V) ne=np,Te=Tp, E_H2=0.1
...
C  to be done: contributions from neutral atomic hydr. in BULK IONS
