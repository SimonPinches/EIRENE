...
C
      SUBROUTINE EIRENE_UPDMOL (XSTOR2,XSTORV2,IFLAG)
C
...
C  DO NOT UPDATE BGK SOURCE RATE TALLIES HERE
          IBGK=NPBGKP(IPLS,1)
C  ELASTIC REACTION IMEL, BETWEEN SPECIES IMOL/IPLS: 
C  IPLS IS A BGK VIRTUAL BACKGROUND SPECIES. 
C  CONTRIBUTIONS TO PMML, PMPL ARE IDENTICALLY ZERO 
C  EMML AND EMPL SHOULD NOT BE UPDATED HERE, BECAUSE THEY ARE SUMMED OVER SPECIES.
...
cdr  it only happens to be correct if the post collision bulk species are allthe same (ipl),
...
C
C  PRESENTLY: PARALLEL COMPONENT OF VSIGCX(IRCX) ISNOT AVAILABLE
...
C  MMPL NEEDS NOT BE UPDATED HERE, ALTHOUGH IT WOULD NOT CAUSE PROBLEMS, BECAUSE 
C       DISTINCT FROM EMPL THIS TALLY DOES HAVE A BULK SPECIES INDEX.
C
C  COLLISION ESTIMATOR IN SUBR. COLLIDE ?
