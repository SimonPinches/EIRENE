...
do
	...
	if [ "$job_state" != "COMPLETED" ]; then
		echo "Job ${job_number} not complete yet, job state is ${job_state}"
		continue
	fi
	...
	cp ./eirene gmon_${N}-${n}-${c}.out slurm-${job_number}.out $case_report_dir/
	printf "Running gprof..."
