...
\begin{list}{}{}
\item[\textbf{N1ST}]  Maximum number of standard mesh points in x-
(radial) direction (block 2a)
\item[\textbf{N2ND}] Maximum number of standard mesh points in y-
(poloidal) direction (block 2b)
\item[\textbf{N3RD}] Maximum number of standard mesh points in z-
(toroidal) direction (block 2c)
\item[\textbf{NADD}] Maximum number of additional zones defined by the
additional surfaces (block 2d)
\item[\textbf{NTOR}] Maximum number of toroidal segments for
approximation of torus by cylindrical segments (block 2c)
\item[\textbf{NLIM}] Maximum number of ``additional surfaces" (block 3b)
\item[\textbf{NSTS}] Maximum number of non-default standard surface
models (block 3a)
\item[\textbf{NPLG}] Maximum number of points in one polygon (block 2a)
\item[\textbf{NPPART}] Maximum number of valid parts in one polygon (block 2a)
\item[\textbf{NKNOT}] Maximum number of knots for mesh of triangles (block 2a)
\item[\textbf{NTRI}] Maximum number of triangles (block 2a)
\item[\textbf{NCOORD}] Maximum number of knots for mesh of
tetrahedrons (block 2a)
\item[\textbf{NTETRA}] Maximum number of tetrahedrons (block 2a)
\item[\textbf{NSTRA}] Maximum number of strata (sources) (block 7)
\item[\textbf{NSRFS}] Maximum number of sub-strata in one stratum (block 7)
\item[\textbf{NSTEP}] Maximum number of different step functions used for
primary source sampling (block 7)
\item[\textbf{NATM}] Maximum number of different atom species (block 4a)
\item[\textbf{NMOL}] Maximum number of different molecule species (block 4b)
\item[\textbf{NION}] Maximum number of different test ion species (block 4c)
\item[\textbf{NPLS}] Maximum number of different bulk ion species (block 5)
\item[\textbf{NADV}] Maximum number of additional volume averaged
tallies, track-length estimated (block 10a)
\item[\textbf{NADS}] Maximum number of additional surface averaged
tallies  (block 10d)
\item[\textbf{NCLV}] Maximum number of additional volume averaged
tallies, collision estimated (block 10b)
\item[\textbf{NSNV}] Maximum number of additional volume averaged
tallies, snapshot estimated (in time dependent mode only) (block 13)
\item[\textbf{NALV}] Maximum number of algebraic
tallies obtained from volume
averaged tallies (block 10c)
\item[\textbf{NALS}] Maximum number of algebraic
tallies obtained from
surface averaged tallies (block 10e)
\item[\textbf{NAIN}] Maximum number of additional input tallies, not
needed by EIRENE, but to facilitate presentation (block 14)
\item[\textbf{NCOP}] Maximum number of tallies for coupling to external
codes (block 14)
\item[\textbf{NBGK}] Maximum number of tallies for iterative \ac{BGK} scheme
for neutral--neutral interactions, see MODUSR, below.
\item[\textbf{NSD}] Maximum number of standard deviation profiles for
volume averaged tallies (block 9)
\item[\textbf{NSDW}] Maximum number of standard deviation profiles
for surface averaged tallies (block 9)
\item[\textbf{NCV}] Maximum number of covariance estimates (block 9)
\item[\textbf{NREAC}] Maximum number of atomic reactions from external
files (block 4)
\item[\textbf{NRRC}] Maximum number of re-combination processes (blocks 4,5)
\item[\textbf{NREI}] Maximum number of electron impact processes
(blocks 4,5)
\item[\textbf{NRCX}] Maximum number of charge-exchange reactions (blocks 4,5)
\item[\textbf{NREL}] Maximum number of elastic reactions
(blocks 4,5)
\item[\textbf{NRPI}] Maximum number of ion impact reactions (blocks 4,5)
\end{list}
...
\begin{list}{}{}
\item[\textbf{NCHOR}] Maximum number of lines of sight for diagnostics module (block 12)
\item[\textbf{NCHEN}] Maximum number of energy values for spectra computed
in diagnostics module (block 12)
\item[\textbf{NDX, NDY, NFL}]~

Problem specific, in case of coupling to external code.
See input block 14. Otherwise (stand alone mode): just set to 1.
\item[\textbf{NPRNL}] Maximum number of particles stored on census array, for
...
\item
INDEX = 0     : \\
Electron temperature (\si{\electronvolt})
\item
INDEX = 1     :\\
Ion temperature (\si{\electronvolt}), NPLSI calls, one for each
ion species. I.e.:
\begin{lstlisting}
      DO IPLS=1,NPLSI
...
\item
INDEX = 1+ NPLS     : \\
Ion density (\si{\per\cubic\centi\metre}), NPLSI calls, one for each
ion species
\item
INDEX = 1+2*NPLS    :   \\
Ion drift velocity, x direction (\si{\centi\metre\per\second}), NPLSI calls, one for each
ion species
\item
INDEX = 1+3*NPLS    :   \\
Ion drift velocity, y direction (\si{\centi\metre\per\second}), NPLSI calls, one for each
ion species
\item
INDEX = 1+4*NPLS    :   \\
Ion drift velocity, z direction (\si{\centi\metre\per\second}), NPLSI calls, one for each
ion species
\item
INDEX = 1+5*NPLS    :   \\
...
\item
INDEX = 5+5*NPLS    :   \\
Cell volume, (\si{\cubic\centi\metre})
\item
INDEX = 6+5*NPLS    :   \\
