...
a large variety of linear and also of non-linear transport problems to be studied. In older versions of EIRENE (2001 and older) in most
...
below in \cref{sec3.1}).  

In it one had defined the
storage requirements for this particular case.

\textbf{In versions EIRENE$_{2002}$ and younger this pre-assignment
...
second line in this block), see \cref{sec2.1}.
...
UPSUSR and UPNUSR given below (\cref{sec3.2}) is meant as a
...
this routine REFUSR is described in \cref{sec3.3}.
...
preprogrammed options described in \cref{sec2.7}.
...
subroutine INPUT. See \cref{sec3.5.1} below. Similarly,
any background data (plasma profiles) may explicitly be modified
in routine PLAUSR (see \cref{sec3.5.2}).
...
in \cref{sec3.6}. It is recommended to use this option,
...
options INDPRO=6, or INDPRO=7, see \cref{sec4}.

The various routines needed for the general user supplied geometry
option (NLGEN, LEVGEO=10, see block 2a) are described in
\cref{sec3.8}. Simple examples are also given there.
\newpage
...
\item[NCOORD] Maximum number of knots for mesh of
...
\item[NSTEP] Maximum number of different step functions used for
...
\item[NADV] Maximum number of additional volume averaged
tallies, track-length estimated (block 10a)
\item[NADS] Maximum number of additional surface averaged
tallies  (block 10d)
\item[NCLV] Maximum number of additional volume averaged
tallies, collision estimated (block 10b)
\item[NSNV] Maximum number of additional volume averaged
tallies, snapshot estimated (in time dependent mode only) (block 13)
\item[NALV] Maximum number of algebraic
tallies obtained from volume
averaged tallies (block 10c)
\item[NALS] Maximum number of algebraic
tallies obtained from
surface averaged tallies (block 10e)
\item[NAIN] Maximum number of additional input tallies, not
needed by EIRENE, but to facilitate presentation (block 14)
\item[NCOP] Maximum number of tallies for coupling to external
codes (block 14)
\item[NBGK] Maximum number of tallies for iterative \ac{BGK} scheme
for neutral--neutral interactions, see MODUSR, below.
\item[NSD] Maximum number of standard deviation profiles for
volume averaged tallies (block 9)
\item[NSDW] Maximum number of standard deviation profiles
for surface averaged tallies (block 9)
\item[NCV] Maximum number of covariance estimates (block 9)
\item[NREAC] Maximum number of atomic reactions from external
files (block 4)
\item[NRRC] Maximum number of re-combination processes (blocks 4,5)
\item[NREI] Maximum number of electron impact processes
(blocks 4,5)
\item[NRCX] Maximum number of charge-exchange reactions (blocks 4,5)
\item[NREL] Maximum number of elastic reactions
(blocks 4,5)
...
\item[NCHEN] Maximum number of energy values for spectra computed
...
\item[NPRNL] Maximum number of particles stored on census array, for
...
The ``volume averaged tallies" estimated by EIRENE (see \cref{tab2})
are profiles of
``responses"
\begin{displaymath}
R_g = <    f | g   >
...
The track-length estimators $R_t$ for detector function $g$ are
updated in subroutine UPDATE (defaults) and UPTUSR (\dots, WV, \dots)
...
collision tallies are updated in UPCUSR(\dots, WS, IND) (additional user
...
weight, \dots) of the particle emerging from the collision. At each
...
They can, however, also be used as a third alternative to track-length and collision
...
The default
snapshot estimated tallies at a fixed time $t^*$, see
\cref{sec2.13}, are updated in subroutine TIMCOL. Currently these are
...
time $t=t^*$, see \cref{est}, i.e., just counting the particle weights at $t=t^*$
provides an unbiased estimate of the particle density.
This is intuitively clear, but also mathematically correct since
these snapshot estimators are formally derived as special cases of track-length- collision
...
(given in input block 7 (\cref{sec2.7}) for each stratum) as a total
number of particles, rather than as flux (particles~\si{\per\second}), i.e.\ ``FLUX is the
integral of the source distribution not only over physical space, but also over time interval $DTIMV$ from initial time $t_0$
to time $t^*$ (\cref{sec2.13}).
...
of block 13 (\cref{sec2.13}) to negative values. Stationary results in
...
DTIMV, \dots,$ i.e.\ at all times $t_n=t_0+n~ DTIMV$ until the flight
...
Finally we note that ``surface averaged tallies" (\cref{tab3})
...
for track-length estimates, equation \cref{tracklest}, now reduce
to:
...
$ . In other words: track-length estimators become formally identical
...
This subroutine returns to EIRENE the (Cartesian) coordinates of
the birth point
...
Only the data for cells from the standard mesh (ICELL = 1, \dots, NSURF)
are used. Default ``vacuum data" are set for all cells in the ``additional
cell region" ICELL = NSURF + 1, \dots, NSBOX.

PROUSR is called, furthermore, from subroutine INPUT to provide cell volumes for all
cells (ICELL = 1, \dots, NSBOX), if the input flag INDPRO(12) has the value INDPRO(12)=5
...
\item[INDEX = 1] Ion temperature (\si{\electronvolt}), NPLSI calls, one for each
ion species. I.e.:
...
\item[INDEX = 1+ NPLS] Ion density (\si{\per\cubic\centi\metre}), NPLSI calls, one for each
ion species
\item[INDEX = 1+2*NPLS] Ion drift velocity, x direction (\si{\centi\metre\per\second}), NPLSI calls, one for each
ion species
\item[INDEX = 1+3*NPLS] Ion drift velocity, y direction (\si{\centi\metre\per\second}), NPLSI calls, one for each
ion species
\item[INDEX = 1+4*NPLS] Ion drift velocity, z direction (\si{\centi\metre\per\second}), NPLSI calls, one for each
ion species
...
flow velocity vector (\si{\centi\metre\per\second}), respectively, in Cartesian coordinates.

Current implementation:
\begin{description}
\item[INDEX = 1] return the magnetic field unit vector, Cartesian coordinates. This local (at position X0, Y0, Z0, in cell NCELL) vector is used
...
Note (see \cref{sec2.2}): Any particular cell in EIRENE can be specified in
...
to detect and exclude flight paths of zero length (time)
...
\end{description}
\item[TIM]
distance (\si{\centi\metre}) from [X0,Y0,Z0]
to the nearest intersection of the trajectory with a
boundary of cell NRCELL.  Since the speed vector [VELX,VELY,VELZ] is normalized to one (\si{\centi\metre\per\second}),
this distance is also the flight time (\si{\second})
\item[ICOS]
...
A is an array of length N, containing the N cell volume values in \si{\cubic\centi\metre}.

\subsection{Subroutine NORUSR}
\label{sec3.8.4}

The subroutine NORUSR provides the outer surface normal unit vector CX,CY,CZ at surfaces,
...
of an intersection point, located on non-default standard surface no.\ M. The routine must return
the surface normal unit vector [CX,CY,CZ].

If surface M has a periodicity condition (i.e.\ surface flag ILIIN(M) $>$3 in input block 3a (\cref{sec2.3a})
