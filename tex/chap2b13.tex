...
run in its own, even if several (NTIME, see section \ref{sec2.1})
...
tallies see Section \ref{sec3.2.3}.
...
{\sl *** 13. Data for nonlinear and time dependent Options }
...
{\bf Meaning of the Input Variables for this block}

\medskip

\begin{list}{ }{}
\item[{\bf NPRNLI }] Total number of test particles in time
...
\item[{\bf NINITL\_READ}]  (new: 2013) Same as NINITL in block 7: provides random number seed for ``time-stratum" (sampling from census
array  (default: =0, no fresh initialization of random number generator for this stratum)
\item[{\bf NPRMUL}] (new: 2013) Multiplicative factor for NPRNLI, in order to increase size of census
...
\item[{\bf NPTST }] Same as NPTS in block 7.
...
user supplied in subroutine UPNUSR, see Sub-section \ref{sec3.2.3}.
...
in a current run or from an earlier run) is {\bf IPRNL}.

The actual number of a score in subroutine TIMCOL is {\bf IPRNLI},
...
stratum ISTRA  is {\bf IPRNLS}, and after
