...
can be read in this input block, their meaning is described in block 6 (\cref{sec2.6})
...
\textsl{*** 3B. Data for Additional Surfaces}
\begin{lstlisting}
      NLIMI
...
followed
by a name \lstinline|modname|. Later, in input block 6 (\cref{sec2.6}),
a ``local reflection model" block with that name
\lstinline|modname| must be included. This option allows
...
\begin{list}{}{}
\item[\textbf{CH0 n1/m1 n2/m2 \dots}]

surfaces from the range n1 to m1, n2 to m2, \dots, are ignored by
...
next. \item[\textbf{NLIMI}] number of surfaces in the input block
\item[\textbf{TXTSUR}] Text to identify a surface (``name of the
surface") on the printout file \item[\textbf{CH1(ILIMI) n1/m1 n2/m2
\dots}]

surfaces from the range n1 to m1, n2 to m2, \dots, are considered
invisible for a
...
\item[\textbf{CH2(ILIMI) n1/m1 n2/m2 \dots}]

Only for second order surfaces ILIMI, n1-m1, \dots
...
\item[\textbf{RLARE}]
Area (in \si{\square\centi\metre}) of the surface element which is seen
...
and $K^+$ (\si{\watt}). These are stored in the POT\dots\ and EOT\dots\ tallies of \cref{tab3}.
If the particle crosses the surface in the direction opposite to the surface normal, then
negative partial particle and energy currents $J^-$ (\si{\ampere})
and $K^-$ (\si{\watt}) are updated. These are stored in the PRF\dots\ and ERF\dots\ tallies of
\cref{tab3}.

\item[= -3]
Net currents (e.g.\ $J^+ - J^-$),
are evaluated, and stored on the POT\dots\ and EOT\dots\ tallies (see \cref{tab3}).
...
overruled by the SORIFL flag (see \cref{sec2.7}), e.g.\ if a
surface source needs to be defined on a surface with ILSIDE = 0.
\item[\textbf{ILEQUI}] The algebraic equations for the surfaces J
...
\item[\textbf{ILBOX}] to be written
\item
[\textbf{ILPLG}] EIRENE can write out information for a finite element
...
\dots, IN, (NN a positive integer) then these IN surfaces form a closed
...
generator (available from FZ-J\"ulich) to produce a triangular
...
are described below (see: \cref{sec2.6}: ``Input Data for Surface
Interaction Models").

The meanings of the input variables for a transformation block are:

\begin{list}{}{}
\item[\textbf{ITINI, ITEND}] ~
