...
in input block 5 (\cref{sec2.5}), or the background volume tallies are simply ready from data files.
...
(see flag NLCRR in input block 1, \cref{sec2.1}) or the semi-implicit
...
close interaction with scientists at FZ J\"ulich may be needed. One also might
...
described with reference to their location and size on a single large, 1D storage array \lstinline|RWK|.
This array \lstinline|RWK| which is then used in the initialization phase of an EIRENE run to bring this information into
regular EIRENE storage places/names.

In Version from 2001 or later, rather than filling 1D array \lstinline|RWK|, now directly
input tallies TEINTF, TIINTF, DIINTF, etc.\ can be filled. These are pointers now to a vector (now named
\lstinline|PLASMA_BACKGROUND|), but which has the same structure as the old \lstinline|RWK| array. Apart from that the rest remains the same
and the material in this section is still valid.

Data are transferred from subroutine INFCOP into EIRENE via the
EIRENE work array \lstinline|RWK|  (Module CSPEI).
They are read onto the EIRENE arrays for input volume tallies (\cref{tab1}) by calls of subroutine PROFR in
...
(see \cref{sec2.5}).
The following addresses are foreseen on \lstinline|RWK| for this data transferring
procedure:
...
In the LEVGEO = 5 option (3D grid of tetrahedrons) THINTF is an array of length NHMAX, which
...
from an external code (e.g.: B2, EMC3, DIVIMP, \dots), the
stratification of the primary source should be such that the first
NTARGI (see \cref{sec2.14}) strata are determined by the plasma
fluxes onto surfaces (``recycling surface sources"). In this case
the surface source distribution $Q_S(\vr,\vv,i,t)$ (\cref{sec1.2}) can be defined automatically from the interfacing
routine, using the data specified in input block 14. The remaining
primary sources NTARGI+1, \dots, NSTRAI (e.g.\ gas puff, volume
...
If INDSRC(ISTRA) = 0--5, then the input flags described in
\cref{sec2.7} are used to define the spatial distribution of a
recycling source on a grid boundary (function STEP(ISTEP), see
\cref{sec2.7.1}) as well as the distribution in velocity
...
block 14, see \cref{sec2.14}, for target recycling source ITARG. In
...
If INDSRC(ISTRA) = 6, then the input flags described in \cref{sec2.14}
...
background plasma \ac{CFD} code, hence module UPTCOP is part of the code interfacing module.
...
Meanwhile these have become default tallies (i.e.\ scoring moved to UPDATE,
 see \cref{sec.tables}. Still some particular
...
As pointed out in \cref{sec2.9}:  for all primary (not derived)
...
Subroutine \lstinline|STATIS_COP| provides these estimates for those
...
At entry \lstinline|IF4COP| (see \cref{sec4.1.4} above) these
