...
described with reference to their location and size on a single large, 1D storage array \verb|RWK|.
This array \verb|RWK| which is then used in the initialization phase of an EIRENE run to bring this information into
regular EIRENE storage places/names.

In Version from 2001 or later, rather than filling 1D array \verb|RWK|, now directly
input tallies TEINTF, TIINTF, DIINTF, \dots etc.\ can be filled. These are pointers now to a vector (now named
\verb|PLASMA_BACKGROUND|), but which has the same structure as the old \verb|RWK| array. Apart from that the rest remains the same
and the material in this section is still valid.

Data are transfered from subroutine INFCOP into EIRENE via the
EIRENE work array \verb|RWK|  (Module CSPEI).
...
(see \cref{sec2.5}).
The following addresses are foreseen on \verb|RWK| for this data transferring
procedure:
...
At entry \verb|IF4COP| (see \cref{sec4.1.4} above) these
...
These are printed from \verb|IF4COP|, together with the overall
balances of a coupled run (\verb|TRCBAL| = \verb|.true.|)
