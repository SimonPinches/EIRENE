...
The statistical performance (\ac{FOM},
see \cref{eq1.13} in \cref{sec1.2.1}) of an EIRENE
...
sampling
flags in this block and estimate the \ac{FOM} from test runs,
and then use this setting, but without evaluation of standard
deviations, in production runs.

The input flag NLANA in block 1 (\cref{sec2.1}) de-activates all non-analog sampling
...
particle trajectories (input block 11, \cref{sec2.11}),
...
IMOL
\item[NLPRCI] conditional expectation estimator is
used for test ion species IION (not ready to use)
\item[NLPRCPH] conditional expectation estimator is
used for photon species IPHOT (in versions 2004 and younger)
\item[IPRSF] conditional expectation estimator is used, if trajectory points
...
(``survival biasing"). If a particle goes into a collision with
...
\item[WMINS] Same as WMINV, but for surface events
...
integrated according to \cref{eq1.12}, until the nearest
...
\cref{sec3.1}.
\item[NSIGI\_BGK]
if \lstinline|NSIGI_BGK| $>$ 0, then the standard deviations are
evaluated for all tallies needed for the iteration procedure for
the non-linear \ac{BGK} collision terms. See subroutine
\lstinline|STATIS_BGK| in code segment BGK.F
\item[NSIGI\_COP]
if \lstinline|NSIGI_COP| $>$ 0, then the standard deviations are
evaluated for all tallies needed for the iteration procedure for
the coupling to a plasma fluid model. The relevant tallies are
selected in subroutine \lstinline|STATIS_COP| in code segment
\lstinline|COUPLE_....F|, \cref{sec4.2}
\item[NSIGI\_SPC]
if \lstinline|NSIGI_SPC| $>$ 0, then the standard deviations are
...
deviation is to be calculated (see \cref{tab2}).
\item[IGHW]
Index of species for selected surface averaged tally
\item[IIHW]
Index of surface averaged tally for which empirical standard
deviation is to be calculated (see \cref{tab3})
\item[IGHC1 IIHC1 IGHC2 IIHC2]~
