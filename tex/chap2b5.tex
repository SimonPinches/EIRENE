...
$\rm{pitch(x)} = b_2/B_{tot}$. From this a cell centered
...
field direction then has components ($0., \rm{pitch},
\sqrt{1-\rm{pitch}^2}$) in the three coordinate directions.
1D simulations (x-grid only) are then across the magnetic field.

An analogous procedure is applied in the second case, but there the pitch is defined as $\rm{pitch(x)} = b_1/B_{tot}$
...
It furthermore also applies to the zone volume array VOL $cm^{-3}$, $INDPRO(12)$.
...
{\sl have } to be specified either explicitly in input block 8
...
{\sl *** 5. Data for Plasma background}
\begin{verbatim}
 NPLSI
 DO 51  J= 1, NPLSI
\end{verbatim}
{\sl * read NPLSI species blocks with \$ = P }
\begin{verbatim}
...
{\bf Meaning of the Input Variables for Plasma Parameters}

\medskip

The meaning of the variables in the bulk ion species cards is as
in block \ref{sec2.4} for the test particle species cards specified there.
...
are expected.
\begin{list}{ }{}
\item[{\bf CDENMODEL = 'Multiply' }]
docu to be written
\item[{\bf CDENMODEL = 'Constant' }]
docu to be written
\item[{\bf CDENMODEL = 'fort.13' }]
...
post processing routines such as the diagnostic module (see Section \ref{sec2.11}),
printout, plotting or iterative mode (input flags NITERI, NITERE, and user subroutine MODUSR, see Section \ref{sec2.1})

\item[{\bf CDENMODEL = 'Saha' }] Set parameters from
Saha-equilibrium of ionization states.
The additional card (only one) reads:
\begin{verbatim}
ISP, ITP, ISTR, ....
\end{verbatim}
to be written \item[{\bf CDENMODEL = 'Boltzmann' }]
Set parameters
...
\end{verbatim}
docu to be written
\item[{\bf CDENMODEL = 'Planck' }]
(only for background photons) to be written
\item[{\bf CDENMODEL = 'Corona' }]
...
\begin{list}{ }{}
\item[{\bf INDPRO }] Flag-array for the type of profile. The
...
supplied profile function. See section \ref{sec3.6}.\\
(NDAT=NSURF) \\

The flag INDEX is as for the ``INDPRO = 6" option, see below, and
sections \ref{sec3.6} and \ref{sec4} . It determines, which of the
...
is determined by the value of INDEX, see section \ref{sec4} below.
...
For further details of subroutine INFCOP see \ref{sec2.1} ``Input
Data for operating mode", input variable NMODE and sections
\ref{sec2.14} and \ref{sec4} for an example.
