...
($INDPRO(5)=1,...3,$ and geometry level $LEVGEO=1,2,3$), the profile input parameters $B1, \dots B5$  control the x- or radial profile
...
the B-field vector in cartesian coordinates ($INDPRO(5)=4,...,7$,
...
For all those: $INDPRO(6)=5,...,7$ in input block 5 only, or the options in input block 14 (routine AMDIAG.f).
...
\end{equation}
Here $Rate_{IS}$ is the rate for excitation from species $IS$ to $IPLS$ from the A\&M data file $FILNAM, ...$,
...
with a reduced population coefficient $POP$ specified via $[FILNAM, H123, ...]$ these are turned 
...
are read, e.g. TE0, ... , TE5 for the $T_e $-profile,
TI0, ... , TI5 for the $T_i $-profile, and so on.
