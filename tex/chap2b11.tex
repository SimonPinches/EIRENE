...
$ in each zone of the computational mesh (see section
\ref{sec2.2}, ``General Remarks").
...
TALUSR (section \ref{sec3.7}). This tally is printed immediately
...
table \ref{tab1}. They are selected for printout or graphical
output by the {\bf negative} value of their tally number. E.g.,
the choice ITALV = 5 selects tally no. 5 from table \ref{tab2},
...
table \ref{tab2} (volume averaged tallies) and in table \ref{tab3}
(surface averaged tallies).

Table \ref{tab1} lists the NTALI  (volume-) tallies which are
...
{\sl  * 11 Data for numerical and graphical output}

{\sl * 11A Block for numerical output written on unit 6 (IUNOUT)}
{\small
\begin{verbatim}
...
\end{verbatim}
 ...
 {\sl  * 11B Data for graphical output}

 {\sl  * 11B.1 Data for 2D and 3D Geometry plot}
{\small
\begin{verbatim}
...
\end{verbatim}
 ...
 {\sl  * 11B.2 Data for plots of volume tallies}
{\small
\begin{verbatim}
 NVOLPL
 PLTSRC(J), J= 0, NSTRAI
...
{\bf Meaning of the Input Variables for numerical and graphical Output}

\medskip

{\sl * 11A. Block for numerical output written on unit 6}
\begin{list}{ }{}
\item[{\bf TRCPLT }]
...
\item[{\bf TRCSRC(ISTRA),ISTRA=0,NSTRAI }] ~
...
\item[{\bf NPRTLV }]
Index of the tally to be printed (first column in table \ref{tab1} or
\ref{tab2}). If the tally has a species index, it is printed for all
species,
...
to be printed. By default all tallies listed in tables (\ref{tab3}, \ref{tab3a}, \ref{tab3old}) (depending on code version)
are printed for this surface.
\item[{\bf NPRTLS(J) NFLAGS(J) NSPEZS(J,1) NSPEZS(J,2)}]
...
printed.
\item[{\bf
NTLSFL(J)}] The spatially resolved tallies (if any) and/or the flux
...
\item[{\bf NUMTAL(J) }]
Number of a tally from table \ref{tab2}, e.g. NUMTAL(J)=1 for neutral atom density tally PDENA.
...
{\sl  * 11B.1 Data for 2D plot of geometry}
...
\end{list}
\item[{\bf PLBOX }] plot box defined by surface inequalities
...
subroutine PLTADD.
\item[{\bf PLNUMV }] print cell number
into standard mesh cells
\item[{\bf PLNUMS }] print numbers
near additional surfaces
\item[{\bf PLARR }] plot arrows to
...
\item[{\bf PLIDL }] print output files for further processing
...
\item[{\bf CH2Y0 }] vertical co-ordinate of midpoint of 2D
plot window (cm).
\item[{\bf CH2Z0 }] distance CONST of
plotting plane to origin.
\item[{\bf NPLINR NPLOTR NPLDLR }]
~
...
{\sl  * 11B.2 Data for 3D plot of geometry}
...
{\sl * 11B3 Graphical output of volume averaged tallies}

\begin{list}{ }{}
\item[{\bf NVOLPL }]
Total number of pictures from volume averaged tallies.
\item[{\bf PLTSRC(ISTRA), (ISTRA=0,NSTRAI) }] ~
