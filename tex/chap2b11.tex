...
ITALV) and NBGV tallies for the (non-linear) \ac{BGK} collision
...
volumes (\si{\per\cubic\centi\metre}) stored in the array VOL(I) are also referred
...
\item[TRCHST] Printout of trajectories of selected test particle histories into
geometry plots.
...
sampling etc.
\item[TRCMOD] Trace-back from routines for iterative mode (MOD\_TMSTEP, MODBGK, and problem
...
\item[TRCFLE] Trace-back from subroutines WRSTRT, WRGEOM, WRPLAS (writing on and
...
\item[TRCINT] Trace back from user specified interfacing routine INFCOP,
...
particle tracing.
\item[TRCSOU] Trace back from primary source sampling routines, e.g.\ from LOCATE,
...
\item[TRCTAL] Print list of activated and de-activated tallies. The default settings eliminate some tallies from storage and
...
\item[NSPCPR] Flag for printout of surface or cell based spectra (defined in input block 10F)
If NSPCPR $> 0$: spectra are printed on output stream fort.(20+ioff), name: spectra.out
\item[NPRTLV] Index of the tally to be printed (first column in \cref{tab1} or
...
\item[NTLVFL] In addition to the standard output stream fort.IUNOUT, this tally is
...
\item[NPRTLS(J) NFLAGS(J) NSPEZS(J,1) NSPEZS(J,2)]
...
tallies, respectively. If the NPRTLS, \dots\ flags are not specified
(i.e.: default=0), then no spatially resolved surface tallies are
printed.
\item[NTLSFL(J)] The spatially resolved tallies (if any) and/or the flux
...
\end{description}
\item[PLBOX] plot box defined by surface inequalities
...
subroutine PLTADD.
\item[PLNUMV] print cell number
into standard mesh cells
\item[PLNUMS] print numbers
near additional surfaces
\item[PLARR] plot arrows to
...
\item[PLIDL] print output files for further processing
...
are printed on files, one file per tally. See calls to routines OUTIDL\dots\ from main program EIRENE.
\item[CH2MX] horizontal
half width of 2D plot window (\si{\centi\metre})
\item[CH2MY]
vertical half width of 2D plot window (\si{\centi\metre})
\item[CH2X0] horizontal co-ordinate of midpoint of 2D plot window (\si{\centi\metre}).
\item[CH2Y0] vertical co-ordinate of midpoint of 2D
plot window (\si{\centi\metre}).
\item[CH2Z0] distance CONST of
...
The following 3 cards reading PL3S, \dots\ 
...
\item[PLTSRC(ISTRA), (ISTRA=0,NSTRAI)] Profiles of the selected
