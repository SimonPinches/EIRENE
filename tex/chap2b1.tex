...
\textsl{*** 1. Data for operating mode}
...
cards starting with character string \texttt{CFILE}  in the input file.
...
The new (2018) flag NPRLL controls the various options for (MPI) parallelisation modes, i.e. the strategies
...
the remarks towards the end of section \ref{sec1.2.1b}).
\item [= 2] not in use
\item [= 3] not in use
...
\item [\textbf{NMODE}] Operating mode
\begin{list}{}{}
\item [= 0 ] EIRENE run as stand-alone-code. Code coupling segment
\texttt{couple\_Dummy.f} may be used. 
Input block 14 has a fixed format, see section \ref{sec2.14}.
\item [$\ne$ 0 ]
EIRENE calls the code coupling subroutine INFCOP in the code coupling segment
%\begin{verbatim}
\verb1 couple_Name.f 1
%\end{verbatim}
...
is called by EIRENE for communication with external data sour\-ces,
...
data are pro\-vi\-ded (overruling corresponding data in input block 2).
...
allocation of CPU time in stratified source sampling is optimized. More details: see paragraph \ref{NFILEK} below.
\item [K = 3 ]
...
specific ``user" routines (see section \ref{sec3}), e.g.: INIUSR, GEOUSR, etc..

Default: NCLTAL(IRAD)=IRAD for IRAD=1,NRAD
\item [\textbf{NREAC\_ADD} ]
...
assumed for the background particles and the input for VXIN,...VZIN is ignored.
...
\item [\textbf{NLCRR} ]
Correlated sampling is used. See discussion at end of section \ref{sec1.7}.
\item [\textbf{NLERG} ]
The case is
automatically reduced to a case for estimating cell volumes by
utilizing an ``ergodic property". More details: see paragraph \ref{NLERG} below.
\item [\textbf{NLIDENT} ]
...
described above, and input block \ref{sec2.13}. More details: see paragraph \ref{NLMOVIE} below.
\end{list}
...
\texttt{CFILE AMJUEL /home/path/AMdata/amjuel.tex}

\texttt{CFILE TRIM /home/path/Surfacedata/TRIM/trim.dat}

Default:
If the specification of an external database name and path,
which is later (e.g. in block \ref{sec2.4} or block \ref{sec2.6}) referred to in
...
which is stored in the same format as the so called ADAS adf11 files and the TRIM
conditional quantil data tables, independent on
whether these tables originate from ADAS or TRIM.

And input variable DBFNAME
specifies only the path to the directory
which is the root of the data tabulated in ADAS (tabular) format and the TRIM format
data tree, respectively.
\\
Example:

\texttt{CFILE ADAS /home/path/AMdata/Adas\_Eirene\_2010/adf11/}
\\
\texttt{CFILE TRMSGL /home/path/Surfacedata/TRIM/}

The particular data file from these trees (e.g.: \texttt{SCD96} in case of ADAS, or \texttt{A\_on\_B}
...
see below, sections \ref{sec2.4}, \ref{sec2.6},
respectively.
...
if proportional allocation of sample size $N_k$ to strata source strength $S_k$ can be achieved. See paragraph \ref{sec1.2.1b}
for more information on stratified source sampling.
The input flag ALLOC in input block \ref{sec2.7} controls this allocation.

Monte Carlo codes such as EIRENE are, in principle,  trivially parallelisable by just distributing the Monte Carlo
trajectory calculation over the available compute nodes.
If parallelisation is activated in combination with stratified source sampling, however, and strata are assigned to compute nodes,
