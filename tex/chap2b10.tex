...
switches ILSWCH (section \ref{sec2.3b}), or are turned off in
input block \ref{sec2.11}.
...
described in more detail in section \ref{sec3.2}. Furthermore
...
Default: If the first card in this block: \texttt{NADVI, .....,}
...
{\sl *** 10. Data for additional tallies}
\begin{verbatim}
NADVI  NCLVI  NALVI  NADSI  NALSI NADSPC
\end{verbatim}
{\sl * 10A. Data for additional track-length estimated volumetric
tallies}
\begin{verbatim}
 ...
 101 CONTINUE
\end{verbatim}
{\sl * 10B. Data for additional collision estimated volumetric
            tallies}
\begin{verbatim}
 ...
 102 CONTINUE
\end{verbatim}
{\sl * 10C. Data for algebraic function of volumetric
             tallies}
\begin{verbatim}
 ...
 103 CONTINUE
\end{verbatim}
{\sl * 10D. Data for additional surface crossing tallies}
\begin{verbatim}
...
{\sl * 10E. Data for algebraic function of
...
{\sl * 10F. Data for energy spectra}
\begin{verbatim}
...
{\bf Meaning of the input variables for additional volume and
surface averaged Tallies}

\medskip

\begin{list}{ }{}
\item[{\bf NADVI }] Total number of additional volume averaged,
track-length estimated tallies
\item[{\bf NCLVI }] Total number of
additional volume averaged, collision estimated tallies
\item[{\bf
NALVI }] Total number of tallies defined as algebraic expressions
of other volume averaged tallies
\item[{\bf NADSI }] Total number
of additional surface averaged tallies
\item[{\bf NALSI }] Total
number of tallies defined as algebraic expressions of other
surface averaged tallies
\item[{\bf NADSPC }] Total number of
surface or cell averaged energy spectra
\end{list}
\subsection{Additional volume averaged tallies, tracklength estimator}\label{sec2.10A}
\begin{list}{ }{}
\item[{\bf IADVE }] flag
...
\begin{list}{ }{}
\item[{= 1 }]
scale tally per unit volume $[1/cm^3]$. The tally is printed and
plotted in the units

$[1/cm^3] \cdot [$units of $g^*] \cdot [cm] \cdot
...
$g$ and $g^*$ see section \ref{sec3.2}, the variable FLUX is explained in
input block no. \ref{sec2.7}. This scaling is default for ``density tallies" of particles, momentum and energy.
\item[{= 2 }]
scale tally per unit cell. Same units as above, however not per
$cm^3$ but per cell instead.
\item[{= 3 }]
...
been in units $[AMP \cdot s / cm^3]$ (``atomic charge density")
rather than $[1/cm^3]$. Energy densities would have been in $[W
\cdot s/ cm^3]$ rather than $[eV/cm^3]$.
\begin{list}{ }{}
\item[{\bf IADVS }]
...
replaced.
\begin{list}{ }{}
\item[{\bf ICLVR }] as IADVR above, for collision estimated tally
(subroutine UPCUSR).
\item[{\bf TXTTAL, TXTSPC, TXTUNT }]: as above
\end{list}
\subsection{Algebraic expression in volume averaged tallies}\label{sec2.10C}
\begin{list}{ }{}
\item[{\bf ALSTRNG }] character string which is interpreted as an
...
tables \ref{tab1} (input tallies) and \ref{tab2} (output tallies) . Note that the first index for
...
interaction in units: \#/s/m$^3$ can be obtained by the line:
...
\ref{sec.tables-old}), and the same expressions in versions 2002
...
tally, by abuse of language (tally number = -14, see table \ref{tab1}).
\item[{\bf TXTTAL, TXTSPC, TXTUNT }]
as above.
\end{list}
\subsubsection{frequently used algebraic tallies}
\begin{itemize}
\item{mean energy}\\
The mean energy (eV) of a test particle species is the ratio of its total energy density (= total pressure) and particle density.
The algebraic tally
...
The kinetic temperature $T_t$ (eV)  is defined such that $3/2  \ T_t$ is the mean thermal energy. The kinetic temperature
...
(\langle i,85\rangle*\langle i,85\rangle+
 ...
 \langle i,1\rangle)$ is the dynamic pressure, scaled to eV\ cm$^3$ units
...
by CVELI2/AMUA/AMUA, (defined in SETCON.F), and the momentum density tallies 85, 89 and 93 are in g cm/s cm$^{-3}$.
\end{itemize}
...
estimators are identical, see section \ref{sec3.2.4}.
...
first dimension N1 of the tally J as listed in table \ref{tab3},
...
\begin{list}{ }{}
\item[{\bf ISRFCLL}] flag to choose between surface and cell averages.
...
{\bf case a:} no direction specified: IDIREC=0
...
{\bf case b:} direction specified: IDIREC=1
...
put into bins in energy units (ISPTYP=3) [cm/s *cm**-3/eV]


\item[{\bf IDIREC, SPCVX, SPCVY, SPCVZ}]
...
\item[{\bf NSPS, SPCMN, SPCMX }] Number of equidistant energy
...
\item[{\bf SPC$\_$SHIFT }] shift spectrum by SPC$\_$SHIFT (eV) in printout and plots.

\item[{\bf SPCPLT$\_$X, SPCPLT$\_$Y }] SPCPLT$\_$X is a flag foreseen to control the
X axis (independent variable) in a plot of the spectrum. (X: e.g. the no. of an energy bin).
This flag is currently not in use.

SPCPLT$\_$Y is a flag to control the
Y axis (dependent variable) in a plot of the spectrum. (Y: e.g. particle or energy flux in an energy bin).
IF SPCPLT$\_$Y $>$ 0: logarithmic scale for Y axix, otherwise: linear scale for Y axix.
.

\item[{\bf SPCPLT$\_$SAME }] plot this spectrum into previous
frame
\end{list}
