...
The first such coupled \acs{CFD} - EIRENE code was SOLXY-EIRENE (\cite{kn:Gerhauser88a})
and had led to
...
linkage between EIRENE and a \ac{CFD} plasma fluid model (one, two,
or
three dimensional).
...
grids into EIRENE, so that the \ac{CFD} and Monte Carlo code operate on identical meshes, without any interpolation
necessary. Also the ``short-cycle" option, a semi-implicit correction scheme for Monte Carlo source terms between
large \ac{CFD} $\Delta t$ cycles, without new or at least with strongly reduced MC runs in between, was implemented.
...
gas excitation population kinetics, etc\dots
