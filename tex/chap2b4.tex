...
\ref{sec3.1} below, for versions EIRENE$_{2000}$ and older), or are determined automatically
from the input file.
...
NREACI
DO I=1,NREACI
IR  FILNAM  H123  REAC  CRC  MASSP  MASST  DP RMN1 RMX1 R2MN R2MX
...
either from external A\&M data files such as \texttt{FILNAM = HYDHEL, = METHAN, = AMJUEL, = H2VIBR},
...
so called ``ADAS database format AFD11",
or any other properly (2D) tabulated
data source, i.e. with \texttt{FILNAM = TAB2D}, or \texttt{FILNAM = ADAS}
...
(e.g. within an ADAS ADF11 file folder).


For photonic emission and absorption data (line shapes), i.e. for \texttt{FILNAM = PHOTON},
...
\end{verbatim}
for reading from the spectroscopic line-shape database \texttt{PHOTON},
...
e.g. comparison with simple analytic or numerical solutions. In this case: \texttt{FILNAM = CONST}, further details: see below.
...
\end{displaymath}
(Very few exceptions, with 3 or more groups of secondaries (N,M,L,...),
...
where {\bf \$} stands for either A (atoms), M (molecules),
...
{\sl *** 4. Data for Species Specification and Atomic Physics Module}

Note: in EIRENE$_{2003}$ or later the format of input flag \texttt{REAC} described below was generalized from
\texttt{A9}  to \texttt{Axxx}, with \texttt{xxx} $\leq 50$,  to accommodate the longer reaction information for photonic
processes into the same format. The old format \texttt{A9} is of course automatically still recognized as special case.
\begin{verbatim}
NREACI
DO
\end{verbatim}
{\bf for ``real particles"}, databases: \texttt{FILNAM = HYDHEL, AMJUEL, METHAN, H2VIBR}
\begin{verbatim}
   IR  FILNAM  H123  REAC  CRC  MASSP  MASST  DP RMN1  RMX1
   (format: I3,1X,A6,1X,A4,Axxx,A3,2I3,3E12.4)
     IF (RMN1.GT.0.) READ IFEXMN, FPARM(J),J=1,3
     IF (RMX1.GT.0.) READ IFEXMX, FPARM(J),J=4,6
\end{verbatim}
{\bf for ``real particles"}, database: \texttt{FILNAM = ADAS}
(or any table in an equivalent format as the ADAS adf11 files)
\begin{verbatim}
   IR  ADAS  H123  REAC  CRC  MASSP  MASST  DP
   (format: I3,1X,A6,1X,A4,Axxx,A3,2I3,3E12.4)
   ELNAME, IZ
...
{\bf for ``real particles"}, database: \texttt{FILNAM = CONST}.
...
\vspace{0.3cm}
{\bf for ``photons"}, database \texttt{FILNAM = PHOTON}
\begin{verbatim}
   IR  PHOTON  H123  REAC
   (format: I3,1X,A6,1X,A4,Axxx)
 ...
 * 4A. atoms species cards
 ...
 DO 44  IATM= 1, NATMI
\end{verbatim}
 {\sl * read NATMI species blocks with \$ = A}
\begin{verbatim}
 44 CONTINUE
 * 4B. molecules species cards
 ...
 DO 46  IMOL= 1, NMOLI
\end{verbatim}
 {\sl * read NMOLI species blocks with \$ = M }
\begin{verbatim}
 46 CONTINUE
 * 4C. test ions species cards
 ...
 DO 48  IION= 1, NIONI
\end{verbatim}
 {\sl * read NIONI species blocks with \$ = I }
\begin{verbatim}
 48 CONTINUE
...
\end{verbatim}
 {\sl * read NPHOTI species blocks with \$ = Ph }
\begin{verbatim}
 49 CONTINUE
\end{verbatim}
...
(see ``Parameter Statements", section \ref{sec3.1}), i.e., one
...
any files tabulated in the same format as ADAS adf11 files,
which must be located in a directory, the path to which is specified in input block 1, section \ref{sec2.1},
by one of the ``CFILE" cards described there:   \texttt{CFILE  ADAS  path/adf11/}
\item[{PHOTON}]  line emission and line absorption constants,
as well as parameters for line shapes (line broadening constants).

\end{list}
\item[{\bf H123 }] Identification flag for the type of data: interaction potential parameters, cross
section, rate coefficient, momentum loss rate coefficient, energy
loss rate coefficient, etc.

{\bf case 0:} FILNAM = CONST
...
\texttt{F(I), I=0,5 (REAL)}\\ \texttt{F(I), I=6,8 (REAL)}\\ are read, in
...
{\bf case 1:} FILNAM = HYDHEL, METHAN,  AMJUEL, H2VIBR
\begin{list}{ }{}
\item[{H.1 }]
single parameter fit for
cross section $\sigma (cm^2)$ as function of energy ELAB (eV),
...
(cm^3/s)$ as function of target temperature (eV), assuming ``zero
velocity" projectile
\item[{H.3 }]
double parameter fit of rate coefficient $\langle \sigma v
\rangle(cm^3/s)$ as function of projectile energy (eV) and target
temperature (eV)
\item[{H.4 }]
double parameter fit of rate coefficient $\langle \sigma v \rangle
(cm^3/s)$ as function of target density $(cm^{-3})$ and target
temperature (eV)
\item[{H.5 }]
single parameter fit of target particle momentum weighted rate
coefficient \\
$\langle~\sigma~v~\cdot~m_p~\cdot~|v_p|~\rangle$ $
(cm^3/s \cdot AMU \cdot cm/s)$ as function of target temperature
(eV)
\item[{H.6 }]
double parameter fit of target particle velocity weighted rate
coefficient \\
$\langle \sigma v  \cdot  m_p  \cdot  | v_p | \rangle
(cm^3/s \cdot AMU \cdot cm/s)$ as function of projectile energy (eV)
and target temperature (eV)
\item[{H.7 }]
double parameter fit of target particle velocity weighted rate
coefficient \\
$\langle \sigma v  \cdot  m_p  \cdot  | v_p | \rangle
(cm^3/s \cdot AMU\cdot cm/s)$ as function of target density $(cm^3)$
and target temperature (eV)
\item[{H.8 }]
single parameter fit of target particle energy weighted rate
coefficient \\
$\langle  \sigma v  \cdot  m_p /2  \cdot  v_p^2 \rangle
(cm^3/s \cdot eV)$ as function of target temperature (eV)
\item[{H.9 }]
double parameter fit of target particle energy weighted rate
coefficient \\
$\langle \sigma v  \cdot  m_p /2  \cdot  v_p^2\rangle
(cm^3/s \cdot eV)$ as function of projectile energy (eV) and target
temperature (eV)
\item[{H.10 }]
double parameter fit of target particle energy weighted rate
coefficient \\
$\langle\sigma~v~\cdot~m_p/2~\cdot~v_p^2\rangle$
$(cm^3/s \cdot eV)$ as function of target density $(cm^3)$ and
...
i.e.,
2.1.5a,....2.1.5e, 2.2.5a, 2.2.5e, etc... \\
...
{\bf case 2:} FILNAM = TAB2D  or FILNAM =ADAS
\begin{list}{ }{}
\item[{H.4 }]
double parameter table of rate coefficient $\langle \sigma v \rangle
(cm^3/s)$ as function of target density $(cm^{-3})$ and target
temperature (eV) read from files containing 2D tabulated rate coefficients, i.e. rate coefficients tabulated vs. 2 independent parameters.
This option uses the ADAS adf11 file format as guidance and is developed/adapted from it for more general
A\&M data structures.

E.g. original ADAS file-names starting with SCD\dots (ionization rate coefficients
...
reaction rate coefficients. In the original ADAS files the names PLT\dots and PRB\dots are used for those quantities. Note:
distinct from the AMJUEL database convention in case of ADAS format the PLT rates (W/cm$^3$) do not contain the potential energy loss
DP $\times$ SCD, and the PRB rates contain, in addition to their corresponding rates from AMJUEL, also
a Bremsstrahlung contribution. Internally EIRENE subtracts this ADAS-Bremsstrahlung from electron energy weighted PRB rates
...
particular database used (ADAS, or AMJUEL, etc.)
\end{list}

{\bf case 3:} FILNAM = PHOTON
to be written
...
directory identified by the path given for ADAS files in input block 1,
see card: ``CFILE" DBHANDLE DBFNAME in block \ref{sec2.1}. For example, REAC = SCD96, then the file \verb+SCD96_h.dat+,
...
In Module \verb+eirene_read-adas.f+, called from \verb+SLREAC.f+, the corresponding data table is then read into EIRENE.
...
onto a cold neutral gas at rest (i.e., the mass in the energy scale is {\bf neither} the reduced mass
\textbf{nor} is the energy given in units energy/AMU).
...
files AMJUEL, HYDHEL, ... data-set already contain that information.
...
next card IFEXMN,.... and IFEXMX,.... must also be omitted, because the
...
RMN1 = minimum energy or temperature  (in eV)
for data as read from file. For energies (cross sections) and temperatures (rate coefficients) below RMN1 (eV) the data are
...
\begin{list}{ }{}
\item[{\bf TEXT\$ }] Name of the species on printout and
plots
\item[{\bf NMASS\$ }] Mass number
\item[{\bf NCHAR\$}] Nuclear charge number
\item[{\bf NPRT\$ }] Flux
...
\item[{\bf NCHRG\$}] Charge state
(irrelevant for atoms and molecules, always set to zero for them)
\item
[{\bf ISRF\$ }] Species index flag for ``fast particle
reflection model" (see block 6A), irrelevant for molecules
\item
[{\bf ISRT\$ }] Species index flag for ``thermal particle
...
for all default volumetric tallies, as described in section \ref{sec3.2}.
\end{list}
...
exchange collisions). If, along a trajectory, the condition $ XGENER > NGEN.. $ is met, after a collision, no test
...
using parameters obtained from the formulas for $n_{AB}, T_{AB}, V_{AB}$ (see section \ref{sec1-bgk})
...
\item[{\bf EDUMMY }]
(was earlier: {\bf ESCD2}) in versions 1999 and older: the parameter ESCD2 = EP was used for velocity space distribution
...
E.g. for reaction $e+CH_4 \rightarrow CH_3 + H + e$, and EP=ESCD=8.0 eV, then
the fragment $CH_3$ would receive an extra energy of 0.5 eV, and the fragment $H$ would
receive 7.5 eV,
...
\end{list}
\medskip
\paragraph{default resonant charge exchange model}
...
undergo re\-so\-nant charge exchange with hydrogenic ions (here: D$^+$ ions), utilizing
...
consequently, sampling is from the a drifting, isotropic and mono-energetic distribution $f_i^{CX}$ (\ref{iso-mono})
...
a) selecting a particular distribution $f_i^{CX}$   (\ref{iso-mono}) rather than e.g. a drifting Maxwellian \\
b) evaluating the reaction rate coefficient by (\ref{sigveff}) rather than a full rate coefficient integration \\
...
product $\sigma(E_{rel}) v_{rel}$ in the hard wired energy range 0.1 to 1.e4 eV,
...
flag  NRCM(IATM)=0 (or NRCM(IMOL)=0, NRCI(IION)=0, resp., or NRCP(IPLS)=0 in input block 5)
...
\paragraph{default model for atomic H (=H,D,T) and He test particles}
...
per ionization of 13.6 and 24.588 eV, respectively.
\medskip
\paragraph{default model for hydrogenic molecules H$_2$ (and isotopomers) and their ions}

For hydrogenic molecules ($\rm H_2, D_2, HT,$...) or molecular ions
($\rm H_2^+, D_2^+, HT^+,$....)  default dissociation, ionization and
...
about $0.896.. \cdot T_e$, see reference \cite{kn:Reiter93}, rather than the perhaps expected $3/2 \cdot T_e$.
...
\medskip
\paragraph{default model for hydrogenic ions}
...
\begin{verbatim}
  1 HYDHEL H.2 2.1.8     RC  0  1   ! single step recombination rate to ground state
  2 HYDHEL H.8 2.1.8     RC  0  1   ! corresponding electron energy loss/gain
\end{verbatim}
...
More generally: the BGK approximation \ref{sec1-bgk} is employed for (bilinear) test-particle  -  test-particle collisions
processes. From the input flags IBGKA(KK), IBGKM(KK), IBGKI(KK) (see above, \ref{sec4}) for a collision processes KK
...
one virtual species for each bgk process KK. The BGK collision integrals are then simulated simply by random sampling the
...
so called ``BGK" collisions, as identified by the process flag IBGK..(KK) .ne. 0.
...
{\bf a) for interaction potentials (elastic processes):}

IFTFLG $=$ 0    :  default, not in use

IFTFLG $=$ 2    :  generalized Morse potential as described in \cite{kn:Bachmann}

{\bf b) for cross sections, rate coefficients, rates:}

IFTFLG $<$ 100  :  rate coeff. or cross section, $[cm^3/s]$ or $[cm^2]$.
...
{\bf c) for photonic processes (line shape profiles:}
...
\medskip
The second argument here controls either the particle energy (in eV) or plasma density (in particles/cm$^3/10^8$) dependence in rate coefficients.
...
In 2008 all data structures for atomic/molecular or photonic processes or CR codes have been
...
LPOT	POT	interaction potential (or: differential cross section, scattering angle, ....) are available
...
LPHR	PHR	 Photonic process (line-shape, Einstein coefficients, population escape factors...) is available
...
\item	CR.. points to a data structure used for collisional radiative models
\end{itemize}
...
Data structure TAB2D\_DATA describes data read from database with tabulated rate coefficients, e.g. ADAS. It consists of two integer variables NDENS and NTEMP denoting the number of densities and temperatures in the fit table.
...
IFLAV: type/flavour of the CR... model (H,H2,He...)
...
The integer array IFLAG = MODCOL(CT,I2=0,...,4,IR) is the internally used identifier for
the various collision process options and related data availability. It is constructed in the initialization
phase (routines XSTEI, XSTCX, XSTEL, XSTPI, ) of a run from the atomic/molecular data read in input block 4
(or hard wired from the minimal default
atomic collision models, in case of NRC..=0 for a certain test particle component).
...
The storage saving mode: atomic/molecular data ``on the fly" routines
