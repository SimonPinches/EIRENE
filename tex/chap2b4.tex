...
either from external A\&M data files such as \verb|FILNAM| = \verb|HYDHEL|, = \verb|METHAN|, = \verb|AMJUEL|, = \verb|H2VIBR|,
...
data source, i.e.\ with \verb|FILNAM = TAB2D|, or \verb|FILNAM = ADAS|
...
For photonic emission and absorption data (line shapes), i.e.\ for \verb|FILNAM = PHOTON|,
...
\end{verbatim}
for reading from the spectroscopic line-shape database \verb|PHOTON|,
...
e.g.\ comparison with simple analytic or numerical solutions. In this case: \verb|FILNAM = CONST|, further details: see below.
...
Note: in EIRENE$_{2003}$ or later the format of input flag \verb|REAC| described below was generalized from
\verb|A9|  to \verb|Axxx|, with \verb|xxx| $\leq 50$,  to accommodate the longer reaction information for photonic
processes into the same format. The old format \verb|A9| is of course automatically still recognized as special case.
\begin{verbatim}
NREACI
DO
\end{verbatim}
\textbf{for ``real particles"}, databases: \verb|FILNAM = HYDHEL, AMJUEL, METHAN, H2VIBR|
\begin{verbatim}
...
\end{verbatim}
\textbf{for ``real particles"}, database: \verb|FILNAM = ADAS|
...
\textbf{for ``real particles"}, database: \verb|FILNAM = CONST|.
...
\vspace{0.3cm}
\textbf{for ``photons"}, database \verb|FILNAM = PHOTON|
\begin{verbatim}
   IR  PHOTON  H123  REAC
   (format: I3,1X,A6,1X,A4,Axxx)
...
by one of the ``CFILE" cards described there:   \verb|CFILE  ADAS  path/adf11/|
...
\verb|F(I), I=0,5 (REAL)|\\ \verb|F(I), I=6,8 (REAL)|\\ are read, in
