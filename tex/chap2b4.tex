...
(\si{\cubic\centi\metre\per\second})$ as function of target temperature (eV), assuming ``zero
velocity" projectile
\item[{H.3 }]
double parameter fit of rate coefficient $\langle \sigma v
\rangle(\si{\cubic\centi\metre\per\second})$ as function of projectile energy (eV) and target
temperature (eV)
\item[{H.4 }]
double parameter fit of rate coefficient $\langle \sigma v \rangle
(\si{\cubic\centi\metre\per\second})$ as function of target density $(cm^{-3})$ and target
temperature (eV)
\item[{H.5 }]
single parameter fit of target particle momentum weighted rate
coefficient \\
$\langle~\sigma~v~\cdot~m_p~\cdot~|v_p|~\rangle$ $
(\si{\cubic\centi\metre\per\second} \cdot AMU \cdot \si{\centi\metre\per\second})$ as function of target temperature
(eV)
\item[{H.6 }]
double parameter fit of target particle velocity weighted rate
coefficient \\
$\langle \sigma v  \cdot  m_p  \cdot  | v_p | \rangle
(\si{\cubic\centi\metre\per\second} \cdot AMU \cdot \si{\centi\metre\per\second})$ as function of projectile energy (eV)
and target temperature (eV)
\item[{H.7 }]
double parameter fit of target particle velocity weighted rate
coefficient \\
$\langle \sigma v  \cdot  m_p  \cdot  | v_p | \rangle
(\si{\cubic\centi\metre\per\second} \cdot AMU\cdot \si{\centi\metre\per\second})$ as function of target density (\si{\cubic\centi\metre})
and target temperature (eV)
\item[{H.8 }]
single parameter fit of target particle energy weighted rate
coefficient \\
$\langle  \sigma v  \cdot  m_p /2  \cdot  v_p^2 \rangle
(\si{\cubic\centi\metre\per\second} \cdot eV)$ as function of target temperature (eV)
\item[{H.9 }]
double parameter fit of target particle energy weighted rate
coefficient \\
$\langle \sigma v  \cdot  m_p /2  \cdot  v_p^2\rangle
(\si{\cubic\centi\metre\per\second} \cdot eV)$ as function of projectile energy (eV) and target
temperature (eV)
\item[{H.10 }]
double parameter fit of target particle energy weighted rate
coefficient \\
$\langle\sigma~v~\cdot~m_p/2~\cdot~v_p^2\rangle$
$(\si{\cubic\centi\metre\per\second} \cdot eV)$ as function of target density (\si{\cubic\centi\metre}) and
...
distinct from the AMJUEL database convention in case of \ac{ADAS} format the PLT rates (W\si{\per\cubic\centi\metre}) do not contain the potential energy loss
...
IFTFLG $<$ 100  :  rate coeff. or cross section, $[\si{\cubic\centi\metre\per\second}]$ or $[cm^2]$.
