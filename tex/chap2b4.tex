...
(A\&M data sets) to each of them from external or internal databases. This is described next:
\bigskip
...
Here $A,B,M$ and $N$ are labels for the type of pre- and post-collision particles. $a,b,m$ and $n$ are the stoichiometric
...
\end{description}
\item[H123] Identification flag for the type of data: interaction potential parameters, cross
...
EIRENE post-processing routines in code section ``output", routines: Ba\_alpha, Ba\_beta, \dots
\end{description}
...
\begin{description}
\item[TEXT\$] Name of the species on printout and
plots
...
\item[NCHRG\$] Charge state
(irrelevant for atoms and molecules, always set to zero for them)
\item[ISRF\$] Species index flag for ``fast particle
reflection model" (see block 6A), irrelevant for molecules
\item[ISRT\$] Species index flag for ``thermal particle
...
the species ISPZ itself. Otherwise it must point to the \ordinalnum{2} test particle
...
are controlled by the input flag ISCDE, as well as by the energies EELEC, EBULK (pre-collision
...
The default CX model is set up in order to define collision kinetics and mean free paths without
...
in symmetric charge-exchange), scattering angle $\pi$ in center of mass system, zero exothermicity,
...
one virtual species for each \ac{BGK} process KK. The \ac{BGK} collision integrals are then simulated simply by random sampling the
...
so called ``\acs{BGK}" collisions, as identified by the process flag IBGK\dots(KK) .ne.\ 0.
If that is the case, this test particle species is labeled, additionally to its label ISPZ1, also as a ``\acs{BGK}-species" no.\ IBGK\_SP
and the corresponding internal flags NPBGKA(IATM), NPBGKM(IMOL) or NPBGKI(IION) are non-zero for this species.
For each ``\acs{BGK}-species" IBGK\_SP automatically additional tallies, so called ``\acs{BGK}-tallies" are scored,
...
to a particular \acs{BGK} collision process IBGK and hence also to a particular test particle species ISPZ1, which
collides with this bulk species IPLS via that \acs{BGK} collision kernel no.\ IBGK.
...
\begin{description}
\item[CT = 1] electron impact processes (`EI' in the names of corresponding variables,
...
\item[CT = 2] not in use. In very old versions (1990 and older): DS, dissociation process, now combined with EI type processes
...
\item[I2 = 1] format/availability of cross section data: H.1.  Cross sections are needed, in addition to rate coefficients,
