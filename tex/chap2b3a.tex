...
Their definition is described in subsection \ref{sec2.3a}.
...
Their definition and properties are described in subsection \ref{sec2.3b}.
...
surface averaged tallies (see block \ref{sec2.11}, end of section
\ref{sec3.2} and Table \ref{tab3}) are only evaluated on either
``Additional Surfaces'' (subsection \ref{sec2.3b}) or on these ``Non
...
block, their meaning is described in block \ref{sec2.6} together
with the globally valid input data for particle-surface
interactions.

In any particular run there are NLIM ``additional surfaces", see
subsection \ref{sec2.3b}. For many arrays containing surface
...
{\sl *** 3A. Data for ``Non-default Standard Surfaces''}
\begin{verbatim}
 NSTSI
 DO 31 ISTSI=1,NSTSI
\end{verbatim}
...
{\bf Meaning of the Input Variables for ``non-default standard
surfaces"}
...
described below, see \ref{sec2.6} : ``Input Data for Particle-Surface Interaction
Models''.
